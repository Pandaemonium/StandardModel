...
theorem rho_intertwine (v w u : V10) (ψ : FockSpinor) :
  ...
  · have h_anticomm : ∀ (a b : V10) (χ : FockSpinor), cliffordAction a (cliffordAction b χ) = B10 a b • χ - cliffordAction b (cliffordAction a χ) := by
    ...
    rw [show cliffordAction v (cliffordAction w ψ) = B10 v w • ψ - cliffordAction w (cliffordAction v ψ) from h_anticomm v w ψ]
    rw [show cliffordAction w (cliffordAction v (cliffordAction u ψ)) = B10 w v • cliffordAction u ψ - cliffordAction v (cliffordAction w (cliffordAction u ψ)) from h_anticomm w v (cliffordAction u ψ)]
    ring
    rw [show cliffordAction v (cliffordAction w (cliffordAction u ψ)) = B10 v w • cliffordAction u ψ - cliffordAction w (cliffordAction v (cliffordAction u ψ)) from h_anticomm v w (cliffordAction u ψ)]
    rw [show cliffordAction v (cliffordAction u ψ) = B10 v u • ψ - cliffordAction u (cliffordAction v ψ) from h_anticomm v u ψ]
    ring
    rw [show B10 w v = B10 v w by exact B10_comm v w ▸ rfl]
    norm_num [mul_assoc, mul_comm, mul_left_comm, smul_smul]
    ring
    rw [show cliffordAction w (B10 v u • ψ - cliffordAction u (cliffordAction v ψ)) = B10 v u • cliffordAction w ψ - cliffordAction w (cliffordAction u (cliffordAction v ψ)) from ?_]
    rw [show cliffordAction u ((1 / 2 : ℂ) • (B10 v w • ψ - cliffordAction w (cliffordAction v ψ) * 2)) = (1 / 2 : ℂ) • (B10 v w • cliffordAction u ψ - cliffordAction u (cliffordAction w (cliffordAction v ψ)) * 2) from ?_]
    ring
    · rw [show B10 w u = B10 u w by exact B10_comm u w ▸ rfl]
      rw [h_anticomm u w (cliffordAction v ψ)]
      ring
      ext; norm_num; ring
    · convert cliffordAction_smul_spinor (1 / 2 : ℂ) u (B10 v w • ψ - cliffordAction w (cliffordAction v ψ) * 2) using 1
      norm_num [mul_assoc, mul_comm, mul_left_comm, smul_smul]
      ring
      rw [show cliffordAction u (B10 v w • ψ - cliffordAction w (cliffordAction v ψ) * 2) = B10 v w • cliffordAction u ψ - cliffordAction u (cliffordAction w (cliffordAction v ψ) * 2) from ?_]
      ring
