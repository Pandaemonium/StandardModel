...
/-!
# Draft.NullEdgeP2ChiralProjectorCoherence

Finite two-level chiral Hamiltonian bridge for the null-edge P2/P4/P7 story.

The module isolates the matrix

`H_h(p,m) = [[-h p, m], [m, h p]]`,

where the off-diagonal scalar `m` is the finite Higgs/Yukawa chirality-coupling
entry.  When the helicity sign satisfies `h^2 = 1`, this first-order operator
squares to the mass-shell scalar `(p^2 + m^2) I`.  Its positive-branch matrix
`(1/2)(I + H/E)` has left/right coherence `m/(2E)`, whose square is the
observer-channel mass-ratio scalar `m^2/(4E^2)`.

This is finite matrix algebra only.  It does not prove a continuum Dirac limit,
but it gives a compact operator-level bridge from the mass block to the
observer-normalized `m/E` readout.
-/

noncomputable section

namespace PhysicsSM.Draft.NullEdgeP2ChiralProjectorCoherence
...
theorem positiveBranch_leftRight_sq_eq_massRatioSq
  ...
  rw [positiveBranch_leftRight_eq_mass_over_two_energy]
  ring

end PhysicsSM.Draft.NullEdgeP2ChiralProjectorCoherence
