...
/-!
# Self-duality of integral unimodular lattices

General lemma: for the inner product bilinear form on `EuclideanSpace ℝ (Fin n)`,
if a basis has Gram matrix with integer entries and determinant ±1, then the dual lattice
equals the original lattice.
-/
...
lemma span_dualBasis_le_span_basis (n : ℕ)
  ...
  have h_comb : ∀ k, ∑ j, ( (gramMatrixZ n b hInt)⁻¹ k j : ℤ) • b j = (ipBilin n).dualBasis hB b k := by
    ...
    have h_comb : ∀ k i, ∑ j, ( (gramMatrixZ n b hInt)⁻¹ k j : ℤ) * (ipBilin n) (b j) (b i) = if k = i then 1 else 0 := by
      ...
      rw [ ← @Int.cast_inj ℝ ] ; push_cast ; congr! 2 ; ring!;
