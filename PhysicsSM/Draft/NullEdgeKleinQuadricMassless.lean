...
namespace PhysicsSM.Draft.NullEdgeKleinQuadricMassless
...
theorem massless_iff_repeated_principal_spinor (a b : Spinor) (ha : a ≠ 0) :
  ...
  constructor <;> intro h
  · -- Since `a ≠ 0`, there exists some `k` such that `a k ≠ 0`.
    obtain ⟨k, hk⟩ : ∃ k, a k ≠ 0 := Function.ne_iff.mp ha
    refine ⟨b k / a k, ?_⟩
    ext i
    fin_cases k <;> fin_cases i <;> simp_all +decide [pluckerBracket] <;> grind
  · obtain ⟨t, rfl⟩ := h
    unfold pluckerBracket
    ring

end PhysicsSM.Draft.NullEdgeKleinQuadricMassless
