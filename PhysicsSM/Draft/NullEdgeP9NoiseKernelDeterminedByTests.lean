...
namespace PhysicsSM.Draft.NullEdgeP9NoiseKernelDeterminedByTests
...
theorem noiseResponse_pairTest_eq_diag_diag_cross
  ...
  rw [Finset.sum_eq_add c d hcd
    (fun x _ hx => by simp [if_neg hx.1, if_neg hx.2]) (by simp) (by simp)]
  rw [Finset.sum_eq_add c d hcd
    (fun x _ hx => by simp [if_neg hx.1, if_neg hx.2]) (by simp) (by simp)]
  rw [Finset.sum_eq_add c d hcd
    (fun x _ hx => by simp [if_neg hx.1, if_neg hx.2]) (by simp) (by simp)]
  simp only [if_true, ite_self, one_mul]
  linarith [noiseKernel_symm s d c]
...
theorem noiseKernel_eq_zero_of_all_noiseResponses_zero
  ...
  intro c d
  by_cases hcd : c = d
  · convert h (deltaTest c) using 1
    rw [noiseResponse_deltaTest_eq_noiseKernel_self, hcd]
  · have hc := h (deltaTest c)
    have hd := h (deltaTest d)
    have hpair := h (pairTest c d)
    rw [noiseResponse_deltaTest_eq_noiseKernel_self] at hc hd
    rw [noiseResponse_pairTest_eq_diag_diag_cross s c d hcd] at hpair
    linarith

end PhysicsSM.Draft.NullEdgeP9NoiseKernelDeterminedByTests
