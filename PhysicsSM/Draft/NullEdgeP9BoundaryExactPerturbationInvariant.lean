...
namespace PhysicsSM.Draft.NullEdgeP9BoundaryExactPerturbationInvariant
...
theorem boundaryExact_perturb_configResponse_eq
  ...
  obtain ⟨pp, hpp⟩ := hexact
  unfold configResponse
  simp only [addSource, hpp, mul_add, Finset.sum_add_distrib]
  rw [hclosed (pp w), add_zero]
...
theorem boundaryExact_perturb_meanResponse_eq
  ...
  rw [boundaryExact_perturb_configResponse_eq B s p t hclosed hexact w]
  rfl
...
theorem boundaryExact_perturb_responseNoise_eq
  ...
  refine Finset.sum_congr rfl fun w _ => ?_
  rw [boundaryExact_perturb_configResponse_eq B s p t hclosed hexact w,
    boundaryExact_perturb_meanResponse_eq B s p t hclosed hexact]
  rfl

end PhysicsSM.Draft.NullEdgeP9BoundaryExactPerturbationInvariant
