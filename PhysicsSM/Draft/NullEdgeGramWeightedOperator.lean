...
/-!
# Gram-weighted operator positivity

This draft module proves the finite operator-level positivity bridge for a
visible momentum obtained by tracing hidden labels with an internal Gram
matrix.

The determinant-level companion lives in
`PhysicsSM.Draft.NullEdgeGramWeightedMassAristotle`. Here the point is
quadratic forms: `v^dagger P_vis v` is the hidden Gram quadratic form evaluated
on the spinor projections of `v`. Thus a positive hidden Gram form induces a
positive visible momentum operator.
-/

noncomputable section

namespace PhysicsSM.Draft.NullEdgeGramWeightedOperator
...
theorem visibleQuadraticForm_gramWeighted_eq_gramQuadraticForm
  ...
  unfold visibleQuadraticForm operatorGramWeightedVisibleMomentum gramQuadraticForm
    spinorProjection
  simp only [map_sum, map_mul, RingHomCompTriple.comp_apply, RingHom.id_apply,
    Finset.mul_sum, Finset.sum_mul, ← Finset.sum_product']
  refine Finset.sum_nbij' (fun x => (x.2.2.1, x.2.2.2, x.2.1, x.1))
    (fun y => (y.2.2.2, y.2.2.1, y.1, y.2.1)) ?_ ?_ ?_ ?_ ?_
  · intro x _
    simp
  · intro y _
    simp
...
theorem gramPositiveSemidefinite_implies_visiblePositiveSemidefinite
  ...
  intro v
  rw [visibleQuadraticForm_gramWeighted_eq_gramQuadraticForm]
  exact hG _

end PhysicsSM.Draft.NullEdgeGramWeightedOperator
