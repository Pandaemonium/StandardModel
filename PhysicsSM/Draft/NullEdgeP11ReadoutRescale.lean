...
namespace PhysicsSM.Draft.NullEdgeP11ReadoutRescale
...
theorem scaled_same_normalized_state_different_mass
    ...
    normalize ((2 : Complex) • P) = normalize P ∧
      massSq ((2 : Complex) • P) ≠ massSq P := by
  constructor
  · exact normalize_smul_eq_normalize (2 : Complex) P (by norm_num)
  · rw [massSq_smul_eq_sq_mul]
    intro h
    have hzero : (((2 : Complex) ^ 2 - 1) * massSq P) = 0 := by
      calc
        ((2 : Complex) ^ 2 - 1) * massSq P =
            (2 : Complex) ^ 2 * massSq P - massSq P := by ring
        _ = 0 := by
          rw [h]
          ring
    have hcoef : ((2 : Complex) ^ 2 - 1) ≠ 0 := by norm_num
    exact hdet ((mul_eq_zero.mp hzero).resolve_left hcoef)

end PhysicsSM.Draft.NullEdgeP11ReadoutRescale
