...
This module records a key P9 guardrail:
...
but its celestial moment mass is positive. When that mass is used as a one-face
source, it is visible to the unit bulk test.

Proven by Aristotle project `e3558146-26c9-4a88-9e3f-df06bc6c52d1`
on 2026-06-22 from the focused standalone package
`null-edge-p9-visible-closure-source-20260622`.
-/

noncomputable section

namespace PhysicsSM.Draft.NullEdgeP9VisibleClosureSource
...
theorem twoRayClosure_eq_zero :
    twoRayClosure = 0 := by
  funext k
  simp only [twoRayClosure, dirPlus, dirMinus, Pi.zero_apply]
  split <;> ring

/-- The same closed visible fan has positive unit mass square. -/
theorem twoRayMomentMassSq_eq_one :
    twoRayMomentMassSq = 1 := by
  have h : normSq twoRayClosure = 0 := by
    simp only [normSq, dot, twoRayClosure_eq_zero, Pi.zero_apply, mul_zero,
      Finset.sum_const_zero]
  simp only [twoRayMomentMassSq, twoRayEnergy, h]
  norm_num
...
theorem twoRayMassSource_visible_to_unitTest :
  ...
  simp only [sourcePairing, twoRayMassSource, unitTest, twoRayMomentMassSq_eq_one,
    mul_one, Finset.sum_const, Finset.card_univ, Fintype.card_fin, one_nsmul]
...
theorem closed_visibleFan_can_be_bulkVisible :
    And (twoRayClosure = 0)
      (sourcePairing twoRayMassSource unitTest = 1) := by
  exact And.intro twoRayClosure_eq_zero twoRayMassSource_visible_to_unitTest

end PhysicsSM.Draft.NullEdgeP9VisibleClosureSource
