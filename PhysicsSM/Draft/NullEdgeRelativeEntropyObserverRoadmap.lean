...
/-!
# NullEdgeRelativeEntropyObserverRoadmap
...
* This file is a **draft scaffold**, not a trusted physics surface. It is now
  imported by `PhysicsSMDraft.lean` so downstream draft modules can share one
  finite observer-channel API. It imports only Mathlib and does not depend on
  the rest of `PhysicsSM`.
* As of this writing every declaration below is FULLY PROVED and checked with
  `lake env lean` (standard kernel assumptions: `propext`, `Classical.choice`,
  `Quot.sound` only; no `s o r r y`). The comments retained on the harder theorems record the proof
