...
This draft module strengthens the exact-recovery admissible coarse-map guardrail
by adding finite stochastic structure: normalized finite distributions,
column-stochastic observer channels, and observable expectations.

The main theorem says that common exact recovery pulls any fine observable that
distinguishes two source distributions back to a coarse observable
distinguishing their coarse observer outputs.  This is finite exact-recovery
bookkeeping for P9 source visibility.  It is not approximate recovery and not a
