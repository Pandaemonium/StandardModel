...
Scientific role: this is the norm/noise companion to the weighted
projector-residual orthogonality theorem.  A weighted self-adjoint idempotent
projector splits source energy into projected/coarse and residual components,
which gives P9 a finite observer-channel energy decomposition.
-/
...
namespace PhysicsSM.Draft.NullEdgeP9WeightedProjectorPythagorean
...
theorem weighted_residual_orthogonal_to_projected
  ...
  have key := weighted_projected_pairing_eq w Pi source test hself hidemp
  have expand : dotW w (subCochain source (lin Pi source)) (lin Pi test)
      = dotW w source (lin Pi test) - dotW w (lin Pi source) (lin Pi test) := by
    unfold dotW subCochain
    rw [<- Finset.sum_sub_distrib]
    apply Finset.sum_congr rfl
    intro i _
    ring
  rw [expand, key, sub_self]
...
theorem weighted_projector_pythagorean
  ...
  have hra : dotW w (subCochain source (lin Pi source)) (lin Pi source) = 0 :=
    weighted_residual_orthogonal_to_projected w Pi source source hself hidemp
  have har : dotW w (lin Pi source) (subCochain source (lin Pi source)) = 0 := by
    rw [dotW_comm]
    exact hra
  have expand : dotW w source source
      = dotW w (lin Pi source) (lin Pi source)
        + dotW w (lin Pi source) (subCochain source (lin Pi source))
        + dotW w (subCochain source (lin Pi source)) (lin Pi source)
        + dotW w (subCochain source (lin Pi source))
            (subCochain source (lin Pi source)) := by
    unfold dotW subCochain
...
end PhysicsSM.Draft.NullEdgeP9WeightedProjectorPythagorean
