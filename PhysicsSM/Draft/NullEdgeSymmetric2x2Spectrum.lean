...
namespace PhysicsSM.Draft.NullEdgeSymmetric2x2Spectrum
...
theorem discriminant_nonneg (a b c : Real) : 0 ≤ discriminant a b c := by
  exact add_nonneg ( sq_nonneg _ ) ( mul_nonneg zero_le_four ( sq_nonneg _ ) )
...
theorem lamPlus_is_eigenvalue (a b c : Real) :
  ...
  unfold lamPlus discriminant
  ring_nf
  rw [Real.sq_sqrt] <;> nlinarith [sq_nonneg (a - c)]

end PhysicsSM.Draft.NullEdgeSymmetric2x2Spectrum
