...
theorem exists_complex_witness (x y z : Octonion) :
  ...
  -- Split into cases based on whether `normSq y` or `normSq z` is zero.
  by_cases hy : normSq y = 0;
  · use ⟨Real.sqrt (normSq x), 0⟩, 0, ⟨Real.sqrt (normSq z), 0⟩;
    simp_all +decide [mul_comm];
    exact ⟨ Real.mul_self_sqrt ( by positivity ), Real.mul_self_sqrt ( by positivity ), by rw [ show y.c0 = 0 by nlinarith, show y.c1 = 0 by nlinarith, show y.c2 = 0 by nlinarith, show y.c3 = 0 by nlinarith, show y.c4 = 0 by nlinarith, show y.c5 = 0 by nlinarith, show y.c6 = 0 by nlinarith, show y.c7 = 0 by nlinarith ] ; ring ⟩;
  · by_cases hz : normSq z = 0;
    · use Complex.mk (Real.sqrt (normSq x)) 0, Complex.mk (Real.sqrt (normSq y)) 0, 0;
      simp_all +decide;
      exact ⟨ Real.mul_self_sqrt ( by positivity ), Real.mul_self_sqrt ( by positivity ), by norm_num [ show z.c0 = 0 by contrapose! hz; positivity, show z.c1 = 0 by contrapose! hz; positivity, show z.c2 = 0 by contrapose! hz; positivity, show z.c3 = 0 by contrapose! hz; positivity, show z.c4 = 0 by contrapose! hz; positivity, show z.c5 = 0 by contrapose! hz; positivity, show z.c6 = 0 by contrapose! hz; positivity, show z.c7 = 0 by contrapose! hz; positivity ] ⟩;
    · -- In the nondegenerate case, choose y', z' as real square roots and x'=a + sqrt(normSq x-a^2)*I, where a is the real triple product divided by the product of square roots.
      obtain ⟨a, ha⟩ : ∃ a : ℝ, (a ^ 2 ≤ normSq x) ∧ (a * Real.sqrt (normSq y) * Real.sqrt (normSq z) = (x * (y * z)).c0) := by
        refine' ⟨ ( x * ( y * z ) |> Octonion.c0 ) / ( Real.sqrt ( normSq y ) * Real.sqrt ( normSq z ) ), _, _ ⟩;
        · -- By the properties of the norm, we know that $(x * (y * z)).c0^2 \leq (normSq x) * (normSq y) * (normSq z)$.
          have h_norm : (x * (y * z)).c0^2 ≤ (normSq x) * (normSq y) * (normSq z) := by
            have h_norm : (x * (y * z)).c0^2 ≤ normSq (x * (y * z)) := by
              exact le_add_of_le_of_nonneg ( le_add_of_le_of_nonneg ( le_add_of_le_of_nonneg ( le_add_of_le_of_nonneg ( le_add_of_le_of_nonneg ( le_add_of_le_of_nonneg ( le_add_of_nonneg_right ( sq_nonneg _ ) ) ( sq_nonneg _ ) ) ( sq_nonneg _ ) ) ( sq_nonneg _ ) ) ( sq_nonneg _ ) ) ( sq_nonneg _ ) ) ( sq_nonneg _ );
            rw [ normSq_mul, normSq_mul ] at * ; linarith;
          rw [ div_pow, mul_pow, Real.sq_sqrt <| normSq_nonneg _, Real.sq_sqrt <| normSq_nonneg _ ];
          rw [ div_le_iff₀ ] <;> first | linarith | exact mul_pos ( lt_of_le_of_ne ( normSq_nonneg _ ) ( Ne.symm hy ) ) ( lt_of_le_of_ne ( normSq_nonneg _ ) ( Ne.symm hz ) ) ;
        · rw [ mul_assoc, div_mul_cancel₀ _ ( mul_ne_zero ( Real.sqrt_ne_zero'.mpr ( lt_of_le_of_ne ( normSq_nonneg _ ) ( Ne.symm hy ) ) ) ( Real.sqrt_ne_zero'.mpr ( lt_of_le_of_ne ( normSq_nonneg _ ) ( Ne.symm hz ) ) ) ) ];
      refine' ⟨ ⟨ a, Real.sqrt ( normSq x - a ^ 2 ) ⟩, ⟨ Real.sqrt ( normSq y ), 0 ⟩, ⟨ Real.sqrt ( normSq z ), 0 ⟩, _, _, _, _ ⟩ <;> norm_num [ ha ];
      · rw [ Real.mul_self_sqrt ] <;> linarith! [ normSq_def x ];
    ...
    ∃ r s t : ℝ,
      ...
      r * s * t
        = alpha * beta * gamma - alpha * Complex.normSq x'
          ...
          + 2 * (x' * (y' * z')).re := by
  -- Let $A$ be the Hermitian matrix with eigenvalues $r, s, t$.
  set A : Matrix (Fin 3) (Fin 3) ℂ := !![alpha, z', star y'; star z', beta, x'; y', star x', gamma];
  -- Since $A$ is Hermitian, its eigenvalues are real.
  have h_eigenvalues_real : ∃ r s t : ℝ, Matrix.charpoly A = (Polynomial.X - Polynomial.C (r : ℂ)) * (Polynomial.X - Polynomial.C (s : ℂ)) * (Polynomial.X - Polynomial.C (t : ℂ)) := by
    have h_eigenvalues_real : Matrix.IsHermitian A := by
      ext i j; fin_cases i <;> fin_cases j <;> aesop;
    have := h_eigenvalues_real.charpoly_eq;
    exact ⟨ h_eigenvalues_real.eigenvalues 0, h_eigenvalues_real.eigenvalues 1, h_eigenvalues_real.eigenvalues 2, by simpa [ Fin.prod_univ_three ] using this ⟩;
  obtain ⟨ r, s, t, h ⟩ := h_eigenvalues_real; use r, s, t; simp_all +decide [ Matrix.charpoly, Matrix.det_fin_three ] ;
  simp +zetaDelta at *;
  have h₁ := congr_arg ( Polynomial.eval 0 ) h; have h₂ := congr_arg ( Polynomial.eval 1 ) h; have h₃ := congr_arg ( Polynomial.eval ( -1 ) ) h; norm_num [ Complex.ext_iff ] at *;
  norm_num [ Complex.normSq ] at * ; exact ⟨ by linarith, by linarith, by linarith ⟩ ;
