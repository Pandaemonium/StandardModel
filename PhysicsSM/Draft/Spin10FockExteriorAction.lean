...
/-
The matrix coefficients of an exterior-power map are the ordered minors.
-/
lemma exteriorPowerMap_entry (U : Matrix (Fin 5) (Fin 5) ℂ) (n : ℕ)
    (S T : Set.powersetCard (Fin 5) n) :
    LinearMap.toMatrix ((Pi.basisFun ℂ (Fin 5)).exteriorPower n)
      ((Pi.basisFun ℂ (Fin 5)).exteriorPower n)
      (exteriorPower.map n (Matrix.toLin (Pi.basisFun ℂ (Fin 5))
        (Pi.basisFun ℂ (Fin 5)) U)) S T =
      compoundEntry U S.1 T.1 := by
  unfold compoundEntry;
  split_ifs with h;
  · simp +decide [ LinearMap.toMatrix_apply, exteriorPower.basis_repr_apply,
      exteriorPower.map_apply_ιMulti_family ];
    simp +decide [ exteriorPower.ιMultiDual_apply_ιMulti, exteriorPower.ιMulti_family ];
    simp +decide [ Matrix.toLin_apply, Pi.basisFun ];
    simp +decide [ Matrix.mulVec, Pi.single_apply ];
    simp +decide [ Set.powersetCard.ofFinEmbEquiv, dotProduct ];
    rw [ ← Matrix.det_transpose ];
    simp +decide [ Matrix.det_apply', Pi.single_apply ];
    convert rfl;
    all_goals exact S.2 |> fun h => by simp;
  · simp_all +decide [ Set.powersetCard ]

/-
The Cauchy--Binet identity for the ordered minors used by `compoundEntry`.
-/
lemma compoundEntry_mul (U V : Matrix (Fin 5) (Fin 5) ℂ)
    (S T : Finset (Fin 5)) :
    compoundEntry (U * V) S T =
      ∑ K, compoundEntry U S K * compoundEntry V K T := by
  by_cases h : S.card = T.card;
  · obtain ⟨n, hn⟩ : ∃ n : ℕ, S.card = n ∧ T.card = n := by
      aesop;
    have h_cauchy_binet : LinearMap.toMatrix ((Pi.basisFun ℂ (Fin 5)).exteriorPower n) ((Pi.basisFun ℂ (Fin 5)).exteriorPower n) (exteriorPower.map n (Matrix.toLin (Pi.basisFun ℂ (Fin 5)) (Pi.basisFun ℂ (Fin 5)) (U * V))) (⟨S, by aesop⟩) (⟨T, by aesop⟩) = ∑ K : Set.powersetCard (Fin 5) n, LinearMap.toMatrix ((Pi.basisFun ℂ (Fin 5)).exteriorPower n) ((Pi.basisFun ℂ (Fin 5)).exteriorPower n) (exteriorPower.map n (Matrix.toLin (Pi.basisFun ℂ (Fin 5)) (Pi.basisFun ℂ (Fin 5)) U)) (⟨S, by aesop⟩) (K) * LinearMap.toMatrix ((Pi.basisFun ℂ (Fin 5)).exteriorPower n) ((Pi.basisFun ℂ (Fin 5)).exteriorPower n) (exteriorPower.map n (Matrix.toLin (Pi.basisFun ℂ (Fin 5)) (Pi.basisFun ℂ (Fin 5)) V)) (K) (⟨T, by aesop⟩) := by
      simp +decide [ ← Matrix.mul_apply, ← LinearMap.toMatrix_comp ];
      rw [ ← exteriorPower.map_comp ];
      rw [ ← Matrix.toLin_mul ];
    convert h_cauchy_binet using 1;
    · rw [ exteriorPowerMap_entry ];
    · rw [ ← Finset.sum_subset ( Finset.subset_univ ( Finset.image ( fun K : Set.powersetCard ( Fin 5 ) n => K.1 ) ( Finset.univ : Finset ( Set.powersetCard ( Fin 5 ) n ) ) ) ) ];
      · rw [ Finset.sum_image ];
        · refine' Finset.sum_congr rfl fun x hx => _;
          rw [ exteriorPowerMap_entry, exteriorPowerMap_entry ];
        · exact fun x _ y _ hxy => Subtype.ext hxy;
      · simp +contextual [ compoundEntry ];
        bv_omega;
  · unfold compoundEntry; aesop;
...
theorem extAction_mul (U V : Matrix (Fin 5) (Fin 5) ℂ) (psi : FockSpinor) :
  ...
  unfold extAction
  ext S
  simp only [compoundEntry_mul, Finset.sum_mul, mul_assoc, Finset.mul_sum]
  rw [Finset.sum_comm]
...
theorem extAction_vacuum (U : Matrix (Fin 5) (Fin 5) ℂ) :
  ...
  unfold extAction vacuumSpinor ;
  unfold compoundEntry basisSpinor; aesop;
