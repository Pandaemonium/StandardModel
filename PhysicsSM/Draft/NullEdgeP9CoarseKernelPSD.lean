...
namespace PhysicsSM.Draft.NullEdgeP9CoarseKernelPSD
...
theorem response_coarseKernel_eq_response_pullback {m n : Nat}
  ...
  simp +decide only [response, coarseKernel, pullback]
  simp +decide only [mul_comm, mul_left_comm, Finset.mul_sum _ _ _]
  simp +decide only [<- Finset.sum_product']
  apply Finset.sum_bij (fun x _ => (x.2.2.1, x.2.2.2, x.1, x.2.1)) _ _ _ _ <;>
    simp +decide
...
theorem coarseKernel_psd {m n : Nat}
  ...
  exact fun y => by simpa only [response_coarseKernel_eq_response_pullback] using hK _
...
theorem psd_diag_nonneg {n : Nat} (K : Fin n -> Fin n -> Real)
    (hK : PSD K) (i : Fin n) :
    0 <= K i i := by
  specialize hK (standardBasis i)
  unfold response standardBasis at hK
  aesop
