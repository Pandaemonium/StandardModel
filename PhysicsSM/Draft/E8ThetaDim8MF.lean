...
Proof: Integral lattice ≤ dual lattice. Covolume 1 ⇒ trivial discriminant ⇒ equal.
The missing Lean API is a dimension-8 version of SPL's discriminant-group/covolume
argument, or an explicit proof that the dual basis of the public `E8Matrix` basis
lies in the same lattice.

### `thetaSeries8_transform_S`
SPL blueprint: `thetaSeries_transform_S` in `ThetaTransform.lean`.
Proof: Apply `SchwartzMap.poissonSummation_lattice (d := 8)` with a Gaussian
Schwartz map. Use `fourier_gaussian_innerProductSpace` for the Fourier transform.
The dimension factor is `(Module.finrank ℝ ℝ⁸) / 2 = 4`.
-/
...
private alias thetaProfile8 := Helpers.thetaProfile
private alias thetaProfile8_contDiff := Helpers.thetaProfile_contDiff
private alias thetaProfile8_decay := Helpers.thetaProfile_decay
...
@[simp] private lemma thetaKernel8_apply (τ : ℂ) (hτ : 0 < τ.im) (x : ℝ⁸) :
  ...
  simp [thetaProfile8, Helpers.thetaProfile, mul_assoc, mul_comm]
...
theorem thetaSeries8_transform_S
  ...
  have hτ1 : 0 < (-1 / τ).im := Helpers.neg_one_div_im_pos hτ
...
end PhysicsSM.Coding.E8ThetaDim8
