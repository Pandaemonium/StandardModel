...
namespace PhysicsSM.Draft.NullEdgeP9GravInvisibleAPI
...
theorem boundaryExact_gravInvisible_of_closed
  ...
  have hc : ∀ w, configResponse s t w = 0 :=
    fun w => boundaryExact_configResponse_zero_of_closed B s t hclosed hexact w
  refine ⟨?_, ?_⟩
  · simp only [MeanInvisible, meanResponse, hc, mul_zero, Finset.sum_const_zero]
  · simp only [NoiseInvisible, responseNoise, meanResponse, hc, mul_zero,
      Finset.sum_const_zero, sub_zero, ne_eq, OfNat.ofNat_ne_zero,
      not_false_eq_true, zero_pow]

end PhysicsSM.Draft.NullEdgeP9GravInvisibleAPI
