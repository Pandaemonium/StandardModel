...
namespace PhysicsSM.Draft.NullEdgeRankOneNullMomentum
...
def pX (a : Spinor) : Real := 2 * (a 0 * conj (a 1)).re

/-- y component `2 Im(psi0 conj psi1)`. -/
def pY (a : Spinor) : Real := 2 * (a 0 * conj (a 1)).im
...
theorem rankOne_momentum_is_null (a : Spinor) : minkowskiSq a = 0 := by
  ...
  norm_num [Complex.normSq, Complex.mul_re, Complex.mul_im]
  ring

end PhysicsSM.Draft.NullEdgeRankOneNullMomentum
