...
The harvested decomposition, reconstruction, orthogonality, and eigenvalue
proofs are completed here by a concrete coordinate proof of degree-four
power-associativity and the resulting idempotence proof.
-/
...
lemma jordan_power_four (X : H3O) :
  ...
  ext <;>
    simp [jordanProduct, octonionInner, Algebra.Octonion.conj,
      Algebra.Octonion.Octonion.mul_c0, Algebra.Octonion.Octonion.mul_c1,
      Algebra.Octonion.Octonion.mul_c2, Algebra.Octonion.Octonion.mul_c3,
      Algebra.Octonion.Octonion.mul_c4, Algebra.Octonion.Octonion.mul_c5,
      Algebra.Octonion.Octonion.mul_c6, Algebra.Octonion.Octonion.mul_c7] <;>
    ring
...
theorem lagrangeE_isProjection (X : H3O) (r s t : ℝ)
  ...
  ext si;
  all_goals unfold lagrangeE; norm_num [ jordan_smul_left, jordan_smul_right, jordan_add_left, jordan_add_right, jordan_sub_left, jordan_sub_right, h3o_characteristic_equation ] ;
  all_goals rw [ ← mul_assoc, ← mul_assoc ] ;
  all_goals rw [ jordan_power_four ] ;
  all_goals rw [ h3o_characteristic_equation ] ;
  all_goals simp +decide [ jordanProduct_oneH3O_left, jordanProduct_oneH3O_right, jordanProduct_comm ] ;
  all_goals simp +decide [ jordan_smul_left, jordan_smul_right, jordan_add_left, jordan_add_right, jordan_sub_left, jordan_sub_right, h.sum_eq, h.pair_eq, h.prod_eq ] ;
  all_goals rw [ h3o_characteristic_equation ] ;
  all_goals simp +decide [ jordanProduct_oneH3O_left, jordanProduct_oneH3O_right, jordanProduct_comm ] ;
  all_goals rw [ ← mul_inv ] ;
  all_goals rw [ ← h.sum_eq, ← h.pair_eq, ← h.prod_eq ] ; ring_nf ;
  grind; all_goals grind
