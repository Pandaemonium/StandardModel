...
6. `witness` — exact rational witness at `mu2 = 4/25`, `lam = 1/4`: vev^2 =
   `16/25` (vev = 4/5), stiffness `8/25`, depth `-16/625` (the depth clause
   in the original seed read `-4/625`, which is numerically wrong: the exact
   value is `-mu2^2/(4 lam) = -(4/25)^2/(4*(1/4)) = -16/625`; corrected
   here); the run's `4/25` scale reappearing as the distance to criticality.
...
Recovered from Aristotle project `6b7080bc-bff9-44a4-ba6e-6593577bec9c`; proof bodies verified locally
under the pinned toolchain before porting.
-/

namespace PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed
...
theorem hasDerivAt_V (mu2 lam σ : ℝ) :
  ...
  unfold V Vp
  convert (((hasDerivAt_pow 2 σ).const_mul (-mu2)).div_const 2).add
    (((hasDerivAt_pow 4 σ).const_mul lam).div_const 4) using 1
...
theorem ssb_of_supercritical (mu2 lam : ℝ) (hmu : 0 < mu2) (hlam : 0 < lam) :
  ...
  unfold Vp
  rw [show Real.sqrt (mu2 / lam) ^ 3 = Real.sqrt (mu2 / lam) * Real.sqrt (mu2 / lam) ^ 2 by ring,
...
theorem no_ssb_of_subcritical (mu2 lam σ : ℝ) (hmu : mu2 ≤ 0)
  ...
  rcases lt_or_gt_of_ne hσ with h | h <;>
    nlinarith [mul_pos hlam (sq_pos_of_ne_zero hσ), sq_nonneg σ]
...
theorem higgs_stiffness_exact (mu2 lam : ℝ) (hmu : 0 < mu2) (hlam : 0 < lam) :
  ...
  unfold Vp
  convert ((hasDerivAt_id (Real.sqrt (mu2 / lam))).const_mul (-mu2)).add
    ((hasDerivAt_pow 3 (Real.sqrt (mu2 / lam))).const_mul lam) using 1
  · ext x; simp only [Pi.add_apply, id]; ring
...
theorem vacuum_depth_exact (mu2 lam : ℝ) (hmu : 0 < mu2) (hlam : 0 < lam) :
  ...
  unfold V
  rw [show Real.sqrt (mu2 / lam) ^ 4 = (Real.sqrt (mu2 / lam) ^ 2) ^ 2 by ring,
      Real.sq_sqrt (by positivity)]
  field_simp
  ring
...
theorem joint_vanishing (mu2 lam : ℝ) (hmu : 0 < mu2) (hlam : 0 < lam) :
  ...
  rw [Real.sq_sqrt (by positivity : (0:ℝ) ≤ mu2 / lam)]
...
vev^2 = 16/25, stiffness 8/25, depth -16/625 (corrected from the
numerically wrong -4/625 in the seed: V(vev) = -mu2^2/(4 lam) =
-(4/25)^2/(4*(1/4)) = -(16/625)/1 = -16/625). -/
theorem witness :
    (Real.sqrt ((4 / 25 : ℝ) / (1 / 4))) ^ 2 = 16 / 25 ∧
    Vp (4 / 25) (1 / 4) (Real.sqrt ((4 / 25 : ℝ) / (1 / 4))) = 0 ∧
    (2 : ℝ) * (4 / 25) = 8 / 25 ∧
    V (4 / 25) (1 / 4) (Real.sqrt ((4 / 25 : ℝ) / (1 / 4))) = -(16 / 625) := by
  have hsq : (Real.sqrt ((4 / 25 : ℝ) / (1 / 4))) ^ 2 = 16 / 25 := by
    rw [Real.sq_sqrt (by norm_num)]; norm_num
  refine ⟨hsq, ?_, by norm_num, ?_⟩
  · unfold Vp
    rw [show Real.sqrt ((4 / 25 : ℝ) / (1 / 4)) ^ 3
          = Real.sqrt ((4 / 25 : ℝ) / (1 / 4)) * Real.sqrt ((4 / 25 : ℝ) / (1 / 4)) ^ 2 by ring,
        hsq]
    ring
  · unfold V
    rw [show Real.sqrt ((4 / 25 : ℝ) / (1 / 4)) ^ 4
          = (Real.sqrt ((4 / 25 : ℝ) / (1 / 4)) ^ 2) ^ 2 by ring, hsq]
    norm_num

end PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed

/-! ## Build-enforced assumption-footprint guards -/

/-- info: 'PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.higgs_stiffness_exact' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.higgs_stiffness_exact

/-- info: 'PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.vacuum_depth_exact' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.vacuum_depth_exact

/-- info: 'PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.joint_vanishing' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.joint_vanishing

/-- info: 'PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.witness' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms PhysicsSM.Draft.NullEdge.HiggsCriticalitySeed.witness
