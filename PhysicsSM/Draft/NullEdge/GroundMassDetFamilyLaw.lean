...
trace-side `budget = c * detP` (see `BudgetSignMismatch`, a REFERENCE only). It is the
positive completion of that §4 kill-test's audit→theorem arc: `BudgetSignMismatch`
showed the trace-side sum-of-squares budget is NOT a family identity; this is the
correct determinant-side family law it pointed to.
...
Scope / provenance: the spectrum `{lam-kap, lam+kap}` and `det = lam^2 - kap^2` of this
aperture-closure block are ALSO in `MassPhaseDiagram` (`B3`) — this module does NOT
re-derive them as novel; the NEW content is the family-law synthesis (`det = mu-(tr-mu-)`),
the exact-defect identity (`Delta = -kap`), and the spectral-vs-invariants contrast that
convert the central tie from a witness-level coincidence into a law. Draft status:
