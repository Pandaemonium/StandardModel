...
* `coboundary_corank`: the linearized/tangent corank equals
  `(N - 1) * (N - 2) / 2`, the physical CP-phase count.
...
`#edges(K_N) - rank = (N - 1) * (N - 2) / 2`. It is the
linearized/tangent-space corank of the additive rephasing action, not a global
multiplicative unitary normal-form theorem. -/
