...
This module isolates the calculus bridge used by quantitative discrete-time
adiabatic theorems. A bounded first derivative gives an `O(h)` one-step
difference, while a bounded second derivative gives an `O(h^2)` second
difference. The final statement specializes to `h = 1 / T`.

This file does not assert a spectral gap, identify an HNU band, or prove an
adiabatic theorem.

Provenance: theorem shape extracted clean-room from the finite-difference
hypotheses in Costa et al., *PRX Quantum* 3, 040303 (2022),
arXiv:2111.08152, and reduced to Mathlib's vector-valued mean-value API.
Proofs returned by Aristotle project `2624a3d5-4d7f-472c-9e36-82bc3896f5f8`,
task `9de07986-91e3-4c34-8ea3-e8b11ba901dd`, then reviewed and rebuilt under
...
namespace PhysicsSM.Draft.NullEdge.DiscreteAdiabaticFiniteDifferences
...
end PhysicsSM.Draft.NullEdge.DiscreteAdiabaticFiniteDifferences
