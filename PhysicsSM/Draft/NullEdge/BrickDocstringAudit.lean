...
/-!
# Adversarial docstring audit of the MC bricks (Opus, verified 6d88b22a)

A self-check of MY OWN brick PROSE against what the stated theorems support. All
FIVE audited claims came back PROSE-OUTRUNS-STATEMENT, each with a kernel witness:

1. 'unitary conjugation is an isometry' - holds for a FIXED unitary; it does NOT
   license comparing two steps conjugated by DIFFERENT unitaries. Witness:
   A = diag(1,-1), U0 = I, U1 = swap, both orthogonal - original squared distance 0,
   conjugated squared distance 8.
2. 'the four-component constant does not accumulate' - valid for BLOCK-DIAGONAL
   assembly only. Witness: [[I,I],[I,I]] has every block of constant 1 yet norm
   ratio 2, so OFF-DIAGONAL blocks do accumulate.
3. 'the tail bound is mass-independent' - the CONSTANT is, but the TAIL SET must be
   fixed independently of the mass. Witness: identity-unitary field with tail set
   empty at one mass and a point at another gives tail energies 0 and 1.
4. 'a walk supplies only its one-step constant' - false; a one-step constant alone
   does not determine later steps (W 1 = id but W 2 x = x+1). A propagation
   hypothesis is required: the group law plus a one-step bound UNIFORM in the
   starting point (under which unitarity is not additionally needed), or unitarity
   to transport a bound available only at a distinguished state.
5. 'componentwise reuse is free' - free in the CONSTANT, NOT in MEASURABILITY;
   measurability must be proved per component or from a common construction.

The affected brick docstrings have been corrected accordingly. Namespace kept as
the prover's. Provenance: verified at pin from task d648ff0c. Standard three.
Grade M, [orig] (independent-review artifact). -/
