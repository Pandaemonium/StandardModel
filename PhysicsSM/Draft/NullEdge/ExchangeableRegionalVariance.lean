...
/-!
# Exchangeable covariances: the regional variance law, and the sharp cost of invariance

Companion to `HyperuniformityRankDichotomy`, working the maximally symmetric case
concretely rather than through pair-orbit rank. Here the symmetry is FULL PERMUTATION
INVARIANCE, and everything is computed in closed form.

## What is proved

* `permutationInvariant_iff_exchangeable` - for `N >= 2`, full permutation invariance is
  EQUIVALENT to the exchangeable normal form: constant diagonal `a`, constant off-diagonal
  `b`.
* `numberVariance_exchangeable` - the exact regional law
  `V(C, A) = |A| a + |A| (|A| - 1) b`.
* `numberVariance_fixedTotal` - imposing the fixed-total (unimodular) constraint forces
  `b = -a / (N - 1)`, hence `V(C, A) = a |A| (N - |A|) / (N - 1)`.
* `exchangeable_psd_signs` - PSD forces `a >= 0` and `a + (N-1) b >= 0`.
* `numberVariance_linear_lower_bound` - for `2 |A| <= N`, the clean bound
  `a |A| / 2 <= V(C, A)`: **regional variance grows at least linearly in region size.**
* `posSemidef_eq_zero_of_diagonal_eq_zero` and `exchangeable_eq_zero_of_a_eq_zero` - a PSD
  matrix with vanishing diagonal is zero, so `a = 0` collapses the covariance entirely.
* `frameBlind_dichotomy` - the packaged statement: **either `C = 0`, or every nonempty
  region of size at most `N/2` has strictly positive variance obeying the linear lower
  bound.** There is no middle option, and in particular no regionally quiet nonzero option.
* `numberVariance_image_eq` - variance is constant along the permutation orbit of a region.
* An explicit `N = 4` NON-invariant rank-one PSD witness built from `v = (1,-1,1,-1)`, with
  fixed total and `C 0 0 > 0`, whose variance on `{0, 1}` is **zero** - packaged with the
  explicit failure of permutation invariance.

## The content, stated correctly

The witness is the point. Selective regional suppression is available **exactly when
invariance is surrendered**: an exchangeable covariance cannot make any region of size at
most `N/2` quiet without collapsing to zero, while a non-invariant one silences a region
outright.

## Scope - `AgentTasks/lambda-harvest-governance-2026-07-21.md` is binding here

Say: *for a fully permutation-invariant (equivalently exchangeable) covariance with fixed
total, the regional variance is forced to be exactly linear in region size.* Do **NOT**
drop the invariance qualifier, and do **NOT** call `S_N`-invariance "frame-blindness",
"isotropy", or "homogeneity" - it is none of those. `S_N` invariance is the much stronger
statement that the symmetry retains no notion of separation at all; physical isotropy
leaves one pair-orbit per distance and comfortably admits hyperuniform processes
(Torquato, arXiv:1801.06924, defines disordered hyperuniform systems as statistically
isotropic).

In particular this module does **not** show that invariance forbids hyperuniformity. The
companion `HyperuniformityRankDichotomy` proves the opposite: `I - J/N` is invariant, PSD
and annihilates the uniform mode at every `N >= 2`. Consistency is exact - that witness has
`a = 1 - 1/N > 0` and therefore lands in this module's "strictly positive variance" branch,
obeying the linear law. What maximal symmetry forbids is regional QUIET, not
hyperuniformity.

Nothing here concerns point processes, sprinklings, or Lorentz invariance, and nothing here
derives the value, sign, or magnitude of `Lambda`.

Provenance: Aristotle project `f3020ad0-133c-4737-b2c7-d245f2ff7f46`, task
`332b1262-edbe-4a06-9b75-90834702f127`, verified verbatim at the pinned toolchain. The
prover's namespace `FrameBlindVariance` is kept verbatim. Claim grade `M`; `[comp]` for the
exchangeable normal form and the finite-population variance law, which are classical,
`[orig]` for the dichotomy packaging and the paired non-invariant witness.
-/
...
/-- info: 'FrameBlindVariance.numberVariance_exchangeable' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms numberVariance_exchangeable
/-- info: 'FrameBlindVariance.totalVariance_exchangeable' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms totalVariance_exchangeable
/-- info: 'FrameBlindVariance.offDiagonal_eq_of_fixedTotal' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms offDiagonal_eq_of_fixedTotal
/-- info: 'FrameBlindVariance.numberVariance_fixedTotal' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms numberVariance_fixedTotal
/-- info: 'FrameBlindVariance.diagonal_nonnegative' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms diagonal_nonnegative
/-- info: 'FrameBlindVariance.totalVariance_nonnegative' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms totalVariance_nonnegative
/-- info: 'FrameBlindVariance.exchangeable_psd_signs' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms exchangeable_psd_signs
/-- info: 'FrameBlindVariance.posSemidef_eq_zero_of_diagonal_eq_zero' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms posSemidef_eq_zero_of_diagonal_eq_zero
/-- info: 'FrameBlindVariance.exchangeable_eq_zero_of_a_eq_zero' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms exchangeable_eq_zero_of_a_eq_zero
/-- info: 'FrameBlindVariance.numberVariance_linear_lower_bound' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms numberVariance_linear_lower_bound
/-- info: 'FrameBlindVariance.frameBlind_dichotomy' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms frameBlind_dichotomy
/-- info: 'FrameBlindVariance.exists_perm_map_pair' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms exists_perm_map_pair
/-- info: 'FrameBlindVariance.permutationInvariant_iff_exchangeable' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms permutationInvariant_iff_exchangeable
/-- info: 'FrameBlindVariance.numberVariance_permutationInvariant' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms numberVariance_permutationInvariant
/-- info: 'FrameBlindVariance.numberVariance_permutationInvariant_fixedTotal' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms numberVariance_permutationInvariant_fixedTotal
/-- info: 'FrameBlindVariance.permutationInvariant_eq_zero_of_diagonal_zero' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms permutationInvariant_eq_zero_of_diagonal_zero
/-- info: 'FrameBlindVariance.permutationInvariant_linear_lower_bound' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms permutationInvariant_linear_lower_bound
/-- info: 'FrameBlindVariance.permutationInvariant_dichotomy' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms permutationInvariant_dichotomy
/-- info: 'FrameBlindVariance.sharpMatrix_psd' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms sharpMatrix_psd
/-- info: 'FrameBlindVariance.sharpMatrix_fixedTotal' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms sharpMatrix_fixedTotal
/-- info: 'FrameBlindVariance.sharpMatrix_positive_diagonal' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms sharpMatrix_positive_diagonal
/-- info: 'FrameBlindVariance.sharpMatrix_region_suppressed' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms sharpMatrix_region_suppressed
/-- info: 'FrameBlindVariance.sharpMatrix_not_invariant' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms sharpMatrix_not_invariant
/-- info: 'FrameBlindVariance.exists_noninvariant_suppressed_region' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms exists_noninvariant_suppressed_region
/-- info: 'FrameBlindVariance.numberVariance_image_eq' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
