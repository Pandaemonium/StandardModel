/-
Provenance: Aristotle job b94c21da (fable-24h-momentum), harvested
2026-07-11 ~22:50 PDT after a stop-and-return instruct. All 18 public
statements integrated UNCHANGED (statement-level diff). File renames
GRCore/GRDiag/GRFacts -> PairMomentum* + import rewires are the only
edits. TRUST: kernel + native_decide (TWO real tokens, both on the
Gaussian-rational twin layer - the diagonalizer identity in GRCore and
the consolidated matrix-identity conjunction in GRDiag); every public
theorem transports from those via the toC bridge. Disclosed per the
submission license (native fallback permitted).
Oracle: momentum_oracle.py (gated on reproducing the boundstate
factorization before anything was seeded).
-/
import Mathlib
