/-
Provenance: Aristotle job e33d3f5f (fable-24h-flowcount), harvested
2026-07-11 ~21:20 PDT. The two placeholder targets were replaced by the
prover with the genuine R2 statements per the submission contract
(verified: T1 = flowDiff cs = 0 from the jump law + periodicity; T2 =
cs.length /= 1 under +-1 signs). KERNEL-ONLY.
INTEGRATION ADDITION (Fable): the summary claimed a non-vacuity check
that was NOT landed as a declaration; the named witness below
(flowOne_hypotheses_nonvacuous) was added at integration - the flow-one
pseudo-doubler crossing list discharges both hypotheses. Statements of
the harvested theorems are untouched.
HONEST SCOPE (self-applied audit style): T1/T2 are finite telescoping
identities whose ANALYTIC content (local constancy, the jump law for
actual eigenphase counts, periodicity) is packaged as hypotheses - that
is the R2 design, chosen so the kernel fixture layer can discharge the
hypotheses per walk; the intrinsic R1 elaboration (deriving them from
trace/det/arg eigenphase geometry) is open and documented in-file.
-/
import Mathlib
...
/-- Non-vacuity of the hypothesis package (integration addition): the
flow-one pseudo-doubler configuration - one 0-crossing and one
pi-crossing, both sign +1 - satisfies the jump law and periodicity with
the explicit count function `0, 1, 0`. -/
theorem flowOne_hypotheses_nonvacuous :
    let cs : List Crossing :=
      [⟨Real.pi, true, 1⟩, ⟨Real.pi, false, 1⟩]
    let n : ℕ → ℤ := fun i => if i = 1 then 1 else 0
    (∀ i : Fin cs.length, n (i + 1) - n i = jumpOf cs[i])
      ∧ n cs.length = n 0 ∧ flowDiff cs = 0 := by
