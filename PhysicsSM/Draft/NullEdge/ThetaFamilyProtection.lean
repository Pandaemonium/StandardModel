/-
Provenance: Aristotle job beebad16 (fable-24h-thetafamily), harvested
2026-07-11 ~11:45 PDT; memo at
`AgentTasks/aristotle-results/thetafamily-beebad16/.../THETA_FAMILY_PROTECTION_MEMO.md`.
Statements integrated UNCHANGED except this header. KERNEL-ONLY: standard
three axioms; no native_decide (the single decide in two_wall_chart is
kernel decide). Pre-verified by symbolic sympy oracle (all identities
reduce to sin^2+cos^2 = 1; negative controls fail by exactly -2 sin theta).
Headline: modes_persist - for EVERY real theta and EVERY two-wall sign
field, the complete walk W(b,theta) over C carries exact +1 and -1
eigenvectors, by chart dispatch through the landed InvolutiveCompression
engine. Stability of the certified modes across the entire coin/mass
family as an exact identity family - no gap or continuity hypotheses.
Massless boundary sin theta = 0 honestly scoped (T6).
-/
import Mathlib
