...
## Relation to the already-landed `KraftCompressionMass` (READ THIS — not a novel claim)

The general fact "linear entropy `Hlin p = 1 - Σ pᵢ²` is the (rational) `mass²` invariant,
`= 0` iff the register is a single pure direction (massless)" is ALREADY landed in
`KraftCompressionMass` (`Hlin`, `mass_is_compressibility`). This module does NOT re-claim that
general framing. Its added content is the SPECIFIC instantiation for the `±c` velocity register:
the 2-outcome mixture `pplus,pminus = (E±p)/2E` whose mean is exactly the physical drift
`p/E` (`mean_is_drift`, tying it to `DiracVelocityOperator`'s `±c` spectrum and
`ZitterbewegungAverage`'s `vbar² = 1 - m²/E²`), with linear entropy `S_L = m²/(2E²)`. So this
is the velocity-register instance of `KraftCompressionMass`'s `Hlin = mass²`, not a new result.
...
`E ≠ 0`; no `Real.log`/`sqrt`/`cos`/`sin`, no `Complex`, no `nlinarith`. Draft status:
kernel-checked, every headline axiom-footprint-pinned to `[propext, Classical.choice, Quot.sound]`.
Supporting in-tree instance; not surfaced as a separate manuscript anchor row (the general
result is `KraftCompressionMass`'s row).
-/
