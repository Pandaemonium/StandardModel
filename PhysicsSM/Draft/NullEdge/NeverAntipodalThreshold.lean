...
/-!
# The never-antipodal threshold: homotopy invariants survive bounded, not vanishing, perturbation

Elementary homotopy theory, developed for the `3+1` sector-gate problem. A separate no-go
shows that the natural leakage bound for a moving band converges to the total band rotation
rather than to zero, so a gate of the form "leakage tends to `0`" is unreachable. This
module supplies the replacement: **a homotopy invariant does not need a vanishing
perturbation, only one below an explicit threshold.**

## What is proved

* `straightLine_ne_zero_iff` - the straight-line interpolation between two unit vectors is
  nonzero throughout `[0,1]` **exactly when** the endpoints are not antipodal.
* `normalizedStraightLineHomotopy`, `continuous_straightLine` - the normalized
  straight-line (geodesic) homotopy, with continuity and the endpoint equations proved
  rather than assumed.
* `homotopic_of_ne_neg`, `ne_neg_of_dist_lt_two`, `homotopic_of_forall_dist_lt_two` - **the
  threshold lemma.** Continuous sphere-valued maps at pointwise distance `< 2` are
  homotopic. The constant `2` is explicit and dimension-free.
* `eventually_homotopic_of_uniformly_tendsto` - the uniform-convergence form: eventually
  every map in a uniformly convergent sequence is homotopic to the limit.
* `invariant_eq_of_uniform_dist_lt_two` - the application-shaped corollary: **every
  homotopy-invariant quantity is unchanged** under a uniform perturbation of size `< 2`.
* `twoSphere_sharp_pair_distance`, `twoSphere_antipodal_distance_exactly_two` - identity and
  antipodal maps on `S^2` at distance exactly `2`, with the classical degree obstruction
  (`1` versus `-1`) documenting that the threshold cannot simply be raised.

## MY PROMPT WAS WRONG ABOUT THE SHARPNESS WITNESS, and the correction matters here

I suggested exhibiting the sharpness pair on `S^1` or `S^3`. The prover corrected this:
**in ODD sphere dimensions the antipodal map has degree `+1`**, so on `S^1` and `S^3` it is
homotopic to the identity and establishes nothing. The witness was moved to `S^2`, where the
antipodal degree is `-1`.

This is not a bookkeeping detail for the intended application. The endpoint map of interest
takes values in `SU(2) = S^3`, which is **odd**-dimensional. So:

* the threshold lemma still applies and is still SAFE - distance `< 2` implies homotopic,
  in every dimension;
* but the constant `2` is **not demonstrated sharp for an `S^3` target**, because the
  obstruction used to show sharpness on `S^2` is absent on `S^3`.

Read that as: the gate this module licenses is conservative. Do not claim `2` is optimal for
`SU(2)`-valued endpoint maps on the strength of the `S^2` witness.

## Scope

This is elementary homotopy theory about continuous maps into a sphere. It says **nothing**
about whether any particular physical perturbation is below the threshold. In the intended
application the leakage bound is in an OPERATOR norm while this threshold is a SUP norm on
the endpoint map, and that conversion is not automatic - it is the explicit kill condition
recorded in `AutonomousLab/work/NE-3PLUS1/OPUS_IMPACT_STRATEGIST_2026-07-21.md`.

Provenance: Aristotle project `85e4b8a3-91bd-4b4c-9fe3-86a2cd72fca5`, task
`4a00c820-da85-4f78-8f58-cffc6eaa47f9`, verified verbatim at the pinned toolchain. The
prover's namespace `NeverAntipodal` is kept verbatim. Claim grade `M`; `[comp]` for the
threshold lemma, which is classical, `[orig]` for its use as a replacement sector gate.
-/
...
/-- info: 'NeverAntipodal.straightLine_ne_zero_iff' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms straightLine_ne_zero_iff
/-- info: 'NeverAntipodal.normalizedStraightLineHomotopy' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms normalizedStraightLineHomotopy
/-- info: 'NeverAntipodal.homotopic_of_ne_neg' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms homotopic_of_ne_neg
/-- info: 'NeverAntipodal.homotopic_of_forall_dist_lt_two' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms homotopic_of_forall_dist_lt_two
/-- info: 'NeverAntipodal.twoSphere_sharp_pair_distance' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms twoSphere_sharp_pair_distance
/-- info: 'NeverAntipodal.eventually_homotopic_of_uniformly_tendsto' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms eventually_homotopic_of_uniformly_tendsto
/-- info: 'NeverAntipodal.invariant_eq_of_uniform_dist_lt_two' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
