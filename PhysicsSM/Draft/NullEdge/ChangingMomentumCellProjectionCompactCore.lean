...
/-!
# Compact-core convergence for changing momentum-cell projections

This module closes the compact-Lipschitz half of the Gate D changing-cell
projection limit. The proof localizes to cells on which the fixed compactly
supported field is nonzero. This is essential: the full scheduled box has
unbounded volume, whereas the active-cell union is eventually bounded.

The result is representative-safe because `projectAt` uses normalized cell
averages rather than point samples. It does not yet extend the result to every
`L2` field, identify projected coefficients with the live walk, apply inverse
Fourier transport, or prove a position-space Dirac PDE limit.

Provenance: theorem architecture prepared for Aristotle project
`b7405f03-7bf4-47c2-9b3a-71ce9040df3f`. After that project remained stalled,
Codex factored and kernel-checked the proof through the explicit helper lemmas
below during the AFPL continuum lane on July 12, 2026. No target statement was
weakened.
-/
...
#print axioms compact_lipschitz_projectAt_tendsto_sq_error_zero
