/-
Provenance: Aristotle job 92e5c459 (fable-24h-faithful), harvested
2026-07-12 ~05:36 PDT. KERNEL-ONLY (0 native) - re-proves the fixture's
'faithful' 28x28 GaussianInt identity Bz*Kz = 5*Vz via per-entry decide
(784 entry goals, maxHeartbeats 4000000; a whole-matrix decide times
out). HEAVY elaboration - kept on the on-demand path (this file /
PairSpectrumFixtureGuard), NOT the always-on aggregate guard. Shows the
fixture's faithful identity is kernel-provable, not only native.
-/
import Mathlib
