...
Mathlib-only draft module for the finite algebra at the start of Gate I1.
...
repo draft port of the standalone staging file under
`AgentTasks/aristotle-standalone/gate-i1-kinematic-core-20260702/`.  No project
modules are imported here, so the file remains easy to isolate for focused
Aristotle submission if later lemmas need remote proof search.
