...
theorem lambda2Act_antisymm (A W : M5) (hW : Wᵀ = -W) :
  ...
  unfold lambda2Act; simp +decide [ hW, Matrix.transpose_add, Matrix.transpose_mul ] ;
...
theorem dualAct_bracket (A B : M5) (v : Fin 5 → ℂ) :
  ...
  simp [dualAct]
  simp_all +decide [Matrix.mulVec_neg, sub_eq_add_neg]
...
theorem lambda2Act_bracket (A B W : M5) :
  ...
  unfold lambda2Act; simp +decide [ Matrix.mul_assoc, Matrix.add_mul, Matrix.mul_add ] ;
  grind
...
theorem dualAct_YGen_eigen (i : Fin 5) :
  ...
  fin_cases i <;> ext j <;> fin_cases j <;> simp +decide [ dualAct, YGen ];
  · simp +decide [ Y5bar ];
  · unfold Y5bar; norm_num [ Y5 ] ;
  · unfold Y5bar; norm_num [ Y5 ] ;
  · unfold Y5bar; norm_num [ Y5 ];
  · unfold Y5bar; norm_num [ Y5 ] ;
...
theorem lambda2Act_YGen_eigen (i j : Fin 5) :
  ...
  unfold lambda2Act YGen wedgeBasis Y10;
  ext k l ; by_cases hk : k = i <;> by_cases hl : l = j <;> simp_all +decide [ Matrix.mul_apply ] ; ring;
  · fin_cases i <;> fin_cases j <;> simp +decide [Matrix.diagonal]
  · simp +decide [ Matrix.single, Matrix.diagonal ];
    grind;
  · simp +decide [diagonal, single]
    ring
    grind;
  · simp +decide [Matrix.single, Matrix.diagonal]
    grind
...
theorem rep_hypercharge_trace_zero :
  ...
  simp [Y5bar, Y10, Y5, Fin.sum_univ_five]
  norm_num
...
theorem blockDiagonal_comm_YGen (A : M5)
    ...
    A * YGen = YGen * A := by
  ext i j;
  by_cases hij : i < 3 ∧ 3 ≤ j ∨ 3 ≤ i ∧ j < 3 <;> simp_all +decide [ Matrix.mul_apply ];
  · unfold YGen; simp +decide [ *, Matrix.diagonal ] ;
    rw [ hblock i j hij, MulZeroClass.zero_mul, MulZeroClass.mul_zero ];
  · simp_all +decide [ Fin.sum_univ_five, YGen ];
    fin_cases i <;> fin_cases j <;> simp +decide [ Y5 ] at hij ⊢ <;> ring!
