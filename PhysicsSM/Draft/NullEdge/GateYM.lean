...
and two-step trace identities and their decomposition/ratio facts, center
flip/projector algebra, flux/center toggle laws, and instantiates the positive
...
`-gap = log localSpectralRatio`, direct contraction-factor interval and
eigenvalue-recovery facts for `exp (-gap)`, plus the corresponding
nonzero/non-one bookkeeping facts for the ratios, eigenvalue slots,
contraction factor, and local gap; no transfer
...
positive/zero-dimensional selected-or-other corollaries; the selected
endomorphism range is identified with the sector inclusion range, giving the
...
`ReflectedBoundaryCoupling` slot with plus/minus block PSD wrappers and their
summed temporal block in direction `timeDir`. The concrete Wilson
boundary-coupling formula, reflection-hermiticity hypothesis, Berezin wrap, and
physical fermionic RP theorem remain open);
`BanksCasherShadow`
