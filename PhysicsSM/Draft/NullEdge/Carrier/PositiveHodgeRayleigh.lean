...
/-!
# Affine Hodge-cost lemmas and the boundary of a variational reading

**Audit status.** The original variational interpretation of this module is
retired. The concrete `witness` below uses a projection `Q` with `Q^2 != 0`, so
it is not a cohomology-class witness; its bare pairing varies because the norm
varies, while the normalized quotient stays constant. See
`Carrier.PositiveHodgeClassCostNoGo` and
`Carrier.PositiveHodgePhysicalMass` for the corrected cohomological theorem,
conditional positivity, and a nondegenerate nilpotent quartet.

The reusable finite content here consists of conditional affine cost identities:
...
3. `variational_mass_isLeast` — for the displayed unnormalized affine pairing,
   with ghost-positivity
   (`0 ≤ B (Q χ) (S (Q χ))` for all `χ`), the cost set over the class has
   least element `μ2`, attained at the harmonic representative. This is not a
   normalized Rayleigh quotient and is not, by itself, a physical mass theorem.
...
5. `witness` — an explicit rational affine instance with a nontrivial direction
   and `μ2 = 4/25`; its `Q` is non-nilpotent and its varying norm prevents a
   cohomological variational interpretation.
