...
Recovered from Aristotle project `680e2f13-1900-427e-9e5f-536c3068c994`; proof bodies verified locally
under the pinned toolchain before porting.
-/

namespace PhysicsSM.Draft.NullEdge.SpinStatisticsInterface
...
theorem singlet_exchange_neg :
  ...
  constructor
  · funext i j; fin_cases i <;> fin_cases j <;> simp [swap, singlet, basis]
  · simp [singlet, basis]
...
theorem symmetric_exchange_pos :
  ...
  constructor
  · funext i j; fin_cases i <;> fin_cases j <;> simp [swap, symm01, basis]
  · funext i j; fin_cases i <;> fin_cases j <;> simp [swap, basis]
...
theorem two_pi_pair_pos (x y : Fin 2 → ℂ) :
  ...
  funext i j; simp only [pairOf]; ring
...
theorem interface_match (x y : Fin 2 → ℂ) :
  ...
  refine ⟨?_, ?_, ?_⟩
  · funext i j; simp only [swap, wedgePair]; ring
  · funext i j; simp only [swap, pairOf]; ring
  · simp [wedgePair]
...
theorem pauli_exclusion (x : Fin 2 → ℂ) :
  ...
  funext i j; simp [wedgePair]

end PhysicsSM.Draft.NullEdge.SpinStatisticsInterface
