...
/-!
# The sigma-map `P(p) = p.σ` IS a sum of two null-edge dyads (closing "which P")
...
This answers an adversarial audit point: "`mass = det P` is frame-independent
ONLY for the little-group spinor `P`." The sigma-map `P(p)` (PhysLean-grounded)
and the manuscript's null-edge Gram are the SAME `P`, decomposable into null
edges — not two different constructions.

Provenance: PhysLean's Pauli / `spaceTime.gamma` convention (`P(p) = p.σ`,
`det P = m²`) is used as a REFERENCE / convention only, NOT imported. Honest
scope: the real `(t,z)`-restricted rational avatar; the general complex case is
identical with Hermitian dyads. The det-`P`-is-mass reading is `[import]`
little-group theory; the null-edge decomposition of this concrete `P` is the
kernel-checked content here.

Draft status: kernel-checked with no proof escape hatches; every headline
carries an in-file `#print axioms` guard pinned to exactly
