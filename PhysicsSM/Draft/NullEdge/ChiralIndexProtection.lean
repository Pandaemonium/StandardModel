...
theorem corner_ker_ge_index {np nm : ℕ} (A : Matrix (Fin nm) (Fin np) ℂ) :
  ...
  have hdom : Module.finrank ℂ (Fin np → ℂ) = np := by simp
  have hcod : Module.finrank ℂ (LinearMap.range (Matrix.mulVecLin A)) ≤ nm := by
    calc Module.finrank ℂ (LinearMap.range (Matrix.mulVecLin A))
        ≤ Module.finrank ℂ (Fin nm → ℂ) := Submodule.finrank_le _
      _ = nm := by simp
  rw [hdom] at hrn
  omega
...
theorem witness_one_protected_mode (A : Matrix (Fin 1) (Fin 2) ℂ) :
  ...
  have := corner_ker_ge_index A
  simpa using this

#print axioms corner_ker_ge_index
#print axioms corner_ker_ge_index_perturbed
#print axioms witness_one_protected_mode
