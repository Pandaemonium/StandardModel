...
gauge-overlap interface, the index-equals-signature controlling fact, the
Hermitian Sylvester inertia bridge, the free / density benchmarks, the explicit
witnesses, and the concrete nonzero-flux example).  It lets the whole C2 layer be
kernel-checked in one command:
...
* inertia under congruence: `HermitianSylvester`;
