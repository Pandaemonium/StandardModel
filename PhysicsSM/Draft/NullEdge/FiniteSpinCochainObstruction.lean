...
/-! ## The choice-independent quotient class -/

/-- Cokernel-style obstruction space for the supplied edge-to-face coboundary
map. No chain-complex or topological interpretation is built into this type. -/
abbrev ObstructionSpace
    (boundary : FaceBoundary Face Edge) :=
  FaceDefect Face ⧸ LinearMap.range boundary.mulVecLin

/-- Quotient class of one supplied face defect modulo edge coboundaries. -/
def obstructionClass
    (boundary : FaceBoundary Face Edge) (defect : FaceDefect Face) :
    ObstructionSpace boundary :=
  Submodule.Quotient.mk defect

omit [Fintype Face] in
/-- The quotient obstruction vanishes exactly when the defect is correctable. -/
theorem obstructionClass_eq_zero_iff_correctable
    (boundary : FaceBoundary Face Edge) (defect : FaceDefect Face) :
    obstructionClass boundary defect = 0 ↔ Correctable boundary defect := by
  rw [obstructionClass, Submodule.Quotient.mk_eq_zero]
  simp [Correctable, faceCoboundary]

omit [Fintype Face] in
/-- Adding an edge coboundary does not change the quotient obstruction class. -/
theorem obstructionClass_add_coboundary
    (boundary : FaceBoundary Face Edge) (s : EdgeCorrection Edge)
    (defect : FaceDefect Face) :
    obstructionClass boundary (faceCoboundary boundary s + defect) =
      obstructionClass boundary defect := by
  rw [obstructionClass, Submodule.Quotient.mk_add]
  have hmem :
      faceCoboundary boundary s ∈ LinearMap.range boundary.mulVecLin := by
    exact ⟨s, by simp [faceCoboundary]⟩
  have hzero :
      (Submodule.Quotient.mk (faceCoboundary boundary s) :
        ObstructionSpace boundary) = 0 :=
    (Submodule.Quotient.mk_eq_zero
      (LinearMap.range boundary.mulVecLin)).2 hmem
  rw [hzero, zero_add]
  rfl

/-- The quotient class vanishes exactly when no closed-cycle pairing detects
the defect. -/
theorem obstructionClass_eq_zero_iff_not_detectedObstruction
    (boundary : FaceBoundary Face Edge) (defect : FaceDefect Face) :
    obstructionClass boundary defect = 0 ↔
      ¬ HasDetectedObstruction boundary defect := by
  exact (obstructionClass_eq_zero_iff_correctable boundary defect).trans
    (correctable_iff_not_detectedObstruction boundary defect)
...
/-- The explicit glued-square mismatch represents a nonzero quotient
obstruction class. -/
theorem doubleSquareMismatch_obstructionClass_ne_zero :
    obstructionClass doubleSquareBoundary doubleSquareMismatch ≠ 0 := by
  rw [ne_eq, obstructionClass_eq_zero_iff_correctable]
  exact doubleSquareMismatch_not_correctable
...
/-- info: 'PhysicsSM.Draft.NullEdge.FiniteSpinCochainObstruction.obstructionClass_eq_zero_iff_not_detectedObstruction' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms obstructionClass_eq_zero_iff_not_detectedObstruction
...
/-- info: 'PhysicsSM.Draft.NullEdge.FiniteSpinCochainObstruction.doubleSquareMismatch_obstructionClass_ne_zero' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms doubleSquareMismatch_obstructionClass_ne_zero
