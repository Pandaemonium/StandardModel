...
1. **No on-plane vertices.** Every vertex is strictly on the positive or
   ...
   avoids a three-valued vertex predicate. NOT yet checked against
   Osterwalder-Seiler's primary-source construction (LIT_LOG item 1
   remains open on this specific point) - flagged, not resolved, by this
   file.
...
links - checked in `reflectE_positiveLink` / `reflectE_cutLink`), and
`reflectLinkField` is an involution on configuration space. NO Wilson
action, expectation, or RP inequality lives here yet - this is purely
the combinatorial/type-theoretic scaffolding `<(theta F)* F> >= 0` will
eventually be stated over.

Draft-trust: kernel-checked, no `s o r r y`, no `n a t i v e _ d e c i d e`.
Claim label: **finite identity / first-pass definitions**, explicitly
NOT cross-reviewed yet. Prerequisites: `GateYM.GaugeCoreGeneral`.
-/
