...
* **The massless walk has exactly TWO band-touching quasimomenta on the
  fundamental zone `(-pi, pi]`:** the Dirac cone at `k = 0` (bands touch at
  quasienergy `0`, `U = 1`) and the zone-edge partner at `k = pi` (bands
  touch at quasienergy `pi`, `U = -1`). In the terminology of Gupta--Short,
  the latter is a pseudo-doubler rather than a second zero-quasienergy root.
...
1. `zero_mode` -- `U(0, 0) = 1`: the bands are degenerate at quasienergy `0`
   at the zone center.
2. `pi_mode_partner` -- `U(pi, 0) = -1`: the bands are degenerate at
   quasienergy `pi` at the zone edge, exhibited exactly.
...
Do not weaken the statements.  Helper lemmas welcome.  Proofs completed by Aristotle project `e7f8ed51-d073-41ea-8b42-86a4c2cc79ef` and independently checked under the pinned toolchain.
-/

namespace PhysicsSM.Draft.NullEdge.FermionDoublingAudit
...
theorem pi_mode_partner : walkU Real.pi 0 = -1 := by
  unfold walkU
  ext i j
...
/-- info: 'PhysicsSM.Draft.NullEdge.FermionDoublingAudit.massless_gapless_classification' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms massless_gapless_classification

/-- info: 'PhysicsSM.Draft.NullEdge.FermionDoublingAudit.gap_uniform_bound' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms gap_uniform_bound

/-- info: 'PhysicsSM.Draft.NullEdge.FermionDoublingAudit.pi_mode_partner' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms pi_mode_partner

end PhysicsSM.Draft.NullEdge.FermionDoublingAudit
