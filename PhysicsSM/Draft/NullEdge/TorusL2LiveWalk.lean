...
/-!
# Fixed-torus L2 convergence of the live 3+1 walk

`FullLiveCoefficientConvergence` controls the complete square-summable
four-spinor error over `Z^3`, including omitted ultraviolet modes. Mathlib's
multivariate Fourier Hilbert basis identifies scalar `L2` on the unit
three-torus isometrically with square-summable coefficients indexed by `Z^3`.

This module performs the exact reindexing and Parseval transport. Each scalar
spinor coordinate converges strongly in torus `L2`, and therefore all four
coordinates converge in their finite product.

Claim boundary: this is a fixed unit-torus Fourier theorem. It is not a
changing-lattice sampling/interpolation theorem on `R^3`, does not identify the
limit with a position-space Dirac PDE solution, and does not prove Lorentz
restoration. The four-component target is a finite product of scalar `L2`
spaces, not a separately packaged Bochner-`L2` space.

Provenance: clean-room composition of Mathlib's
`UnitAddTorus.mFourierBasis`, `lp.norm_rpow_eq_tsum`, and exact reindexing with
the landed complete live coefficient theorem. The proof was prepared locally
while Aristotle project `3f6bb22c-323a-4b3d-9f3a-718a0eeb7355` was running;
the remote job was canceled after the local target compiled. No compiled
evaluator is used.
-/
...
/-- Parseval plus the explicit reindexing: the scalar torus-L2 norm is exactly
the corresponding coefficient-square sum. -/
...
/-- Strong fixed-torus L2 convergence for each spinor coordinate. -/
...
/-- The four scalar reconstructions, viewed as a finite product of torus-L2
spaces. -/
...
#print axioms scalarFullErrorTorus_norm_sq_eq
...
#guard_msgs (whitespace := lax) in
#print axioms fullSpinorErrorTorus_tendsto_zero
