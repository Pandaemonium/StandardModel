...
/-!
# The positive repair of the gap-to-pole obstruction: moment data restores well-posedness

Origin-of-mass gate A4. The repository already owns the NEGATIVE half: in every finite
dimension there exist operators with the SAME spectrum whose physical-sector weight takes
maximally different values (`GapPoleGeneralObstruction`), so a `spectrum -> mass readout`
map is ill-posed. That is an obstruction without a remedy, which is only half a result.

This module supplies the remedy and packages the two into a dichotomy: **the physically
correct input is the spectral MEASURE, not the spectrum.**

## What is proved

* `spectralWeight_nonneg`, `sum_spectralWeight` - the spectral weights `|<e_j, v>|^2` are
  nonnegative and sum to `‖v‖^2`, by Parseval: they are a genuine mass distribution.
* `matrix_power_inner_eq_moment` - the moment identity `<v, H^k v> = sum_j w_j lam_j^k`
  for an orthonormal eigenbasis. This is what ties the abstract weights to a measurable
  quantity.
* `moments_injective`, `response_eq_of_moments_eq` - **the repair.** With fixed distinct
  atoms, the first `n` moments determine the weights (Vandermonde injectivity), hence
  determine the entire resolvent response.
* `weights_eq_of_response_eq_on_infinite`, `response_data_unique_on_infinite` - uniqueness
  from the response itself: equal responses on an infinite subset of the resolvent set
  force equal weights, and with distinct nonzero-weight atoms the atomic presentation
  agrees up to permutation.
* `four_moments_determine_two_atoms` - the `n = 2` Prony/Hankel case: four moments
  determine two distinct atoms and positive weights up to swapping.
* `two_moments_do_not_determine_three_weights` - **sharpness.** At atoms `0, 1, 2` the
  weight vectors `(1,0,1)` and `(0,2,0)` are distinct, nonnegative, and share their first
  two moments. So `n - 1` moments do NOT suffice at `n = 3`; the count in the repair is
  not improvable by one.
* `spectrum_obstructed_moments_repair` - the packaged dichotomy: a self-contained
  same-spectrum/different-readout witness together with the statement that equal moment
  data forces equal response.

## Honest scope

**Grading.** The mathematics is classical: Parseval, Vandermonde invertibility, and the
truncated moment problem are textbook, and the Kaellen-Lehmann spectral representation is
standard practice in lattice QCD (see e.g. arXiv:2103.11846 for spectral-density
extraction from correlators, arXiv:1201.3415 for the representation itself). The claim
grade is therefore `M`, `[comp]` for the mathematics; the `[orig]` element is only the
packaging as a finite well-posedness DICHOTOMY for a spectrum-to-mass readout, with a
matched sharpness witness.

**Not proved.** The general `2n`-moment Prony theorem was not developed; the reported
obstacles are the general Hankel-rank nondegeneracy argument and recovery of an unordered
root multiset. The explicit `n = 2` case is proved instead, and the general case remains
open. Do not state a general moment-recovery theorem on the strength of this module.

**Not a physical mass claim.** Everything here concerns a finite spectral measure and its
resolvent response. It says nothing about whether any particular physical observable
realizes this readout, and in particular it does not identify a spectral gap with a
physical mass - that identification is exactly what the negative half forbids doing on
spectral data alone.

Provenance: Aristotle project `fd14e094-df01-4305-bbaf-fecca8702c29`, task
`fe0a025c-c206-4749-84d3-30abd1019fc6`, verified verbatim at the pinned toolchain. The
prover's short namespace `SpectralMeasureRepair` is kept verbatim, per this repository's
practice of not renaming into long dotted paths.
-/
...
/-- info: 'SpectralMeasureRepair.spectralWeight_nonneg' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms spectralWeight_nonneg
/-- info: 'SpectralMeasureRepair.sum_spectralWeight' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms sum_spectralWeight
/-- info: 'SpectralMeasureRepair.moments_injective' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms moments_injective
/-- info: 'SpectralMeasureRepair.matrix_power_inner_eq_moment' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms matrix_power_inner_eq_moment
/-- info: 'SpectralMeasureRepair.weights_eq_of_response_eq_on_infinite' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms weights_eq_of_response_eq_on_infinite
/-- info: 'SpectralMeasureRepair.response_data_unique_on_infinite' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms response_data_unique_on_infinite
/-- info: 'SpectralMeasureRepair.two_moments_do_not_determine_three_weights' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms two_moments_do_not_determine_three_weights
/-- info: 'SpectralMeasureRepair.four_moments_determine_two_atoms' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms four_moments_determine_two_atoms
/-- info: 'SpectralMeasureRepair.same_spectrum_different_readout' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms same_spectrum_different_readout
/-- info: 'SpectralMeasureRepair.spectrum_obstructed_moments_repair' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
