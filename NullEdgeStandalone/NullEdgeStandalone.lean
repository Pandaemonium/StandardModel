...
Standalone, mathlib-only extraction of the most important null-edge finite
algebra material from the `PhysicsSM` project.

The package keeps the original `PhysicsSM.*` module names so theorem references
match the monorepo.  Its trust labels are documented in `docs/TRUST_AND_SCOPE.md`.
-/
