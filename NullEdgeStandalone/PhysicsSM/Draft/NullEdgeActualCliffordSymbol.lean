...
theorem clifford_relation (μ ν : Fin 4) :
  ...
  fin_cases μ <;> fin_cases ν <;> simp +decide [ gamma, eta, gamma0, gamma1, gamma2, gamma3, pauli1, pauli2, pauli3, Matrix.fromBlocks_multiply ];
...
theorem blockA_mul_blockB (p : Fin 4 → ℂ) :
  ...
  ext i j; fin_cases i <;> fin_cases j <;> simp +decide [ blockA, blockB, Matrix.mul_apply ] <;> ring_nf; all_goals unfold mink; norm_num; ring_nf;
...
theorem blockA_det (p : Fin 4 → ℂ) : (blockA p).det = mink p := by
  unfold mink; unfold blockA; norm_num [ Fin.sum_univ_succ ] ; ring_nf;
  norm_num ; ring_nf

theorem blockB_det (p : Fin 4 → ℂ) : (blockB p).det = mink p := by
  unfold blockB mink; norm_num [ Fin.sum_univ_four ] ; ring_nf;
  norm_num ; ring_nf
