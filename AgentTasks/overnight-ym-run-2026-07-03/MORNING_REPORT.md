...
- `WilsonReflectionCompatibility.lean`: `rhoOppositeInv`, `rhoOppositeInv_mul`, `rhoOppositeInv_one`, `rhoOppositeInv_unitary`, `wilsonLocalWeight_rhoOppositeInv`, `wilsonOppositeKernel_posSemidef`, `localWeight_hol_reflectLinkField_mirrorPlaquette_wilson`, `productWeight_reflectLinkField_mirrorPlaquette_wilson`, `weight_reflectLinkField_mirrorPlaquette_wilson`, `wilsonLocalOppositeCompatibility_of_rhoOppositeInv`, Wilson mirror-stable product/weight/numerator/expectation wrappers, Wilson paired-family product/weight/numerator/expectation wrappers, `wilsonOppositeWeight_gauge`, `wilsonOppositeExpectation_observable_comp_gauge`, and `wilsonOppositePartition_pos`.
  ...
  This closes the Wilson/opposite inverse-representation bridge at the single-plaquette, product-weight, and `PlaquetteEnsemble.weight` levels: reflecting a Wilson plaquette product for `rho` becomes the mirrored product for the opposite inverse representation `rhoOppositeInv rho`. It also packages mirror-stable and paired-family same-family product/ensemble reflection identities under an explicit Wilson local-compatibility hypothesis. The opposite inverse Wilson ensemble inherits one-link kernel PSD, gauge invariance, and partition positivity. It still does not prove concrete same-family compatibility, cut factorization, or RP-LINK.
...
- RP-LINK is not assembled. Missing pieces: concrete reflection-stable plaquette-family instantiation, concrete same-family Wilson local compatibility, cut factorization, positive-side observable algebra, and PSD assembly. The Wilson/opposite inverse-representation bridge, Wilson-language mirror-stable wrappers, and one-link kernel PSD transfer are now proved in `WilsonReflectionCompatibility.lean`.
...
2. Instantiate the paired mirror-family constructor in a concrete lattice shape, then prove or correctly restate the same-family Wilson local compatibility required by the Wilson mirror-stable wrappers in `WilsonReflectionCompatibility.lean`. That is the closest YM3 paper unit after tonight's T3 work.
