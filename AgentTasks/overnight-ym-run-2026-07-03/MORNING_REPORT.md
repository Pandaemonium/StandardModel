...
- `WilsonReflectionCompatibility.lean`: `rhoOppositeInv`, `rhoOppositeInv_mul`, `rhoOppositeInv_one`, `rhoOppositeInv_unitary`, `wilsonLocalWeight_rhoOppositeInv`, `localWeight_hol_reflectLinkField_mirrorPlaquette_wilson`, `productWeight_reflectLinkField_mirrorPlaquette_wilson`, `weight_reflectLinkField_mirrorPlaquette_wilson`, `wilsonOppositeWeight_gauge`, `wilsonOppositeExpectation_observable_comp_gauge`, and `wilsonOppositePartition_pos`.
  ...
  This closes the Wilson/opposite inverse-representation bridge at the single-plaquette, product-weight, and `PlaquetteEnsemble.weight` levels: reflecting a Wilson plaquette product for `rho` becomes the mirrored product for the opposite inverse representation `rhoOppositeInv rho`, and the opposite inverse Wilson ensemble inherits gauge invariance and partition positivity. It still does not prove same-family reflection invariance, cut factorization, or RP-LINK.
