...
/-! ### Explicit ambient / admissible / codimension split

The codimension theorem above is sharpened into a literal dimension bookkeeping
of the *full block matrix space*: forbidding the two diagonal blocks removes
exactly `(card L)^2 + (card R)^2` parameters, leaving the `2·card L·card R`
admissible off-diagonal Yukawa parameters.  This is the precise sense in which
the chirality grading "cuts the full block matrix space by the stated
codimension". -/

omit [DecidableEq L] [DecidableEq R] in
/-- The ambient finite operator space on the chiral square `L ⊕ R` has dimension
`(card L + card R)^2`. -/
theorem ambient_finrank :
    Module.finrank ℂ (Matrix (Sum L R) (Sum L R) ℂ)
      = (Fintype.card L + Fintype.card R) ^ 2 := by
  rw [Module.finrank_matrix]
  simp [Fintype.card_sum, sq]

omit [DecidableEq L] [DecidableEq R] in
/-- The admissible odd subspace (kernel of `diagBlocks`) has dimension
`2 · card L · card R`: exactly the off-diagonal Yukawa/Higgs parameters, the
only counterterms the chirality grading leaves free. -/
theorem admissible_odd_finrank :
    Module.finrank ℂ (LinearMap.ker (diagBlocks (L := L) (R := R)))
      = 2 * Fintype.card L * Fintype.card R := by
  have hrn := (diagBlocks (L := L) (R := R)).finrank_range_add_finrank_ker
  have hrange : LinearMap.range (diagBlocks (L := L) (R := R)) = ⊤ :=
    LinearMap.range_eq_top.2 diagBlocks_surjective
  rw [hrange, finrank_top, Module.finrank_prod, Module.finrank_matrix,
    Module.finrank_matrix] at hrn
  simp only [Module.finrank_self, mul_one] at hrn
  have hamb : Module.finrank ℂ (Matrix (Sum L R) (Sum L R) ℂ)
      = (Fintype.card L + Fintype.card R) ^ 2 := ambient_finrank
  rw [hamb] at hrn
  nlinarith [hrn]

omit [DecidableEq L] [DecidableEq R] in
/-- **Ambient = admissible + forbidden (codimension split).**  The full block
matrix space splits, dimensionally, as the admissible off-diagonal Yukawa space
plus the forbidden diagonal-mass space:

```text
(card L + card R)^2 = 2·card L·card R + ((card L)^2 + (card R)^2).
```

Forbidding diagonal blocks therefore cuts the ambient operator space by exactly
the codimension `(card L)^2 + (card R)^2`. -/
theorem ambient_eq_admissible_add_codimension :
    Module.finrank ℂ (Matrix (Sum L R) (Sum L R) ℂ)
      = Module.finrank ℂ (LinearMap.ker (diagBlocks (L := L) (R := R)))
        + (Fintype.card L ^ 2 + Fintype.card R ^ 2) := by
  rw [ambient_finrank, admissible_odd_finrank]; ring
