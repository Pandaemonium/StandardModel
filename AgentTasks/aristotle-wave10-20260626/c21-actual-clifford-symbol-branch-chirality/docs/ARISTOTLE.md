...
scans executable Lean code for placeholder and escape-hatch tokens, compares
candidate theorem/lemma signatures against the live file when possible, and
prints a `git diff --no-index` command for the exact low-token review surface.
Do not rely on ad hoc summary filenames in returned projects; the dry-run
helper report is the canonical first inspection surface.

### Low-token integration checklist

Use the helper and mechanical checks to reduce review to the changed proof
bodies.

1. Run `aristotle list --limit N` before resubmitting anything. A shell timeout
   can happen after Aristotle has created a project.
2. Run the dry-run helper with the relevant task note. Read the report, not the
   extracted tree.
3. If the helper reports unchanged theorem/lemma signatures, review only the
   diff it prints. If any signature changed, stop and audit the statement before
   copying code.
4. Ask Aristotle in the submission prompt to finish with a short completion
   report: solved targets, statement changes, remaining proof holes, and
   axioms or assumptions used. This report is useful, but it does not replace
   the dry-run helper or Lean checks.
5. Use the strongest mechanical axiom/source check available in the current
   Lean MCP session for each promoted declaration. If that tool is unavailable,
   use the repo's usual `lake env lean` checks plus an explicit axiom/source
   audit before promotion.
6. Reuse the context pack generated by `make_context_pack.py` during
   integration. It is the review cheat-sheet for conventions and nearby lemmas,
   not just a submission artifact.

Batch fetching, extraction, placeholder scans, and builds when several projects
complete together. Do not batch draft-to-trusted promotion: each theorem still
needs a separate semantic review for statement identity, conventions, and hidden
assumptions.
...
- Run `lake env lean <file>` for each integrated Lean file.
- Run targeted `lake build <module>` for each integrated module.
- Run one full `lake build` and one `pre-commit run --all-files` per integration
  batch before claiming trusted work complete.
