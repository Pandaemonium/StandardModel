...
/-!
# Aristotle target: full coefficient-space convergence, including the tail

Close all proof holes without changing statements. This target differs from
`liveModeError_tendsto_zero`: the approximation is genuinely zero outside the
current box, so the full error there is minus the exact evolved coefficient.
Use exact-flow unitarity, eventual box inclusion, the live bulk-rate bound, and
Tannery domination to prove convergence over all countable modes.

Claim boundary: this is the complete common coefficient-space theorem. It is
not a changing-lattice Fourier-isometry, sampling/interpolation, or
...
theorem bulkRate_le_full_constant (t : Real) (M N : Nat) :
  ...
  have hden : (1 : Real) <= (refinementRadius M N : Real) := by
    exact_mod_cast (show 1 <= refinementRadius M N by
      ...
      omega)
  have hpos : (0 : Real) < (refinementRadius M N : Real) := lt_of_lt_of_le zero_lt_one hden
