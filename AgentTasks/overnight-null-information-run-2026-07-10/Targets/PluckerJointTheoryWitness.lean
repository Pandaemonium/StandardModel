...
/-!
# One-pair joint witness for the finite mass/dynamics/ensemble chain

This staging target requires the same supplied spinor pair to instantiate the
Hodge class cost, action Hessian, variational-flow stiffness, Gibbs gap, and
both finite response identities. It is a composition theorem on one witness,
not a claim that the spinor decorations or action have been derived.
-/

namespace PluckerJointTheoryWitness
...
/-- The same pair supplies every scalar in the finite mass/action/flow/ensemble
chain; no interface is allowed to introduce an unrelated mass parameter. -/
theorem one_pair_joint_chain
    ...
    ((quartetB (qe2 + quartetQ chi)
      ...
      HasDerivAt
          (fun b => Real.log
            (PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.partition
              (PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.pluckerTwoLevel
                psi phi) b))
          (-PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.meanEnergy
            (PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.pluckerTwoLevel
              psi phi) beta) beta ∧
      HasDerivAt
          (PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsVariance.meanEnergy
            (PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.pluckerTwoLevel
              psi phi))
          (-PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsVariance.variance
            (PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.pluckerTwoLevel
              psi phi) beta) beta := by
  ...
  · rw [arbitrary_spinor_class_cost_eq_plucker,
    ...
    rfl
  · exact
      PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.log_partition_hasDerivAt
        _ beta
  · exact
      PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsVariance.meanEnergy_hasDerivAt
        _ beta
...
theorem rational_joint_chain_control :
  ...
  · norm_num [step]
  · exact
      PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsVariance.rational_plucker_variance_control.1
...
theorem collinear_joint_zero_control :
  ...
  · have hE :
      ...
      fin_cases i <;>
        norm_num
          [PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsResponse.pluckerTwoLevel,
            massSq, edge0, collinearEdge, spinorWedge, Complex.normSq]
    rw [hE]
    exact
      PhysicsSM.Draft.NullEdge.Carrier.FiniteGibbsVariance.degenerate_spectrum_zero_control
        0 0

end PluckerJointTheoryWitness
