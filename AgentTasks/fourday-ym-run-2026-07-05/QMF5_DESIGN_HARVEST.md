<!-- Verbatim harvest of Aristotle design job d1e7bece (task 604f7774), lightly ASCII-normalized in prose punctuation only; Lean notation preserved. Reference/design artifact, not kernel-checked Lean. -->
...
`sorry` bodies acceptable). Where a symbol comes from an existing module it is named in
...
noncomputable def reflectedWilsonBlock [NeZero L] [Fintype (posHalf L nc)]
    ...
    Matrix (posHalf L nc) (posHalf L nc) ℂ := sorry
...
noncomputable def fermionReflectedWeight {A C : Type*} [Fintype A] [Fintype C]
    ...
    A -> C -> A -> ℂ :=
  fun a c b => sorry   -- ((berezinGaussian (Dₐ𝑐b))^2) or det Dₐ𝑐b, via B
```
...
N5 reflectedWilsonBlock_eq_gram   block = Mᴴ M          : NEW-HARD  : CRUX. cross-mirror hopping xP₊ = Gram of half-op ; ⊣ N3,N4  (see §D1.4)
...
* Fully explicit, 2x2, no numerics.
...
theorem transfer2_gap_pos {a b : ℝ} (hb : 0 < b) (hba : b < a) : 0 < gap2 a b := sorry
theorem z2_glueball_gap_pos {β : ℝ} (hβ : 0 < β) :
    0 < gap2 (Real.exp β) (Real.exp (-β)) := sorry
```
...
noncomputable def NE_U5_Toy.mesonTransfer (T : NE_U5_Toy) : Matrix (Fin 2) (Fin 2) ℝ := sorry
...
noncomputable def NE_U5_Toy.confinementGap (T : NE_U5_Toy) : ℝ := sorry
...
assembly and the Ward subtraction are more than a 2x2 diagonalization; do this only after
Deliverable 1 lands.
...
theorem reflectedWilsonBlock_free_posSemidef [NeZero L] [Fintype (posHalf L nc)]
    ...
    (reflectedWilsonBlock (L := L) (nc := nc) m (fun _ _ => (1 : Matrix (Fin nc) (Fin nc) ℂ)))
      .PosSemidef := by
  sorry   -- reflectedWilsonBlock_eq_gram with U == 1 ⇒ posSemidef_conjTranspose_mul_self
```
