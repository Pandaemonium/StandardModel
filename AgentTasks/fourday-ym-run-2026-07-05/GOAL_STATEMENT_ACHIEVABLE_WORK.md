...
- `TwoStateTransferZ2L1.lean`, a one-link Z2 slab bridge proving the oracle's
  ...
  two-time flux-correlation numerator identities, plus/minus center-projected
...
- Current status: Aristotle project `7c0ed511` completed as a partial
  ...
  deleted vertex type. The `treeRootChildComponent` helper now names the
  connected component generated by each root child in the deleted graph and
  ...
  positive-cardinality facts, plus the block-size bound
  `treeRootChildBlock_card_add_one_le`. The
  component-inequality-to-disjoint-block helper is checked; the remaining
  named blocker is still the tree-specific
