...
The follow-up object `treeRootChildComponent` is now integrated as the next
root-deletion scaffold.  It names the connected component of the deleted graph
generated by a root child, proves the child belongs to that component, and
proves that every vertex in the component support is reachable from the child
inside `treeRootDeletedGraph`.

## Remaining blocker

The remaining core is the geometric deletion and counting construction:

- turn the `treeRootChildComponent` supports into a disjoint block
...
Local check passed after integrating the latest helper additions:
