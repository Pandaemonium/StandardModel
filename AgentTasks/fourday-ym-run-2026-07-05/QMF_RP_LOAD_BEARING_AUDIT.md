...
No `s o r r y` or `a x i o m` declarations appear in any of the four files (grep-confirmed).
...
Caveat: because the build was skipped this session, "kernel-checked / `s o r r y`-free"
...
## a x i o m/dependency concerns

- Source grep finds **no `a x i o m` and no `s o r r y`** in the four files.
- **Not re-verified this session** (build skipped by request). The
  "`s o r r y`-free, kernel-checked" and `#print a x i o ms`-clean status asserted in the
  headers was **not** reconfirmed here; a full `lake build PhysicsSM.Draft.NullEdge.QMF`
  plus `#print a x i o ms` on the capstone lemmas is the remaining check before
