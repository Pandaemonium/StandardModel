...
- Aristotle: focused Mathlib-only packages; convention-pin `rfl` lemmas;
  ...
  thread has a proposed resolution awaiting cross-review (you have
  several open right now), submit an Aristotle audit job on it IN
  PARALLEL with peer review - do not let a design sit waiting on a
  single peer's cycle when a third opinion is one submission away.
  Check `aristotle list` at every midday integration point; fewer than
  ~4 of 8 slots running is itself a finding - go looking for the next
  audit/strategy/proof submission across the open threads and task
  board. Proofs remain encouraged wherever a step is Aristotle-sized
  (RUN_PLAN
