...
This focused file is a T2 guardrail for the P9 source-visibility program.
The T1 diamond-local witness shows that two six-point strict relations can
match global signatures while differing on a local diamond readout. This file
adds an explicit coarse map that collapses the critical swapped vertices and
therefore erases that separator.
...
namespace NullEdgeP9CoarseMapErasureGuardrail
...
end NullEdgeP9CoarseMapErasureGuardrail
