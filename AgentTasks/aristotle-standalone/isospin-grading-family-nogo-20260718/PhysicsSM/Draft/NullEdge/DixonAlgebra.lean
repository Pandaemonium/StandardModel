...
/-! ## Additive-group, `C`-scalar, and `C`-module structure (slot-wise from
`ComplexOctonion`'s landed instances). Foundational: downstream operator
modules (CAR assembly, gamma bridge) need the group/module laws. -/

instance : SMul ℂ Dixon :=
  ⟨fun z d => ⟨z • d.x0, z • d.x1, z • d.x2, z • d.x3⟩⟩

@[simp] theorem smul_x0 (z : ℂ) (d : Dixon) : (z • d).x0 = z • d.x0 := rfl
@[simp] theorem smul_x1 (z : ℂ) (d : Dixon) : (z • d).x1 = z • d.x1 := rfl
@[simp] theorem smul_x2 (z : ℂ) (d : Dixon) : (z • d).x2 = z • d.x2 := rfl
@[simp] theorem smul_x3 (z : ℂ) (d : Dixon) : (z • d).x3 = z • d.x3 := rfl

instance : Sub Dixon := ⟨fun a b => a + (-b)⟩

instance : AddCommGroup Dixon where
  add_assoc a b c := by
    refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp [add_assoc]
  zero_add a := by refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp
  add_zero a := by refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp
  add_comm a b := by
    refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp [add_comm]
  neg_add_cancel a := by
...
instance : Module ℂ Dixon where
  smul_add c a b := by
    refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp [smul_add]
  add_smul c c' a := by
    refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp [add_smul]
  mul_smul c c' a := by
    refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp [mul_smul]
  one_smul a := by refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp
  zero_smul a := by refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp
  smul_zero c := by refine Dixon.ext ?_ ?_ ?_ ?_ <;> simp
