...
- **K-B (numeric): PASSED (2026-07-08).**
  ...
  UPDATE (2026-07-08, kernel): K-B is now also kernel-checked. An explicit
  coordinate-aligned reconstruction of the 6x6 witness in
  `S1CCPhysicalSectorWitness.lean` proves (no `sorry`, standard axioms):
  `[G,K]=0` (`GK_comm`), `Q_G^2=0` (`QG_nilpotent`), `J Q_C` Hermitian
  (`JQc_hermitian`), grading anticonjugation `b(JQ_C)b = -(JQ_C)`
  (`bg_anticonj`), `N` in the radical / descent (`N_in_radical`), and the
  compressed physical-sector form `B = J Q_C|_{V'/N}` has inertia `(2,2,0)`
  (`balanced_on_physical_sector`) and is genuinely indefinite
  (`JQc_not_positive_on_sector`) - via the M engine
  `hermitian_balanced_count_of_neg_charpoly` plus `B^2=1`, `tr B=0`,
  `det B != 0`. See `S1CC_MEMO_TO_M_STRATEGY.md` for the design/no-go analysis.
