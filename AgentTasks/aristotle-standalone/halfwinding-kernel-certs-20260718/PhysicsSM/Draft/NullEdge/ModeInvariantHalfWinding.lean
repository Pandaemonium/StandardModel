...
with the **count** read off as candidate **(d)** (the fixed-leg dimension).
The displayed zero/two/four fixtures originally suggested candidate **(b)**,
half-winding parity, but the complete 16-field four-site audit later refuted
that family-level inference: wall position relative to the reflection-fixed
sites is also load-bearing. This module proves only the displayed fixtures.
...
THE RESULT: self-adjointness of the reflection-fixed-leg compression is an
exact mode-pinning mechanism for the displayed two-wall walk, while the
displayed zero-wall and four-wall controls do not carry that compression.
Determinants and naive trace indices are blind to the displayed separation.
No wall-count, half-winding, protection, or stability classification follows
from this module. The later complete four-site sign-family audit supplies an
explicit same-winding/different-signature counterexample. Axioms: standard
three for the abstract engine and counting lemmas; documented compiled
evaluation draft trust on the explicit rational K6 facts, consistent with
the sibling modules.
-/
...
/-- **The main C-rung-6 conclusion (composed).**  The displayed derived
two-wall walk has
...
stay closed).  The zero-wall control and the four-wall walk carry no such
   compression, hence no mode forced by this engine. The historical theorem
   name predates the later counterexample; the statement proves exact pinned
   modes for this fixture, not topological protection. -/
