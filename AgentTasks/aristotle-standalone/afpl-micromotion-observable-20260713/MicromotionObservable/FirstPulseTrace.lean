...
Endpoint equality alone forgets an ordered Floquet history.  This focused
target defines the trace of the first partial endpoint.  It is invariant under
a simultaneous unitary basis change, yet distinguishes a flip-flip loop from
an idle-idle loop even though both complete periods end at the identity.

This observable is not claimed to be a topological winding number.  It is the
smallest exact anti-collapse fixture that a future winding must refine.
-/

noncomputable section

namespace MicromotionObservable
...
/-- Equal Floquet endpoints do not determine this basis-invariant micromotion observable. -/
theorem equal_endpoint_distinct_micromotion :
    endpoint pulseLoop = endpoint idleLoop ∧
      firstPulseTrace pulseLoop ≠ firstPulseTrace idleLoop := by
  refine ⟨equal_endpoints, ?_⟩
  rw [firstPulseTrace_pulse, firstPulseTrace_idle]
  norm_num

/-- info: 'MicromotionObservable.firstPulseTrace_conjugate' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms firstPulseTrace_conjugate

/-- info: 'MicromotionObservable.equal_endpoint_distinct_micromotion' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms equal_endpoint_distinct_micromotion

end MicromotionObservable
