...
measurability, bounded-support `MemLp` packaging, sequential `Lp` slope limit,
strong derivative, and nonzero witness are complete and guard-pinned below.
...
theorem orbit_slope_tendsto (m R : Real)
  ...
  have h_applied : ∀ᵐ k ∂(volume : Measure FourierMomentum3), ‖k‖ ≤ R → Filter.Tendsto (fun n => (u n)⁻¹ • (momMult m (u n) k (f k) - f k)) Filter.atTop (nhds (genMult m k (f k))) := by
    refine' Filter.Eventually.of_forall fun k hk => _;
    convert ( momMult_apply_hasDerivAt_zero m k ( f k ) |> HasDerivAt.tendsto_slope_zero |> Filter.Tendsto.comp <| ?_ ) using 2;
    rotate_left;
    exacts [ u, hu, funext fun n => by simp +decide [ momMult_zero_time ] ];
  rw [ MeasureTheory.Lp.tendsto_Lp_iff_tendsto_eLpNorm' ];
  have h_dominated : ∀ᵐ k ∂(volume : Measure FourierMomentum3), ∀ n, ‖(u n)⁻¹ • (momMult m (u n) k (f k) - f k) - genMult m k (f k)‖ ≤ 2 * (3 * R + |m|) * ‖f k‖ := by
    have h_dominated : ∀ᵐ k ∂(volume : Measure FourierMomentum3), ‖k‖ ≤ R → ∀ n, ‖(u n)⁻¹ • (momMult m (u n) k (f k) - f k) - genMult m k (f k)‖ ≤ 2 * (3 * R + |m|) * ‖f k‖ := by
      have h_dominated : ∀ᵐ k ∂(volume : Measure FourierMomentum3), ‖k‖ ≤ R → ∀ n, ‖(u n)⁻¹ • (momMult m (u n) k (f k) - f k)‖ ≤ ‖fibreGenerator (k 0) (k 1) (k 2) m‖ * ‖f k‖ := by
        refine' Filter.Eventually.of_forall fun k hk n => _;
        convert slope_norm_le m ( u n ) k ( f k ) using 1;
        unfold fibreGenerator; norm_num [ norm_smul ] ;
      filter_upwards [ h_dominated, MeasureTheory.ae_of_all _ fun k hk => fibreGenerator_opNorm_le_on_ball m R k hk ] with k hk₁ hk₂;
      intro hk n; specialize hk₁ hk n; specialize hk₂ hk; refine' le_trans ( norm_sub_le _ _ ) _ ; simp_all +decide [ two_mul, add_mul ] ;
      exact le_trans ( add_le_add hk₁ ( genMult_apply_norm_le m k _ ) ) ( by nlinarith [ norm_nonneg ( f k : Spinor ) ] );
    filter_upwards [ h_dominated, hf ] with k hk₁ hk₂ n ; by_cases hk₃ : ‖k‖ ≤ R <;> simp_all +decide [ BoundedSupport ] ;
  have h_dominated : Filter.Tendsto (fun n => ∫⁻ k, ‖(u n)⁻¹ • (momMult m (u n) k (f k) - f k) - genMult m k (f k)‖ₑ ^ 2 ∂(volume : Measure FourierMomentum3)) Filter.atTop (nhds (∫⁻ k, 0 ∂(volume : Measure FourierMomentum3))) := by
    refine' MeasureTheory.tendsto_lintegral_of_dominated_convergence' _ _ _ _ _;
    use fun k => ENNReal.ofReal ( ( 2 * ( 3 * R + |m| ) * ‖f k‖ ) ^ 2 );
    · intro n
      have h_aemeasurable : AEMeasurable (fun k => (u n)⁻¹ • (momMult m (u n) k (f k) - f k) - genMult m k (f k)) volume := by
        have h_aemeasurable : AEMeasurable (fun k => momMult m (u n) k (f k)) volume := by
          have h_aemeasurable : AEStronglyMeasurable (fun k => momMult m (u n) k (f k)) volume := by
            apply momMult_apply_aestronglyMeasurable;
          exact h_aemeasurable.aemeasurable;
        have h_aemeasurable : AEMeasurable (fun k => genMult m k (f k)) volume := by
          have h_aemeasurable : AEStronglyMeasurable (fun k => genMult m k (f k)) volume := by
            exact genMult_apply_memLp m R f hf |> fun h => h.1
          exact h_aemeasurable.aemeasurable;
        exact AEMeasurable.sub ( AEMeasurable.smul ( aemeasurable_const ) ( AEMeasurable.sub ‹_› ( Lp.aestronglyMeasurable _ |> fun h => h.aemeasurable ) ) ) h_aemeasurable;
      exact AEMeasurable.pow_const ( h_aemeasurable.enorm ) _;
    · intro n; filter_upwards [ h_dominated ] with k hk; simp_all +decide [ ENNReal.ofReal ] ;
      rw [ ← ENNReal.toReal_le_toReal ] <;> norm_num;
      exact Or.inl ( pow_le_pow_left₀ ( norm_nonneg _ ) ( hk n ) 2 );
    · refine' ne_of_lt ( MeasureTheory.Integrable.lintegral_lt_top _ );
      have h_integrable : MeasureTheory.Integrable (fun k => ‖f k‖ ^ 2) (volume : Measure FourierMomentum3) := by
        convert MeasureTheory.MemLp.integrable_sq ( show MeasureTheory.MemLp ( fun k => ‖f k‖ ) 2 volume from ?_ ) using 1;
        exact MeasureTheory.MemLp.norm ( MeasureTheory.Lp.memLp f );
      convert h_integrable.const_mul ( ( 2 * ( 3 * R + |m| ) ) ^ 2 ) using 2 ; ring;
    · filter_upwards [ h_applied, h_dominated, hf ] with k hk₁ hk₂ hk₃;
      by_cases hk₄ : ‖k‖ ≤ R <;> simp_all +decide [ ENorm.enorm ];
      convert Filter.Tendsto.pow ( Filter.Tendsto.norm ( hk₁.sub_const ( genMult m k ( f k ) ) ) ) 2 using 2 ; norm_num;
      norm_num [ ← ENNReal.tendsto_toReal_iff ];
  convert ENNReal.continuous_rpow_const.continuousAt.tendsto.comp h_dominated using 2 ; norm_num [ eLpNorm_eq_lintegral_rpow_enorm_toReal ];
  any_goals exact ( 1 / 2 : ℝ );
  · rw [ MeasureTheory.lintegral_congr_ae ];
    filter_upwards [ Lp.coeFn_smul ( ( u ‹_› ) ⁻¹ ) ( ( momMultL2Isometry m ( u ‹_› ) ) f - f ), Lp.coeFn_sub ( ( momMultL2Isometry m ( u ‹_› ) ) f ) f, momMultL2Isometry_coeFn m ( u ‹_› ) f, genRepr_coeFn m R f hf ] with k hk₁ hk₂ hk₃ hk₄ ; aesop;
  · norm_num
...
/-- info: 'PhysicsSM.Draft.NullEdge.CompactSupportL2Generator.orbit_slope_tendsto' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms orbit_slope_tendsto
