...
This standalone target packages the finite scalar bridge requested by the P1-F
publication plan.

Physics context: for a two-null observer-axis block with nonnegative null
energies `a` and `b`, the unnormalized Plucker/mass scalar is
`m^2 = 4ab`, while the trace-normalized visible determinant is
`ab / (a+b)^2`. The observer readout is therefore
`2 sqrt(det rho) = m / E`, where `E = a+b`.

This is the smallest finite theorem joining the Plucker mass scalar and the
observer-conditioned normalized celestial readout.
-/

noncomputable section

namespace NullEdgeP1PluckerObserverScalarBridge.Core
...
end NullEdgeP1PluckerObserverScalarBridge.Core
