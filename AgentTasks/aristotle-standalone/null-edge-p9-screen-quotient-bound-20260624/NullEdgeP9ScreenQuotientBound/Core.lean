...
This standalone Aristotle target isolates the next finite source-visibility
lemma for the null-edge P9 branch.

Physics context: recent P9 jobs established that exact local bookkeeping is
invisible to closed screen tests, and that screen-supported residual noise is
bounded by the screen size. The next publication-facing step is to package those
facts as an actual observer-channel statement: closed-test source responses
factor through the quotient by exact sources, are stable under exact
perturbations, and have an explicit finite screen bound when the residual is
screen-supported.

Important guardrail: this file does not identify the screen projection with a
harmonic/Hodge projection. That would require a chosen inner product. The target
here is only the finite quotient-and-bound layer that can later be paired with a
separately specified Hodge metric.
-/

noncomputable section

namespace NullEdgeP9ScreenQuotientBound.Core
...
This supplies the finite-rank witness Claude flagged as necessary: quotient
visibility is being measured by a finite screen observable with an explicit
operator-size bound, not by an implicit continuum norm.
-/
...
end NullEdgeP9ScreenQuotientBound.Core
