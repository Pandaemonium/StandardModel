...
theorem fin_bundle_det_im_eq_zero {n : Nat} (psi : Fin n -> CSpinor) :
    ((finBundleMomentum psi).det).im = 0 := by
  rw [fin_bundle_det_eq_ofReal_pluckerMassReal]
  simp

/-- The determinant mass is the coercion of a nonnegative real number. -/
theorem fin_bundle_det_eq_ofReal_nonneg {n : Nat} (psi : Fin n -> CSpinor) :
    ∃ r : ℝ, 0 ≤ r ∧ (finBundleMomentum psi).det = (r : ℂ) := by
  exact ⟨finPairwisePluckerMassReal psi,
    finPairwisePluckerMassReal_nonneg psi,
    fin_bundle_det_eq_ofReal_pluckerMassReal psi⟩
