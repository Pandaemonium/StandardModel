...
/-!
## A proved affine-chart slice

Along every line through the vacuum in a basis two-form direction, each point
away from the vacuum has common-annihilator dimension three. The nonzero
hypothesis is exactly the projective-distinctness boundary on this slice.
-/

theorem annihilatorIntersectionDim_vacuum_affine_basisTwo_eq_three
    (T : Finset (Fin 5)) (hT : T.card = 2)
    (a b : ℂ) (hb : b ≠ 0) :
    annihilatorIntersectionDim vacuumSpinor
      (a • vacuumSpinor + b • basisSpinor T) = 3 := by
  unfold annihilatorIntersectionDim
  rw [show commonAnnihilator vacuumSpinor
      (a • vacuumSpinor + b • basisSpinor T) =
      commonAnnihilator vacuumSpinor (basisSpinor T) by
    ext v
    simp +decide [commonAnnihilator, mem_annihilator]
    simp_all +decide [cliffordAction_add_spinor, cliffordAction_smul_spinor]]
  rw [show vacuumSpinor = basisSpinor ∅ by rfl]
  change annihilatorIntersectionDim (basisSpinor ∅) (basisSpinor T) = 3
  rw [basis_commonAnnihilator_finrank]
  fin_cases T <;> simp_all +decide

/-- The same affine-chart result with all three scientific hypotheses kept
explicit. On this coordinate slice they are redundant for the dimension
calculation, but retaining them records the exact boundary of the general
incidence statement. -/
theorem annihilatorIntersectionDim_vacuum_affine_basisTwo_of_genuine
    (T : Finset (Fin 5)) (hT : T.card = 2)
    (a b : ℂ) (hb : b ≠ 0)
    (hψ : IsPureSpinor (a • vacuumSpinor + b • basisSpinor T))
    (horth : OrthogonalPureSpinors vacuumSpinor
      (a • vacuumSpinor + b • basisSpinor T))
    (hdist : ProjectivelyDistinct vacuumSpinor
      (a • vacuumSpinor + b • basisSpinor T)) :
    annihilatorIntersectionDim vacuumSpinor
      (a • vacuumSpinor + b • basisSpinor T) = 3 := by
  exact annihilatorIntersectionDim_vacuum_affine_basisTwo_eq_three T hT a b hb

/-- The standard weak-spinor direction is a specialization of the proved
affine basis-two-form chart. -/
theorem annihilatorIntersectionDim_vacuum_affine_weak_eq_three
    (a b : ℂ) (hb : b ≠ 0) :
    annihilatorIntersectionDim vacuumSpinor
      (a • vacuumSpinor + b • weakSpinor) = 3 := by
  simpa [weakSpinor] using
    annihilatorIntersectionDim_vacuum_affine_basisTwo_eq_three
      ({3, 4} : Finset (Fin 5)) (by decide) a b hb

/-- info: 'PhysicsSM.Draft.Spin10AnnihilatorIncidence.annihilatorIntersectionDim_vacuum_affine_basisTwo_eq_three' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms annihilatorIntersectionDim_vacuum_affine_basisTwo_eq_three
