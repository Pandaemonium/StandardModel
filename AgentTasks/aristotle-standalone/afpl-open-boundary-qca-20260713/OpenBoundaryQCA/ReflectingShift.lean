...
right channel moves right and a left channel moves left.  At either open end
the update stays at that endpoint for one substep and flips the channel.  The
...
This is a one-dimensional seed.  Tensoring three copies and adding a local
Pauli coin is a later theorem; no single-Weyl claim is made here.
-/

namespace OpenBoundaryQCA
...
theorem stepInv_step {N : Nat} (s : State N) : stepInv (step s) = s := by
  ...
  | false =>
      ...
      · have hxpos : 0 < x.val := Nat.pos_of_ne_zero h0
        ...
        have hpredne : x.val - 1 ≠ N := by omega
...
/-- info: 'OpenBoundaryQCA.stepInv_step' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms stepInv_step

/-- info: 'OpenBoundaryQCA.step_local' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms step_local

/-- info: 'OpenBoundaryQCA.step_right_interior' depends on axioms: [propext, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms step_right_interior

/-- info: 'OpenBoundaryQCA.stepEquiv' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
