...
* Suite D kills / nondegeneracy. `channel_charges_traceless` is the Gibbs-Duhem
  ...
  nondegeneracy gate: the four channel charges are linearly independent, so the
  GGE torus is not collapsed.
-/
...
open ModularSelection in
/-- Nondegeneracy gate for the GGE torus. The four channel charges
`Q_A, Q_C, Q_T, E` are linearly independent, so the generalized Gibbs generator
is a genuine 4-parameter family. -/
