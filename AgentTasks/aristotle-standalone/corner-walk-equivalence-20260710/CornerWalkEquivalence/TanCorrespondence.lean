...
Pro-review integration: the manuscript currently places the polynomial
checkerboard kernel (corner weight `i eps m`) NEXT TO the exactly unitary
coin `exp(-i a mu sigma_x)` without stating their exact finite relationship.
This package proves it.  The unitary coin is
...
Run `lake env lean CornerWalkEquivalence/TanCorrespondence.lean` first.
-/

namespace CornerWalkEquivalence
...
-/
theorem unitary_kernel_eq_scaled_checkerboard (aμ : ℝ)
    (hcos : Complex.cos aμ ≠ 0) (n : ℕ) (x : ℤ) (d : Bool) :
    K (Complex.cos aμ) (-(Complex.I * Complex.sin aμ)) n x d =
      (Complex.cos aμ) ^ n *
        K 1 (-(Complex.I * Complex.tan aμ)) n x d := by
  have hcw : -(Complex.I * Complex.sin aμ)
      = Complex.cos aμ * (-(Complex.I * Complex.tan aμ)) := by
...
`-i tan(a mu)`.
-/
theorem corner_ratio (aμ : ℝ) (_hcos : Complex.cos aμ ≠ 0) :
    (-(Complex.I * Complex.sin aμ)) / Complex.cos aμ =
      -(Complex.I * Complex.tan aμ) := by
