...
> tree-gauge bridge, with the boundary-lasso observable identification still
> open.
...
- `lake build PhysicsSM.Draft.NullEdge.GateYM` passed during T11 after
  `RectBoundaryLasso.lean` was wired into the aggregator.
...
- T12 verified bibliographic/abstract-level source status for historical RP/KP
  references only. It did not clear novelty claims.
- The oracle file is a convention-pinning and regression tool, not proof.

## Remaining gaps

- Boundary-lasso identity: `RectBoundaryLasso.lean` pins the boundary walk and
  holonomy order, but does not yet prove the tree-slice lasso identity.
- Full freeze Theorem 2 needs the boundary observable identification composed
  with `rect_wilson_loop_expectation_area_law`.
