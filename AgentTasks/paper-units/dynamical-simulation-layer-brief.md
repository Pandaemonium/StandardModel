...
- The v0.14 extension adds a saved-record verifier:
  `python Scripts/oracle/z2_transfer_oracle.py --verify-record <json> --json`
  validates a previously emitted JSON summary, recomputes descriptor-backed
  scalar checks, and replays partition/flux/correlation checks from the saved
  matrix payload when present.
- The v0.15 extension adds an explicit JSON-schema-style descriptor contract:
  `python Scripts/oracle/z2_transfer_oracle.py --write-schema <json>` emits
  the supported model, group, lattice-boundary, coupling, observable,
  correlation, and sector-label conventions.
- The v0.16 provenance update records the one-link Lean trace identities
  `slabTransfer_trace` and `fluxMatrix_mul_slabTransfer_trace` in the oracle
  `lean_surfaces` list.
- The v0.17 provenance update records the one-link two-step partition trace
  and raw two-time flux-correlation numerator theorem surfaces.
- The v0.18 provenance update records the normalized one-link `T = 2`,
  `tau = 1` spatial-flux autocorrelation ratio theorem:
  `fluxCorrelation_T2_eq_tanh_two_mul`.
- The v0.19 provenance update records the concrete one-link slab transfer
  symmetry and Hermitian theorem surfaces.
- The v0.20 provenance update records the normalized one-link `T = 1`
  spatial-flux expectation theorem `fluxExpectation_T1_eq_zero`.
...
- The v0.27 oracle update adds saved-record and regression checks that the
  emitted spatial-flux insertion matrix is Hermitian and involutive.
- The v0.28 oracle update adds saved-record and regression checks for the
  emitted transfer-kernel symmetry and its commutation with the center
...
- The v0.31 oracle update verifies the emitted plus/minus center-sector block
  matrices in saved records and rejects tampered sector-block matrices.
- The v0.32 oracle update replays positive spectra from the emitted
  plus/minus center-sector block matrices in saved records and rejects
  tampered block spectra.
- The v0.33 provenance update records the one-link plus/minus
  center-projected trace decomposition and the minus/plus projected trace
  ratio theorem.
- The v0.34 provenance update records the one-link plus/minus
  center-projected two-step trace identities, their reconstruction of the
  full two-step trace, and their minus/plus projected trace-ratio theorem.
- The v0.35 oracle update emits center-sector partition traces
...
- The v0.36 provenance update records arbitrary finite-time one-link
  ...
  `tanh(beta)^T`.
- The v0.37 provenance update records arbitrary finite two-interval one-link
  spatial-flux autocorrelation theorem surfaces: the raw numerator
  `Tr(F K^tau F K^sigma)` and the normalized eigenvalue-branch ratio.
- The v0.38 provenance update records the cross-multiplied contraction-factor
  form of that arbitrary two-interval one-link spatial-flux autocorrelation,
  identifying the `tanh beta` factor without adding denominator side
  conditions.
...
- `PhysicsSM/Draft/NullEdge/GateYM/TwoStateTransferZ2L1.lean` formalizes the
  ...
  The concrete one-link transfer matrix is also proved symmetric and
  Hermitian.
  ...
  a center-projected trace decomposition, and a minus/plus projected trace
  ratio theorem identifying that ratio with `tanh beta`. It also proves
  normalized one-step flux expectation zero,
  ...
  `tanh(beta)^T`. It also proves the arbitrary two-interval spatial-flux
  numerator `Tr(F K^tau F K^sigma)` and its normalized eigenvalue-branch
  ratio, plus a cross-multiplied contraction-factor form in terms of
  `tanh beta`. The normalized one-link `T = 2`, `tau = 1`
...
- `PhysicsSM/Draft/NullEdge/GateYM/FermionicReflection.lean` now names the
  ...
  direction, and proves the plus/minus blocks and their temporal sum PSD and
  Hermitian for any instantiated coupling using the lifted Wilson-projector
  Gram lemmas.
...
| --- | --- | --- |
| `Scripts/oracle/z2_transfer_oracle.py` | Exact Z2 1+1D finite slab transfer engine | Descriptor-driven oracle evidence: transfer traces, sector partition traces, observables, requested two-time flux-correlation profiles, spectra, full/sector first gaps, sector blocks, optional reproducibility matrices, descriptor schema emission, and saved-record replay verification including spectrum lists, sector-block matrices and replayed block spectra/partition traces, transfer-kernel, spatial-flux insertion, center-projector, and flux/center matrix algebra. |
