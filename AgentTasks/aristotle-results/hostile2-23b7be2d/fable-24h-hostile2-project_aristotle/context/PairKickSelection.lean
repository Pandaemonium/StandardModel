...
POSITIVELY - block-gauge equivariance + Hermiticity + vanishing at
z = 0 force exactly the one-complex-parameter pair-kick family
(uniqueness, converse, and load-bearing control). The supplied
interaction is now the UNIQUE phase-reading coupling in its class.
-/
import Mathlib
...
* EQUIVARIANCE: for all unimodular `u` and all `z`,
  ...
  field under the walk's exact common-phase action);
...
T3 (control, the kill-condition check embodied): dropping equivariance
admits a coupling OUTSIDE the family - exhibit
`H' z = !![1, 0; 0, 0]` (constant, Hermitian, but neither vanishing at
zero nor equivariant with any nonzero charge); state precisely which
constraint it violates (`H' 0 /= 0`), so the constraint set is
