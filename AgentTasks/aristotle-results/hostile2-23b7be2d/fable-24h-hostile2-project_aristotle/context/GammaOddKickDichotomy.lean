...
demanded by the C3 audit is IMPOSSIBLE on same-site pair blocks (the
...
* Gamma-odd generator spaces on invariant 2-blocks
  ...
  - on a FIXED-pair block (e.g. span of `(0,1)` and `(2,3)`): `g = -1`
    ...
    In particular the derived kick's support can never host the odd
    repair. (T1)
...
with exactly four `-1`-fixed states (the four same-site pairs) and
twelve two-cycles; state as: for every pair index `p`, either
