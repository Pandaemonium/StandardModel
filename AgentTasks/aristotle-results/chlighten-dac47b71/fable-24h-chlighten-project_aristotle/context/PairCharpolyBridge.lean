/-
Provenance: Aristotle job 778f63f0 (fable-24h-charbridge), harvested
2026-07-12 ~03:50 PDT. Closes the E-headline charpoly IDENTIFICATION:
V_charpoly_eq proves 5^11 * charpoly(V) = the explicit degree-28
polynomial, composed in V_charpoly_factored with the sibling
factorization to give charpoly(V) (scaled) = the product of spectral
factors. HEAVY: four native_decide on the 28x28 charpoly over ZZ[i] -
verified in Aristotle's build; local rebuild needs a high-memory machine
(>34GB), so this module is NOT routed through the always-on aggregate
guard (see PairSpectrumFixtureGuard.lean).
-/
import Mathlib
import PhysicsSM.Draft.NullEdge.PairCharpolyBridgeAux
