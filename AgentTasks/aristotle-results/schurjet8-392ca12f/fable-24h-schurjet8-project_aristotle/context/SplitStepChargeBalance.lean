/-
Provenance: Aristotle job 90c10f85 (fable-24h-chargebal), harvested
2026-07-11 ~19:50 PDT. All four statements integrated UNCHANGED; three
declared helper lemmas added by the prover. KERNEL-ONLY.
Oracle: chargebal oracle (2026-07-11) - Schur reduction at all 8
crossing nodes of the 3+1 successive-axis walk; Jacobians collapse to
+-(4/5) diag(-1,1,-1).
Program role: the first kernel instance of the corrected route-A charge
bookkeeping on this paper's own walk - per-node Weyl-sector charges
+-1 (det +-64/125), exact Floquet opposition at every node, and the
8-node census sums to ZERO at each gap (A4's total-charge-zero input,
fixture grade).
-/
import Mathlib
