...
the composed step's two-particle spectrum is computed exactly
(oracle-exact): the free levels that persist and twelve
interaction-shifted quasienergies solving a rational cubic in
$2\cos2\varepsilon$; the kernel formalization of this factorization is
the paper's pending flagship (Section~4).  A
...
is the landed theorem.  The ring-level witness is now machine-checked:
...
At the rational $3$-$4$-$5$ kick the
...
formalization in flight --- the in-flight spectrum-fixture job fills
exactly this slot: the factorization identity, the cubic reduction, and
pinned-mode witnesses; until it lands every constant in this section
carries the oracle-exact tag and nothing in this section is claimed as
kernel].  The structural companion is now machine-checked
(\texttt{PairMomentumBlocks}, \DraftTrust{}: kernel statements
transported from two disclosed compiled-evaluator identities on a
Gaussian-rational twin): the cyclic momentum $K\in\mathbb{Z}/4$ splits
the 28
