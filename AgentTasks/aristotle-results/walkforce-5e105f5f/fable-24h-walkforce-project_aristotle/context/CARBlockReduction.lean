/-
Provenance: Aristotle job 4f6a5cef (fable-24h-carblock), harvested
2026-07-12 ~00:55 PDT. All four targets KERNEL-ONLY. Packaging choices
documented by the prover in the module docstring (extensional
dependence condition for the iso; Kop bundled as the linear map KopL
with a pointwise-equality lemma; conjugation direction fixed); no
statement weakened. Closes Paper A's last selection boundary: the
reduction from pair-block Fock operators to 2x2 matrices is now a
linear equivalence carrying Hermiticity both ways, the CAR generator
corresponds to the selection family (blockOf KopL z = !![0,z;conj z,0]),
and Dphase is the block shadow of the explicit Fock unitary Dfock
(phase on mode 0) with exact equivariance. T4 sharpener: a 2x2 unitary
with charpoly (X -+ 1)^2 IS -+1.
-/
import Mathlib
