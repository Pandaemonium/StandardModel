/-
Provenance: Aristotle job 4db82ed8 (fable-24h-flowr1), harvested
2026-07-12 ~00:00 PDT. HONEST PARTIAL per the submission license:
periodicity (c) and the reduction (d) are complete and feed the
verbatim hinge; the two analytic lemmas (a) local constancy and (b) the
jump law are DOCUMENTED sorries, restated by the prover with the
bracket/isolate hypotheses that make them true (the originally sketched
statements were under-specified). PARKED HANDOFF: not imported by the
draft root and not guard-pinned, because the top theorems inherit
sorryAx from (a)/(b). Next step: close the two analytic sorries (IVT +
eigenphase-branch continuity) or hand to a fresh analysis-focused job.
-/
/-
