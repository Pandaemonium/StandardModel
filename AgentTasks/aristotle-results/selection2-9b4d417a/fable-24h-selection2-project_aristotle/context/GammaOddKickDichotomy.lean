...
corner proven here (audit F9: extension to all four same-site pairs is
queued; G2 is -1 on all four, so the argument extends) (the
...
PROVEN is the per-state disjunction below plus the two pinned fixed
points 0 and 13; the exact four-fixed/twelve-two-cycle census and the
component-flip identification of `q` are queued): for every pair index `p`, either
