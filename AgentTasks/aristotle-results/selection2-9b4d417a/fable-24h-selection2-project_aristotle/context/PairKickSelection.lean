...
collapse theorem is queued in selection2, together with a lemma tying
`Dphase` to the repo's one-particle phase operator). The uniqueness is a
...
equivariance-violating control, a lemma tying `Dphase` to the repo's
one-particle phase operator, and the scalar-gauge collapse theorem
(under `diag(u,u)` the constraints force `H = 0`) are queued as the
selection2 follow-up.  Until they land, "load-bearing" is established
for the vanishing constraint only.
