/-
Provenance reference module (kept verbatim, commented out).

This file was supplied purely as a provenance reference for the pair-spectrum
fixture.  It imports upstream modules `PhysicsSM.Draft.NullEdge.FiniteCARFockBasic`
and `PhysicsSM.Spinor.PluckerMass` which are **not part of this fixture project**,
so it cannot be elaborated here.  To keep the project building, its original
contents are preserved verbatim inside the block comment below.  The actual
proof deliverables live in `PairSpectrumFixture.lean` and
`PairSpectrumFixtureC.lean`.
-/
