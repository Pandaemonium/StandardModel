...
| A-SELECT | selection conjecture RESOLVED at block level; control suite complete (vanishing + equivariance + scalar collapse) | PairKickSelection.*; Selection2Repairs.scalar_gauge_collapse/equivariance_violating_control | kernel-clean; LANDED 07-11; CAR-to-block reduction now a THEOREM (CARBlockReduction); constraint set FORCED (MassCovarianceForcing); dynamical commutant open |
...
| A-FORCE | selection constraint set FORCED: covariance group of {B_z} = chiral phase circle (static) | MassCovarianceForcing.classification/covariance_group_eq_chiralPhase/blockOf_gauge_forced | kernel-clean; LANDED 07-12; FULL covariance group both cosets (CovarianceGroupFull); static-family scope, dynamical commutant OPEN |
