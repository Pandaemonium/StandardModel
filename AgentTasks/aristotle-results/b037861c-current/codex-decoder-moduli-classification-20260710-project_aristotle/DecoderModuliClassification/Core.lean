...
/-- Entrywise characterisation of the commuting condition. -/
lemma commutesQ_iff (D : Mat3) :
  ...
  · intro h
    refine ⟨?_, ?_, ?_, ?_⟩
    · have := h 1 1
      simp [Q, Matrix.mul_apply, Fin.sum_univ_three] at this
...
/-- The physical coordinate of a normal-form matrix. -/
...
/-- The value of the exact deformation on the physical coordinate is zero. -/
lemma exactDeformation_apply_22 (R : Mat3) : (exactDeformation R) 2 2 = 0 := by
  simp [exactDeformation, Q, Matrix.mul_apply, Fin.sum_univ_three, Matrix.add_apply,
    Matrix.vecMul, dotProduct, Matrix.cons_val]

/-- Every exact deformation commutes with `Q`. -/
lemma commutesQ_exactDeformation (R : Mat3) : CommutesQ (exactDeformation R) := by
  rw [commutesQ_iff]
  refine ⟨?_, ?_, ?_, ?_⟩ <;>
    simp [exactDeformation, Q, Matrix.mul_apply, Fin.sum_univ_three, Matrix.add_apply,
      Matrix.vecMul, dotProduct, Matrix.cons_val]

/-- An explicit presentation `R` realising any four physical-free coordinates as
an exact deformation. -/
lemma exactDeformation_normalForm (a b c h : Complex) :
    exactDeformation (!![b, 0, 0; a, 0, c; h, 0, 0]) = normalForm a b c h 0 := by
  apply Matrix.ext
  intro i j
  fin_cases i <;> fin_cases j <;>
...
theorem chainHomotopic_iff_physicalEigenvalue_eq
  ...
  · intro hphys
    -- D' - D commutes and has zero physical coordinate, hence is exact
    ...
    obtain ⟨R, hR⟩ :=
      (exact_iff_commutes_and_physical_zero (D' - D)).mpr ⟨hdiff_comm, hdiff_phys⟩
...
theorem canonical_representative (D : Mat3) (hD : CommutesQ D) :
  ...
  have hcomm' : CommutesQ (normalForm 0 0 0 0 (physicalEigenvalue D)) := by
    rw [commutesQ_iff]; refine ⟨?_, ?_, ?_, ?_⟩ <;> simp [normalForm]
