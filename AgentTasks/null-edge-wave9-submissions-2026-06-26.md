...
| c20-massive-branch-lifting-audit | Proof/Audit | `2869964e-5656-4eb5-9b72-1b24c64a0ab7` | Massive branch lifting for `det(iD_+ + Gamma_s Phi_H)` (**integrated**; see below) |
...
All seven jobs completed and are integrated. Two new
...
| c20 (integrated 2026-06-26) | **new** `PhysicsSM/Draft/NullEdgeMassiveBranchLifting.lean` | `null-edge-massive-branch-lifting-audit.md` | **Massive lifting, algebraic half settled.** With the Gate A relations (`Gs^2=1`, `{Gs,Dp}=0`, `Dp^2=p^2.1`): `massive_square_scalar` (`M=iDp+m Gs` squares to `(m^2-p^2).1`), `massive_square_matrix` (`M=iDp+Gs Phi_H` squares to `Phi_H^2-p^2.1`, the `+Phi_H^2` Gate-A sign). Determinant verdicts on null branches (`p^2=0`): scalar `m != 0` => **lifted** (`nullBranch_scalar_lifted`), `m=0` => preserved; matrix => **lifted iff `Phi_H` invertible** (`det M=0 <-> det Phi_H=0`, so a zero mode of `Phi_H` survives). Linked to the proven four three-pi corners (`threePi_branch_scalar_lifted`, `_matrix_lifted_iff`). Did NOT assume `Phi_H` cures the branch -- the matrix case explicitly shows survivors. Verdict: PENDING (algebraic lifting fully decided; physical release still needs energy-slice real/complex split, Krein signature of survivors, and the standing sign-forcing blocker C19). Verified: `lake build PhysicsSMDraft` exit 0 (8685 jobs); 8 theorems kernel-clean; no `native_decide`. |
