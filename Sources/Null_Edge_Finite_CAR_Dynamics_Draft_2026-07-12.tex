...
formalization in flight].  The structural companion is a momentum
block-diagonalization of the free lift [oracle-exact; formalization in
flight]: the cyclic momentum $K\in\mathbb{Z}/4$ splits the 28
...
blocks carry the two Pythagorean quadruples; the local pair kick breaks
the translation symmetry exactly (explicit nonzero commutator entry
$3\ii/5$) while weighting all four momentum sectors equally
($|P_K\,e_{01}|^2=1/4$), so the composed automaton has no exact momentum
decomposition and the interaction can, and does, recruit levels from
every sector.
