...
## 32. Furey/internal-ideal selector hypothesis - 2026-06-26

Provenance: Codex literature and repo analysis prompted by the question of
whether Furey's complex-octonion minimal-ideal Standard Model construction
supplies clues for the missing null-edge mathematics. Primary literature
consulted includes Furey `1806.00612`, Furey `1611.09182`, Furey-Hughes
`2209.13016`, Furey `1910.08395`, and related Clifford/octonion internal
spectrum work.

### 32.1 Bottom-line update

Furey is likely relevant, but not as a direct mass formula.

The best current interpretation is:

```text
null edges supply the kinetic/lightlike substrate;
Furey supplies a preferred internal particle-state projector;
Phi_H supplies the zero-order mass/Yukawa block;
the missing theorem is compatibility among these three layers.
```

Furey should therefore be treated as a candidate answer to the post-C21
projection problem:
...
not to the already-refuted bare problem:

```text
OperatorForcesAlignment.
```

### 32.2 What Furey supplies

The repo already contains a substantial Furey surface:

```text
PhysicsSM/Algebra/Furey/MinimalLeftIdeal.lean
  primitive idempotent omega;
  eight basis states;
  Cl(6)-style ladder-operator action table;
  number operators;
  charge assignments.

PhysicsSM/Draft/NullEdgeInternalSpectrum.lean
  abstract internal-spectrum API;
  one-generation anomaly inheritance;
  Furey as one realization, not an assumption.

AgentTasks/null-edge-furey-internal-spectrum-bridge.md
  bridge analysis and guardrails.
```

The load-bearing guardrail remains:

```text
Furey supplies internal spectrum and anomaly consistency.
Furey does not supply fermion mass values.
Furey does not supply Phi_H by itself.
Furey does not solve QCD/confinement.
```

### 32.3 The main clue: ideals are projectors

C21 proved that the actual bare tetrahedral Clifford symbol has
two-dimensional, chirality-balanced branch kernels. Therefore ordinary bare
chirality cannot assign a single sign to a branch.

Furey's construction is built around primitive idempotents, minimal ideals, and
ladder-operator occupation structure. This is exactly the kind of algebra that
can define physical subspaces.

Hypothesis:

```text
The missing Gate C projector may be an internal-ideal projector, or a product of
branch/Krein/Weyl projectors with a Furey-style internal projector.
```

Candidate shape:

```text
P_phys(a) = P_branch(a) * P_Krein/Weyl(a) * P_Furey
```

or, if the internal and branch factors do not commute:

```text
P_phys(a) = controlled composition/quotient built from branch data and the
Furey internal ideal.
```

The target theorem becomes:

```text
FureyProjectorSelectsBranchChirality:
  for each branch a, P_phys(a) cuts the C21 two-dimensional bare kernel to a
  one-dimensional survivor, and Gamma_s or Gamma_f acts on that survivor by
  g5_a.
```

This would discharge the chirality part of `OperatorForcesAlignmentAfterProjection`.
It would not by itself discharge Krein positivity, ghost safety, or moduli
audits.

### 32.4 Possible canonical-selector theorem

C45/C46 showed that tetrahedral `S4` symmetry alone does not force the
`(+,+,-,-)` pattern and that the Route B species-splitting coefficient remains
a free modulus.

Furey/Krasnov/Todorov-style internal constructions suggest a different possible
selector:

```text
a distinguished complex structure;
a primitive idempotent;
a pure-spinor datum;
a minimal-ideal decomposition;
an internal conjugation/involution whose invariant subalgebra is
SU(3)_C x U(1)_EM.
```

The important question is whether the null-edge frame selects one of these
objects naturally.

High-value theorem target:

```text
NullFrameSelectsFureyComplexStructure:
  the tetrahedral null-edge frame canonically selects the same complex
  structure / primitive idempotent / ideal projector used by the Furey
  internal realization.
```

If true, this could turn an apparently chosen 2+2 branch partition into a
structurally selected one.

If false, Furey remains a preferred internal realization but not a consequence
of null-edge geometry.

### 32.5 Internal spectrum to Phi_H

Furey helps most cleanly with Gate H:

```text
which internal states exist;
their charges;
their colour/electroweak representation content;
their anomaly consistency.
```

The next bridge to the mass operator should not claim mass values. It should
claim legality and block structure:

```text
FureyPhiHLegalYukawa:
  the Furey internal spectrum defines the allowed left/right internal spaces;
  legal Yukawa maps are exactly the gauge-compatible maps between them;
  Phi_H is chi_E-odd and Gamma_s-even;
  the super-Dirac square therefore contains the correct +Phi_H^2 block.
```

This can make `Phi_H` less arbitrary at the level of allowed texture, but the
Yukawa magnitudes remain free unless a separate rank/codimension theorem is
proved.

### 32.6 Electroweak stabilizer comparison

Furey-Hughes-style results emphasize that certain algebraic involutions preserve
an `SU(3)_C x U(1)_EM` subalgebra. Our finite FMS/electroweak theorem isolates
the photon direction as the stabilizer of the Higgs reference section.

Possible comparison theorem:

```text
FureyConjugationStabilizer_eq_EWPhotonStabilizer:
  the algebraic conjugation-invariant subalgebra in the Furey/Hughes
  realization matches the null-edge electroweak stabilizer kernel span{Q}.
```

If true, the unbroken electromagnetic direction would be jointly selected by
the internal algebra and the finite FMS/orbit-stiffness theorem.

### 32.7 Operator-preservation audit

Furey's literature repeatedly distinguishes algebraically allowed maps from
maps that preserve the intended particle-state structure. This may be useful for
our ghost/counterterm/projector guardrails.

Define a future predicate:

```text
PreservesFureyIdealStructure(op)
```

Then prove or audit:

```text
allowed gauge actions preserve it;
legal Phi_H/Yukawa maps preserve it in the correct off-diagonal sense;
candidate branch projectors preserve it;
bad SU(5)-like or proton-decay-like maps do not;
ghost-generating or nonphysical projectors fail it.
```

This could supply a finite algebraic safety test before a projector is admitted
as physical.

### 32.8 Claim discipline

Safe claims:

```text
Furey is the preferred internal-spectrum realization.
Furey can inherit anomaly freedom through the NullEdgeInternalSpectrum API.
Furey may provide the canonical internal projector needed for projected Gate C.
Furey may constrain legal Phi_H/Yukawa textures.
```

Unsafe claims:

```text
Furey derives particle masses.
Furey closes Gate C by itself.
Furey solves the branch/nodal-line problem.
Furey fixes the species-splitting coefficient r.
Furey proves the full Standard Model or QCD dynamics.
```

### 32.9 New follow-up jobs

| ID | Type | Target | Why it matters | Output |
| --- | --- | --- | --- | --- |
| FUR-C1 | Proof/Strategy | Furey projector compatibility with C21 branch kernels. | Tests whether the internal ideal projector can discharge `OperatorForcesAlignmentAfterProjection`. | `PhysicsSM/Draft/NullEdgeFureyProjectedBranchChirality.lean` |
| FUR-C2 | Proof/Strategy | Null frame selects Furey complex structure / primitive idempotent. | Decides whether the Furey projector is geometrically forced or chosen. | `AgentTasks/null-edge-null-frame-furey-selector-audit.md` |
| FUR-H1 | Proof | Actual Furey minimal-ideal states instantiate `NullEdgeInternalSpectrum.RealizesOneGeneration`. | Turns Furey-as-preferred-realization into a concrete interface theorem. | `PhysicsSM/Draft/NullEdgeFureyInternalSpectrum.lean` |
| FUR-H2 | Proof/Audit | Furey internal grading to `Phi_H` legality and checkerboard square. | Connects internal spectrum to the zero-order mass block without claiming mass values. | `PhysicsSM/Draft/NullEdgePhiHFromFureySpectrum.lean` |
| FUR-E1 | Proof/Audit | Compare Furey/Hughes `SU(3)_C x U(1)_EM` stabilizer with finite FMS photon stabilizer. | Tests whether electromagnetic survival is selected in both layers by the same algebraic structure. | `PhysicsSM/Draft/NullEdgeFureyEWStabilizerComparison.lean` |
| FUR-S1 | Audit | Allowed-operator predicate preserving Furey ideal structure. | Provides a safety screen for projectors, Yukawa maps, and ghost-prone branch controls. | `AgentTasks/null-edge-furey-allowed-operator-audit.md` |

### 32.10 Updated architecture

The clearest current architecture is:

```text
1. Spacetime/kinetic layer:
   tetrahedral null-edge Dirac operator, finite Lichnerowicz square, null
   quadrature, determinant branches, nodal curves.

2. Internal algebra layer:
   Furey minimal ideals / division-algebraic ladder operators / internal
   spectrum and anomaly inheritance.

3. Projection layer:
   physical-sector projector combining branch, Krein, Weyl, and Furey ideal
   data.

4. Mass layer:
   Phi_H as a chi_E-odd, Gamma_s-even operator between selected internal
   spaces; Phi_H^2 appears in the super-Dirac square.

5. Safety layer:
   Krein positivity, ghost-zero safety, retarded/advanced placement, and
   moduli/prediction ledger.
```

This reframes Furey's role:

```text
Furey is not the origin of mass in this program.
Furey may be the origin of the physical internal state selector on which the
null-edge mass operator acts.
```


## 33. Furey/Baez/DVT as the finite internal spectral-triple half - 2026-06-26

Provenance: Claude follow-up analysis supplied in attachment
`66a2b3a1-f6dd-48f3-be76-73d4d74a1143/pasted-text.txt`, integrated with the
Furey/internal-ideal selector hypothesis in Section 32.

### 33.1 Correction to Section 32

Section 32 correctly identifies Furey as a candidate internal projector/selector
layer, but Claude's correction sharpens the emphasis.

Primary role:

```text
Furey/Baez/DVT supplies the finite internal spectral-triple half:
  internal Hilbert space;
  internal grading chi_E;
  gauge group and quotient;
  one-generation charge data;
  anomaly inheritance;
  candidate constraints on Phi_H.
```

Secondary, speculative role:

```text
Furey may also help constrain post-C21 projected Gate C selectors, but Gate C
itself remains a kinetic/doubling problem for D_N.
```

So the main fusion target is not:

```text
Furey solves the branch projector problem.
```

It is:

```text
Null-edge D_N supplies the external finite Dirac operator.
Furey/Baez/DVT supplies the internal finite algebra/Hilbert space/grading.
The project assembles the almost-commutative product
  D = i D_N tensor 1 + Gamma_s tensor Phi_H.
```

### 33.2 What is already stronger than the previous summary suggested

The repo already contains a substantial kernel-checked division-algebra pillar,
including:
...
According to Claude's integration note, the checked surface includes:

```text
minimal left ideal J = (C tensor O) omega;
omega = 1/2 (1 - i e111);
Cl(6)-style module/action table;
G2 fixing e111 equivalent to SU(3), with action/exactness statements;
the physically correct Standard Model gauge group quotient by Z6;
one-generation left-handed sector data and anomaly inheritance;
exceptional-Jordan / DVT stabilizer and triality scaffolding;
Krasnov-style complex structure tied to right multiplication by e111.
```

This means the Furey/Baez/DVT side is not merely motivational. It is a large
formal island waiting to be docked to the null-edge operator island.

### 33.3 Honest boundary: right-handed singlets and generations

The most important boundary is also sharper now:

```text
The formal Furey ideal currently covers the left-handed doublet side,
roughly 8 of the 15 Weyl states.

The 7 right-handed singlets are still open in the formalization and are
currently supplied by the conventional one-generation table.
```

Use the explicit marker:

```text
FureyRightHandedSectorOpen.
```

The most valuable Furey completion theorem is therefore:

```text
formalize the conjugate ideal J*
and prove that it supplies the missing right-handed singlets with the correct
charges, weak behavior, and all-left conjugation convention.
```

Three generations remain future/scaffolding only. Do not lean on triality or
three-generation Furey literature as if it were a settled theorem in this repo.

### 33.4 Almost-commutative fusion target

The strongest organizing frame is the almost-commutative spectral triple:

```text
External half:
  null-edge finite Dirac D_N from lightlike causal edges.

Internal half:
  Furey/Baez/DVT finite algebra, internal Hilbert space H_F, gauge group,
  grading chi_E, and candidate finite Dirac Phi_H.
...
```text
Connes-Chamseddine inputs both the smooth spacetime triple and the finite
internal algebra by hand.

The null-edge/Furey program attempts to derive both halves:
  D_N from null causal geometry;
  H_F, gauge algebra, and internal grading from division algebra/Jordan data.
```

The existing Gate A finite super-Dirac square was built for exactly this shape.
The next task is to fill its abstract internal slot with Furey/Baez/DVT data.

### 33.5 The lynchpin theorem: Furey-derived Phi_H

The highest-value bridge theorem is:

```text
FureyGaugeCovariantPhiH:
  define Phi_H : J -> J*
  and prove:
    it is gauge-covariant for the derived SM gauge action;
    it is chi_E-odd;
    it is Gamma_s-even in the super-Dirac convention;
    it lands in the legal Yukawa/mass-map class;
    its square enters the existing finite Lichnerowicz theorem as +Phi_H^2.
```

This does not derive Yukawa eigenvalues. It upgrades the abstract `Phi_H` slot
from a generic free block to a Furey-compatible finite Dirac/Yukawa operator.

If DVT/Jordan cubic or triality data constrain the allowed form of `Phi_H`, that
is structural constraint language, not mass-magnitude prediction unless a
separate rank/codimension theorem is proved.

### 33.6 Concrete bridge sequence

Recommended theorem order:

```text
1. FureyInternalSpectrumComputed:
   replace the posited rational-charge realization in NullEdgeInternalSpectrum
   with charges computed from the actual J-states.

2. FureyChiEEqualsKrasnovComplexStructure:
   identify the abstract internal grading chi_E with the derived Furey/Krasnov
   complex-structure data.

3. FureyRightHandedSector:
   formalize J* and close FureyRightHandedSectorOpen.

4. FureyGaugeCovariantPhiH:
   construct Phi_H : J -> J* and prove gauge covariance and chi_E oddness.

5. FureyAlmostCommutativeProduct:
   assemble D = i D_N tensor 1 + Gamma_s tensor Phi_H and feed it into the
   existing Gate A finite square.

6. FureyDVTYukawaConstraint:
   test whether DVT/Jordan cubic/triality constrains legal Phi_H textures.
```

### 33.7 Literature ingestion gap

The foundational Furey/Baez papers should be explicitly curated, especially
because repo modules cite them:

```text
1806.00612
  Furey, division-algebraic ladder operators and the complex-octonion ideal
  construction used by MinimalLeftIdeal.lean.

1810.10465
  Furey, SU(3) x SU(2) x U(1) from octonions.

0904.1556
  Baez-Huerta, The Algebra of Grand Unified Theories; division-algebra to GUT
  dictionary and convention reference.
```

Add these to Zotero/Neo4j with tags:
...
### 33.8 Updated claim discipline

Safe stronger claim:

```text
The project now has two formal pillars that appear designed to dock:
  a null-edge finite external Dirac operator;
  a Furey/Baez/DVT finite internal algebra and particle-spectrum construction.
```

Unsafe claim:

```text
Furey already supplies the full SM Hilbert space, all right-handed singlets,
three generations, or mass values.
```

Best current headline:

```text
The next unification theorem is not a mass prediction. It is a finite
almost-commutative product theorem: derive the external and internal halves
separately, then prove that the Furey-derived internal Dirac block fits the
null-edge super-Dirac square.
```


## 34. Wave 12/13 integration: Route-B Gate C and Furey internal-spectrum status

The latest integrated Aristotle waves significantly sharpen the project. They do
not change the conservative thesis of the manuscript: the null-edge program is a
reconstruction framework first, and prediction language remains conditional until
specific finite parameters or forbidden textures are derived rather than chosen.
...
with a precise Route-B proof chain.

### 34.2 Prediction language after Route B

The post-Route-B prediction ledger keeps the same conservative conclusion: the
program currently reconstructs known structures more strongly than it predicts
new numerical values. In particular, the species-splitting coefficient is not a
prediction. The best near-term prediction candidates are structural:

| Candidate | Current verdict |
|---|---|
| Forbidden same-Weyl counterterms | A consistency/codimension theorem, not yet a physical prediction. |
| Forced texture from internal algebra | Still open; Furey/DVT does not yet constrain `Phi_H` enough. |
| Gate C physical-sector structure | Promising if the projected, ghost-safe release theorem is completed. |

### 34.3 What Furey now contributes

The Furey work is now useful in a more exact way than earlier speculation. It is
not needed for the null-edge kinetic core, but it is a strong candidate for the
finite internal layer.

| Furey contribution | Integrated meaning |
|---|---|
| Computed internal spectrum | `NullEdgeFureyInternalSpectrum.lean` realizes the null-edge internal spectrum/anomaly wrapper from the Furey occupation lattice. |
| Electroweak chirality | `NullEdgeFureyChiE.lean` warns that the Krasnov/Furey complex structure is not `chi_E`; the plausible `chi_E` is number-operator parity. |
| Conjugate ideal | `Algebra/Furey/ConjugateIdeal.lean` gives a coordinate model for the right-handed/conjugate ideal side. |
| Higgs/Yukawa interface | `NullEdgeFureyPhiH.lean` gives the abstract off-diagonal, `chi_E`-odd, `Gamma_s`-even finite Higgs operator interface. |
| Almost-commutative product | `NullEdgeFureyAlmostCommutativeProduct.lean` combines the finite Furey-style layer with the null-edge super-Dirac square. |
| Photon stabilizer | `NullEdgeFureyEWStabilizerComparison.lean` matches the Furey electromagnetic generator with the null-edge FMS photon stabilizer direction. |

The next Furey step should be concrete rather than philosophical: bridge the
abstract `Phi_H` and number-parity grading to the live `J` and `J*` ideal modules.
If that bridge produces nontrivial intertwiner constraints, it may become the
first serious route toward a finite-sector texture theorem. If it does not, the
Furey layer still gives an elegant internal-spectrum realization without
claiming extra predictive power.

### 34.4 Manuscript/P1 implication

The P1 crosswalk audit again reports that manuscript patches A-F are still
needed before the draft is release-clean. Those edits mainly downgrade overmarked
claims and upgrade the now-bannered Pluecker/SL(2,C) covariance rows. They were
not applied in this integration pass because they touch the user-owned
manuscript directly.
...
### 34.6 Late Wave 12 correction: projected Gate C release API

The previously running C59 job has now landed. `NullEdgeProjectedGateCRelease.lean`
adds the conditional release certificate for Route B. This is not yet the final
physical proof that the null-edge model releases Gate C; rather, it states the
right seven-clause theorem shape:

1. nodal set controlled,
2. branch projectors controlled,
3. projected kernels are one-dimensional,
4. projected chirality is aligned,
5. the projected physical sector is Krein-positive,
6. ghost zeros are cleared,
7. the species-splitting modulus has been audited.

The important guardrail is that none of the attractive partial results is enough
by itself. A nonzero flavored index, projected chirality, or Krein positivity can
all coexist with a failure of full release. This makes the next wave's
post-gauge ghost/residue and selector jobs even more central.
...
## 35. Literature update: Gate C and Furey finite-layer clues

A targeted literature pass on 2026-06-26 added eight papers to Zotero/Neo4j and
confirmed that several central Route-B papers were already curated. Full notes:
`AgentTasks/null-edge-literature-gate-furey-search-2026-06-26.md`.

### 35.1 Gate C: Route B is externally supported

The strongest new Gate C clue is the four-dimensional MDF index-theorem paper
(arXiv:2602.19767). It verifies the index-theorem story for Karsten-Wilczek and
Borici-Creutz fermions using flavored mass terms and modified chirality. This
matches our Lean trajectory: the bare tetrahedral Clifford symbol does not carry
a single branch chirality; the meaningful object is the projected/flavored sector.

Newly curated supporting papers also sharpen the required audits:

| Paper lane | Lesson for the program |
|---|---|
| MDF index theorem in 4D | Modified chirality/flavored mass is the right topology-sensitive object. |
| KW/BC dispersion and spectral range | The split/lifting parameter behaves like a real modulus; do not claim it is predicted. |
| Taste breaking and eigenvalue-pair splitting | Gate C should monitor spectral/eigenvalue diagnostics, not only branch labels. |
| Graph-matrix/lattice-operator equivalence | C64 should use spectral-graph normal forms for nodal-set exhaustion. |

Practical update: `NullEdgeProjectedGateCRelease` has the right seven-clause
shape. Future Gate C prose should say that release requires a projected,
flavored, Krein-positive, ghost-safe sector with controlled nodal zeros. A
nonzero flavored index alone is not a release theorem.

### 35.2 Ghost safety remains independent

The already-curated Golterman-Shamir propagator-zero paper remains the key
hazard. The new dispersion/taste-breaking papers give a more concrete diagnostic
language: survivors should be pole-like positive-residue modes, while fatal
propagator zeros must be excluded from the retained sector. This should feed the
still-running C63 job and any successor if C63 returns only a conditional API.

### 35.3 Krein lock is not yet derived

C65 proves the physical selector is canonical relative to locked Krein signs, but
this literature pass did not find a theorem deriving that Krein sign from MDF
branch data alone. K3 is therefore still the right question: either derive the
Krein lock from causal/retarded-advanced/reflection structure, or keep it as an
explicit convention/input.

### 35.4 Furey layer: representation architecture, not mass prediction yet

The Furey/internal-space papers are useful, but in a bounded way.

| Paper lane | Lesson for the program |
|---|---|
| Todorov internal octonion algebra | Complex structures organize lepton/quark and Pati-Salam/Spin(10)-style structure, but should not be identified with `chi_E` without a theorem. |
| Furey-Hughes symmetry breaking | Quaternionic triality and left-right Higgs representations are the best clue for the representation carrier of `Phi_H`. |
| Furey graded superalgebra | Supports the idea that number/parity grading may be the finite-layer `chi_E` language. |
| Gesteau matrix-Yukawa duality | Useful for a Yukawa/intertwiner dimension audit, but not a numerical mass prediction. |

Thus the next Furey jobs should stay narrow: build coordinate bridges first, then
ask whether triality/left-right structure imposes a nontrivial codimension on
admissible `Phi_H` intertwiners. Until that theorem exists, Gate F prediction
language remains off.
...
## 36. Furey ideals as edge/half-edge internal fibers

Provenance: ChatGPT Pro feedback supplied in attachment
`ed9b7b7e-37d6-4e91-9267-fd2514c0a742/pasted-text.txt`, integrated with the
existing Furey/internal-spectrum and Gate C status.
...
```text
Furey supplies the finite internal fiber.
Null edges supply the external kinetic/lightlike substrate.
Phi_H supplies the zero-order Higgs/Yukawa block.
```

The edge-state architecture should be written as:
...
The null-edge operator acts on the first factor. The Furey layer supplies the
finite internal state space, charge table, internal grading, conjugate/dual
ideal structure, anomaly bridge, and admissible internal maps. The product
operator remains:
...
with `Phi_H` internal-odd under `chi_E`, spacetime-even under `Gamma_s`, and
gauge-covariant.

This is a compatibility theorem target, not a claim that Furey solves the null
kinetic core or Gate C by itself.

### 36.2 Hard wording convention: left-action algebra, not raw octonion ideals

The mathematically safe Furey object is not a naive left ideal of the raw
nonassociative complex octonions. The safe object is a module/minimal left ideal
for the associative operator algebra generated by complex-octonion left
multiplications:
...
where `p` is a primitive idempotent in the operator algebra.

Use this phrase in manuscripts and docs:
...
Avoid this unqualified phrase:

```text
particles are minimal left ideals of the complex octonions.
```

This distinction should be treated as a permanent convention guardrail, because
raw octonions are nonassociative and node composition becomes ambiguous if the
operator algebra is not separated from raw multiplication.

### 36.3 Edge/half-edge ontology

The cleanest formulation is edge-carried propagating states valued in internal
fibers:

```text
psi_e in S_{ell_e, branch} tensor F_{tau(e)}
```

where `S_{ell_e, branch}` is the external null/spin/Krein/branch factor and
`F_{tau(e)}` is a Furey ideal module, conjugate ideal module, dual module, or
selected submodule.

The cautious publication formulation is:

```text
vertices carry internal fibers;
oriented half-edges carry boundary copies of those fibers;
full edges propagate states between them.
```

Then the slogan "Furey states live on edges" means that propagating edge fields
are sections valued in the pulled-back Furey internal bundle. It does not mean
that the finite Hilbert space is ontologically edge-only.

### 36.4 Node maps are intertwiners

Nodes should compose linear maps and intertwiners, not raw unparenthesized
octonion products. A node map should be typed as something like:

```text
T_v in Hom_G(
  tensor_in F_e tensor Higgs_factors,
  tensor_out F_e
)
```

with declared behavior under gauge symmetry, internal `chi_E`, conjugation/dual
pairing, and the external spin/Krein data.

Yukawa nodes should be represented before symmetry reduction as legal
Higgs-dressed maps such as:

```text
Y_f in Hom_G(F_L tensor H, F_R)
```

or, in an all-left convention:

```text
Y_f in Hom_G(F_L tensor H, F_R^c).
```

After choosing a Higgs background, these maps become finite mass/Yukawa blocks.
Before that choice, they are gauge-covariant tensors, not numerical mass
predictions.

### 36.5 Structures not to identify without a theorem

Keep the following data separate:
...
Useful default interpretation:

```text
incoming/outgoing status -> duals and pairings;
particle/antiparticle status -> conjugate representations or charge
  conjugation;
left/right chirality -> external Weyl/spacetime factor;
chi_E -> finite/Furey internal factor;
Krein sign -> sheet/causal orientation data used by Gate C.
```

Do not collapse these structures merely because they are all two-valued in some
toy model. C21, C22, C65, and K3 show that chirality, Krein sign, branch label,
and orientation can pull apart.

### 36.6 How this helps the mass program

The Furey-on-edge/half-edge picture helps most with the finite internal half of
the almost-commutative product:

| Target | Contribution |
|---|---|
| Concrete `Phi_H` | Gives source and target modules such as `J`, `J*`, conjugate ideals, and all-left sectors. |
| Number-parity `chi_E` | Occupation-number parity is the current best internal grading candidate. |
| Conjugate ideal bridge | Separates dualization, charge conjugation, and edge reversal. |
| Node/Yukawa legality | Replaces a generic finite matrix with a gauge-compatible intertwiner between typed Furey modules. |
| Anomaly inheritance | Once ideal states map to one-generation multiplets, existing anomaly theorems can be reused. |
| Prediction gate | A real prediction begins only if the allowed intertwiner space has smaller rank/codimension than generic SM Yukawa data. |
| Gate C | Furey projectors may constrain the internal part of the retained physical sector, but they do not prove Krein positivity, ghost safety, or nodal-set control. |

The right near-term theorem target is therefore not "Furey derives all masses."
It is:
...
PhiHFromFureyIdeals:
  ...
  feed the result into the existing finite super-Dirac square.
```

### 36.7 Definitions to add to the Lean roadmap

The Pro note recommends making the following concepts explicit Lean/API targets:

| Proposed name | Purpose |
|---|---|
| `OctonionLeftActionAlgebra` | Associative algebra generated by complex-octonion left multiplication operators. |
| `FureyIdealModule` | The minimal/cyclic module `J = A_F p` for a primitive idempotent. |
| `PrimitiveIdempotentReference` | Edge-local internal reference idempotent/frame, not a global physical vacuum. |
| `FureyConjugateIdeal` | Charge-conjugate or all-left module data, kept distinct from duals. |
| `FureyDualIdeal` | Incoming/outgoing dual and evaluation/coevaluation data. |
| `OccupationNumberParityChiE` | Internal grading `chi_E = (-1)^N`, with compatibility theorems. |
| `OrientedEdgeInternalFiber` | Assignment of Furey module data to oriented edge or half-edge states. |
| `NullEdgeFureyBundle` | Null-edge graph with external edge data plus Furey internal fibers and transports. |
| `NodeIntertwiner` | Gauge-equivariant, parity-typed node maps between incident fibers. |
| `GaugeCovariantYukawaFlip` | Higgs-dressed legal maps between left/right or `J`/`J*` sectors. |
| `PhiHFromFureyIdeals` | Concrete internal finite Dirac/Yukawa block used by the product operator. |
| `IntertwinerCodimensionAudit` | Test whether Furey-compatible maps restrict generic Yukawa freedom. |
| `PreservesFureyIdealStructure` | Safety predicate for gauge actions, Higgs maps, branch projectors, and ghost-prone operators. |

### 36.8 No-go tests and publication guardrails

No-go tests:

```text
raw octonion node products are rejected unless lifted to operator expressions;
edge reversal does not mean antiparticle unless the charge table proves it;
chi_E is not Gamma_s;
Krasnov/Furey complex structure is not chi_E unless a theorem proves it;
Furey does not predict Yukawa values if the coefficients remain free;
Furey does not close Gate C without projection, Krein positivity, ghost safety,
  and nodal-set control.
```

Publication-safe wording:

```text
We use the Furey complex-octonionic construction as a candidate finite internal
fiber for the null-edge theory. More precisely, the particle labels are modeled
as modules for the associative algebra generated by complex-octonion left
multiplication operators, rather than as naive ideals of the nonassociative
octonions themselves. An oriented null edge carries a propagating external
null/spin state valued in this finite internal module, while nodes carry
gauge-compatible intertwiners between incident edge fibers. In this
interpretation, the finite Higgs/Yukawa operator is an internal, chi_E-odd and
spacetime-Gamma_s-even map between Furey ideal and conjugate-ideal sectors, so
that the product operator D = i D_N tensor 1 + Gamma_s tensor Phi_H has the
expected +Phi_H^2 mass block.
```

Boundary:

```text
This supplies a structured internal-spectrum and Yukawa-legality framework. It
does not by itself derive Yukawa values, close Gate C, prove ghost safety, or
prove the full Standard Model spectrum.
```


## 37. Claude calibration: Furey mostly docked; Gate C nodal control is binding

Provenance: Claude analysis supplied in attachment
`dd8b5295-8531-402c-b340-c20a8701799b/pasted-text.txt`, responding to the
Section 36 Furey-fiber synthesis.

### 37.1 Calibration

Claude's most useful correction is that the Furey-fiber program is not merely a
future bridge. Much of the bridge described in Section 36 is already present as
kernel-checked Lean:

| Bridge target | Repo status reported by Claude |
|---|---|
| Furey ideal states realize the null-edge internal spectrum | `NullEdgeFureyInternalSpectrum.fureyJ_realizes_nullEdgeInternalSpectrum` computes charges from the Furey number operator on the eight ideal states. |
| Anomaly inheritance | `fureyJ_anomalyFree`, `fureyJ_localAnomalyFree`, and `fureyJ_wittenAnomalyFree` inherit the anomaly package. |
| Lawful `Phi_H` interface | `NullEdgeFureyPhiH` supplies gauge covariance, `chi_E` oddness, `Gamma_s` evenness, square/sign lemmas, and packaged properness predicates. |
| Almost-commutative product | `NullEdgeFureyAlmostCommutativeProduct.product_square_eq_gateA` feeds the Furey block into the Gate A square. |
| Occupation-parity `chi_E` | `NullEdgeFureyOccupationParityChiE` realizes `chi_E = (-1)^N` on the occupation model. |
| Associative action-algebra framing | Existing Furey modules and `Phi_H` are already phrased through module/endormorphism structures, not raw unparenthesized octonion multiplication. |

Thus Section 36 should be read partly as a documentation/convention lock and
partly as a prompt for remaining bridge cleanup. It should not cause us to
resubmit proof jobs that already exist unless the goal is promotion, assumption
audit, or theorem unification.

### 37.2 Two independent blockers

The program now has two separate bottleneck tracks.

Track A is the Furey/internal prediction gate:

```text
Does the Furey-compatible legal-intertwiner space have smaller dimension or
codimension than generic Standard Model Yukawa freedom?
```

The decisive test is:

```text
dim Hom_G(L tensor H, R) < generic SM Yukawa count?
```

This is the FUR-H10-style question. If the dimension is smaller, the program
gets structural prediction language: forbidden entries, forced ranks, or
restricted textures. If the dimension is equal, Furey remains a strong
charge-correct reconstruction and lawful mass-block framework, but not a
Yukawa-value or texture prediction.

Secondary Track A gaps:

```text
J* / right-handed-singlet completion remains a bounded formalization target.
Three generations remain replication/scaffolding only, not a derived theorem.
```

Track B is the Gate C kinetic/doubling gate:

```text
Do the projected, physical null-edge fermions avoid uncontrolled determinant
zeros, ghost zeros, and unphysical doublers?
```

This is currently the binding blocker. C64 shows the bare symbol has an
off-branch determinant zero

```text
q_star = (2*pi/3, 0, 0, 4*pi/3)
```

which is not on the four certified branch lines and is not gapped by the natural
`g5` species split `M_split = r*T_lin`. Therefore the existing `T_lin` split
controls the four branch curves but does not control the full determinant-zero
locus.

### 37.3 Gate C path after the off-branch zero

There are now three viable Gate C routes.

Route B1, recommended first:

```text
Classify the full determinant-zero locus instead of discovering off-branch
zeros one at a time.
```

The cube-root witness suggests an arithmetic/cyclotomic structure on the
Brillouin torus. The desired theorem shape is:

```text
mink(pCov(phaseU q)) = 0 iff q lies in an explicit cyclotomic/nodal family.
```

Yumoto-Misumi's spectral-graph/DFT method remains the main literature guide for
this task. Until the full zero locus is classified, no splitting term can be
claimed to gap every unwanted branch.

Route B2:

```text
Construct a richer species-splitting term sensitive to the cyclotomic/off-branch
zeros.
```

The current `T_lin` is too weak. A successor may need second-neighbour or
cross-term data such as functions of `cos(q_a - q_b)`. The target is:
...
and then the same test for the full classified zero locus. Coefficients of such
terms remain moduli unless a separate symmetry/minimality theorem fixes them.

Route B3:

```text
Release only on the projected physical sector.
```

This route does not require gapping every bare determinant zero, but it does
require a revised release certificate whose nodal-control clause is explicitly
about the projected sector. The required theorem is then:

```text
all off-branch zeros are either removed by the projection/composite observable
or land in the discarded Krein-negative/ghost sector.
```

This is a possible honest fallback, but it must not be sold as bare-symbol
nodal control.

### 37.4 Cheap safety theorem: involution distinctness

The Section 36 warning table should be consolidated into one small Lean module:

```text
PhysicsSM/Draft/NullEdgeInvolutionDistinctness.lean
```

The goal is not a new physics result. The goal is to collect theorem-level
non-identification witnesses showing that the following structures are distinct
unless a later theorem explicitly relates them:
...
Existing ingredients include results like `gammaS_internal_ne_chiE` and the
occupation-parity result that `chi_E` is not the Furey/Krasnov complex
structure. This consolidation is cheap and valuable because it prevents the
most tempting but most dangerous reviewer/agent conflations.

### 37.5 Updated one-screen status

| Claim | Status |
|---|---|
| Charge/anomaly structure from the Furey occupation model | Strong, kernel-checked. |
| Lawful `Phi_H` inside the product Dirac square | Strong, kernel-checked. |
| Occupation-parity `chi_E` | Strong, kernel-checked. |
| Furey legal intertwiners constrain `Phi_H` | Open; wait for the FUR-H10-style moduli audit. |
| Yukawa values, hierarchy, and mixing | Open; not claimed. |
| Three generations forced | Open; replication/scaffolding only. |
| Gate C no-doubling / no uncontrolled zeros | Blocked by C64 off-branch zeros until Route B1, B2, or B3 succeeds. |

Priority update:

```text
Do not spend the next serious wave proving generic Furey bridge statements that
already exist. Prioritize Route B1 full nodal-set classification, with Route B3
projected-sector release as the honest fallback. Run Track A prediction claims
only after the FUR-H10-style intertwiner audit returns.
```
## 38. Furey/Baez sharpening after source ingest - 2026-06-27

Provenance: follow-up analysis supplied in attachment
`81a90b3d-0fff-4d16-9783-12ccb41c01e3/pasted-text.txt`, plus Codex
Zotero/Neo4j ingestion on 2026-06-27.

### 38.1 Verdict

This analysis is mostly confirmation, not a new redirection. It reinforces the
current split:

```text
Furey/Baez/DVT = finite internal algebraic half.
Null-edge      = external kinetic, branch, ghost, Krein, and continuum gates.
```

Use Furey/Baez aggressively for Gate H and Gate F, cautiously for Gate C.
They can supply internal states, charge/gauge data, anomaly inheritance,
internal grading, legal `Phi_H` maps, and possible texture constraints. They do
not solve the determinant-zero/nodal-locus problem, Krein positivity,
ghost-zero safety, DEC/continuum convergence, QCD confinement, or numerical
Yukawa values.

The almost-commutative product remains the clean docking architecture:
...
### 38.2 New useful sharpening

The main operational improvement is the sharper prediction test:

```text
FureyBaezIntertwinerCodimensionAudit
```

Instead of asking vaguely whether Furey/Baez `explains masses`, ask whether the
actual Furey/Baez/DVT internal module restricts the legal Higgs/Yukawa map
space:

```text
dim Hom_G(L tensor H, R)
  versus
ordinary Standard Model Yukawa freedom.
```

If the allowed `Phi_H`/Yukawa intertwiner space has smaller dimension or a
nontrivial codimension relation, this becomes structural prediction language:
forbidden entries, forced ranks, restricted textures, or selected blocks. If the
dimension is the same as generic SM Yukawa freedom, the result remains a strong
charge-correct reconstruction, not a prediction.

A second useful target is:

```text
FureyBaezEMStabilizer_eq_EWPhotonStabilizer
```

Compare the electromagnetic direction selected internally by the Furey/Baez/DVT
algebra with the finite electroweak orbit-stiffness kernel:

```text
ker(B_EW : X -> X H_0) = u(1)_em.
```

If these match, photon survival is jointly selected by the internal algebra and
by the Higgs stabilizer theorem. This is structural, not numerical mass
prediction.

### 38.3 Convention guardrail

Any Furey bridge theorem must use the associative left-action algebra generated
by complex-octonion left multiplication operators. Do not treat particle states
as naive raw octonion ideals under nonassociative multiplication.

Operational rule:

```text
Furey states = modules/minimal ideals for an associative operator algebra.
Not raw octonion ideals with implicit associativity.
```

This guardrail belongs in every Furey-to-null-edge Aristotle job.

### 38.4 Right-handed-sector priority

The analysis reinforces that the useful completion theorem is not another broad
Furey philosophy job. It is:

```text
FureyRightHandedSectorOpen -> closed by a concrete J* / conjugate-sector theorem.
```

Target:

```text
formalize J* or the appropriate conjugate ideal sector;
prove it supplies the missing right-handed singlets under the all-left Weyl
convention;
connect it to the existing NullEdgeInternalSpectrum and anomaly-inheritance API.
```

Only after this bridge is explicit should the program treat the Furey internal
fiber as fully docked to the null-edge product operator.

### 38.5 Source ingest result

The following papers were added to Zotero and Neo4j, with embeddings generated:

| arXiv ID | Zotero key | Role |
| --- | --- | --- |
| `2606.15235` | `DJRMU4CB` | Baez-Schwahn exceptional-Jordan construction of the SM gauge group; source anchor for Furey/Baez gauge-stabilizer comparison. |
| `2209.13016` | `6VI58VGH` | Furey-Hughes one Weyl generation in `R tensor C tensor H tensor O`; source anchor for internal-spectrum docking. |
| `1806.00612` | `Z232K7IU` | Furey ladder-operator symmetry source for SM gauge-group structure and blocked transitions. |
| `0904.1556` | `MZZEZUWA` | Baez-Huerta representation-theoretic GUT/SM scaffold. |
| `math/0105155` | `WRIM6ZI7` | Baez octonion survey; convention/background anchor for octonions, Clifford algebras, spinors, and exceptional structures. |

Already present in Neo4j during the dry run:

| arXiv ID | Existing key | Title |
| --- | --- | --- |
...
Do not cite `1810.10465` as a Furey source without rechecking: in the ingestion
dry run it resolved to an unrelated deep-learning paper.

### 38.6 What this changes operationally

New or promoted next jobs:

```text
1. FureyBaezIntertwinerCodimensionAudit.
2. FureyBaezEMStabilizerComparison.
3. FureyRightHandedSectorCompletion.
4. FureyLeftActionConventionBridge, if needed as a small API guardrail.
```

What it does not change:

```text
Gate C remains the binding external kinetic blocker.
Gate D remains a DEC/Hodge-Dirac and connection-Laplian continuum problem.
Furey/Baez do not derive Yukawa eigenvalues, mixing angles, v, lambda,
Lambda_QCD, or proton mass.
```
