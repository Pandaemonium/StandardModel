...
an optional Sphere-Packing-Lean-facing package plan: it transports
...
lattice of the exceptional root system $E_8$, as a half-integer lattice in
$\R^8$, or as the Construction~A lattice attached to the extended binary Hamming
code.  These descriptions are equivalent in the mathematical literature, but a
formal proof assistant asks for explicit maps, explicit normalizations, and
kernel-checked statements connecting them.
...
$\quad\quad$ with optional SPL route to $\Theta_{E_8} = E_4$ \\
$\quad\to$ Sphere-Packing-Lean E8 basis comparison
\end{tabular}
\end{center}

The contribution is complementary to Sphere-Packing-Lean.
Sphere-Packing-Lean formalizes the analytic proof that the E8 sphere packing
is optimal in dimension eight.  The present development supplies a finite and
combinatorial entrance to the same E8 object: it starts from the extended
Hamming code, builds the lattice by Construction~A, and identifies the
resulting object with the standard E8 data used elsewhere in the Lean
ecosystem.

The main Lean-facing theorem index for the local Hamming-to-E8 development is
\code{CodeLatticeE8.Publication.TheoremIndex}.  A reviewer-facing theorem map
and trust audit are maintained in
\code{Sources/CodeLatticeE8\_Publication\_Theorem\_Map.md} and
\code{Sources/CodeLatticeE8\_Trust\_Report.md}.  The polished theorem spine is
organized through the standalone package root
\code{CodeLatticeE8} and the optional SPL root \code{CodeLatticeE8SPL}.
The older draft files remain provenance and comparison material.  The theorems
cited below are named explicitly so that their status can be checked directly
in Lean.
...
\item The standalone package proves the semantic zero and first theta
      ...
      The all-coefficient analytic identity is completed in the optional
      SPL-facing root.
...
is the unique Type~II binary code of length eight.  It has parameters $[8,4,4]$:
...
coordinate-permutation equivalent to the chosen extended Hamming code.
...
unscaled integer model has minimal squared norm~$4$.  The usual E8 normalization
divides the quadratic form by~$2$, equivalently scaling all coordinates by
$1/\sqrt{2}$, so the standard E8 minimal squared norm is~$2$.
...
codes, and uniqueness up to code equivalence.
...
\code{CodeLatticeE8SPL} root.  The citation-facing clean theorem index is
...
normalized $E_4$ coefficients through $q^6$.  This is a clean standalone table
...
The analytic layer has a clean optional root, \code{CodeLatticeE8SPL}.  It
...
The determinant computation is also in the clean package:
\begin{lstlisting}[style=lean]
...
coordinates, the short shell is $\mathrm{sqNorm}(z) = 4$; in the scaled E8
normalization, this is squared norm~$2$.  The finite cardinality theorem is
proved by private finite parametrizations of the weight-zero and weight-four
Hamming classes.
...
presentation.  The congruence itself is proved by ordinary kernel reduction over
the 64 matrix entries.  The Cartan determinant is proved by nested cofactor
expansion, reducing the calculation to small $7\times 7$ kernel \lean{decide}
checks.
...
\subsection{Theta Coefficients and the Optional $\Theta_{E_8} = E_4$ Bridge}
...
The optional SPL-facing theorem package uses a semantic coefficient function
...
This avoids invoking a broad theorem saying that Type~II codes always give even
unimodular lattices.  Such a theorem would be elegant, and would be a good
...
Construction~A convolution theorem.  The optional SPL-facing layer completes
...
The optional theorem $\Theta_{E_8} = E_4$ identifies the theta series
...
MacWilliams--Sloane, Conway--Sloane, and later lattice and coding-theory texts.
The extended Hamming code is the smallest Type~II binary code, and
...
linear-programming method and Viazovska's construction.  The present work is
not a replacement for that analytic formalization.  Instead, it supplies a
formal combinatorial bridge to the E8 object used there.
...
$\F_2$.  Four parity equations determine a four-dimensional subspace of
$\F_2^8$.  The formal development proves:
\begin{enumerate}
...
\item every binary linear $[8,4,4]$ code is equivalent to it.
\end{enumerate}
...
form to the standard E8 Cartan form:
\[
...
\medskip
\noindent The total is $16 + 224 = 240$.
...
The clean package exposes two finite theta layers.
...
$\mathrm{sqNorm} \le 24$.  The weight multiplicities are:
\[
...
lake build CodeLatticeE8
\end{lstlisting}
For the optional SPL-facing theta route:
\begin{lstlisting}[style=shell]
lake build CodeLatticeE8SPL
\end{lstlisting}
For each theorem cited in Appendix~\ref{app:theorems}, record:
\begin{enumerate}
\item the Lean declaration name;
\item the file;
\item whether the containing module builds under the pinned toolchain;
\item the axiom report;
\item whether the theorem is part of the standalone root or the SPL-facing root.
\end{enumerate}
The final paper should include only claims backed by checked theorem
declarations and should keep convention-sensitive statements tied to their Lean
names.

The current paper-spine axiom audit should be refreshed after the SPL
promotion.  The expected trusted profile is the standard Lean/Mathlib axiom set
$\{\texttt{propext},\; \texttt{Classical.choice},\; \texttt{Quot.sound}\}$,
with \lean{Lean.ofReduceBool} expected for declarations whose proofs contain
ordinary kernel-checked \lean{decide} steps.  The detailed command list is
recorded in \lean{Sources/CodeLatticeE8\_Trust\_Report.md}.

\section{Bibliography Checklist}
\label{app:bibcheck}

The bibliography should include at least:
\begin{itemize}
\item Hamming, ``Error Detecting and Error Correcting Codes,'' 1950.
\item MacWilliams and Sloane, \textit{The Theory of Error-Correcting Codes},
      North-Holland, 1977.
\item Huffman and Pless, \textit{Fundamentals of Error-Correcting Codes},
      Cambridge University Press, 2003.
\item Conway and Sloane, \textit{Sphere Packings, Lattices and Groups},
      Springer, 3rd ed., 1998.
\item Bourbaki, \textit{Lie Groups and Lie Algebras}, Chapters 4--6,
      Springer, 2002.
\item Cohn and Elkies, ``New upper bounds on sphere packings~I,''
      \textit{Ann.\ of Math.}\ 157 (2003), 689--714.
\item Viazovska, ``The sphere packing problem in dimension 8,''
      \textit{Ann.\ of Math.}\ 185 (2017), 991--1015.
\item Hariharan, Birkbeck, Lee, Ma, Mehta, Poiroux, and Viazovska,
      ``A Milestone in Formalization: The Sphere Packing Problem in
      Dimension~8,'' arXiv:2604.23468 (2025).
\item The Sphere-Packing-Lean project:
      \url{https://thefundamentaltheor3m.github.io/Sphere-Packing-Lean/}.
\item Error Correction Zoo, ``$[8,4,4]$ extended Hamming code,''
      \url{https://errorcorrectionzoo.org/c/hamming844}.
\item Nebe and Sloane lattice catalogue.
\end{itemize}
Exact page, chapter, theorem, and blueprint-section references should be filled
in during the final bibliography pass before submission.
