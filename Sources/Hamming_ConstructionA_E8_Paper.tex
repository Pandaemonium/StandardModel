...
also gives the standard E8 metric and octavian data: minimum squared norm~$2$,
exactly $240$ minimal vectors, an explicit bridge to an independently
formalized E8 root list, an octavian multiplication closure theorem for the
$240$-unit shell, and a Cartan-matrix comparison.

The formalization also connects the coding-theoretic model to the theta-series
description of E8.  The core \code{CodeLatticeE8} development proves the
...
coefficients through $q^6$.  An optional Sphere-Packing-Lean-facing layer
transports Construction~A shell counts to the rank-eight theta modular form and
compares that modular form with the normalized Eisenstein series $E_4$; in that
layer this gives the classical identity
\[
  \Theta_{E_8}(q) = E_4(q) = 1 + 240\sum_{n>0}\sigma_3(n)\,q^n.
\]
Here \(\Theta_{E_8}\) is the theta series of the E8 lattice, \(E_4\) is the
normalized Eisenstein series of weight~\(4\), \(q\) is the formal theta-series
variable, and \(\sigma_3(n)=\sum_{d\mid n} d^3\) is the sum of cubes of the
positive divisors of~\(n\).
The development also compares the local
...
Coxeter integral octonions~\cite{Coxeter1946,Baez2002}, or as the
...
$\quad\to$ E8 root, octavian multiplication, and Cartan data \\
...
The contribution is complementary to the Sphere-Packing-Lean project~\cite{SPL2025}.
Sphere-Packing-Lean has formalized the analytic proof that the E8 sphere packing
is optimal in dimension eight~\cite{Viazovska2017}.  The present development
supplies a finite and combinatorial entry point to the same E8 object: it starts
...
organized through the core root \code{CodeLatticeE8} and the optional
Sphere-Packing-Lean (SPL) root \code{CodeLatticeE8SPL}.
...
\item In the Coxeter-octavian normalization, the same Hamming Construction~A
      order is closed under the normalized Cayley--Dickson product, and its
      $240$ norm-one units are closed under multiplication.
...
\item The core development proves the semantic zero and first theta
...
Viazovska's analytic sphere-packing theorem enters through the existing
Sphere-Packing-Lean formalization, and the construction avoids appealing to the
abstract classification theorem for even unimodular positive-definite lattices
of rank eight.  The development works concretely: basis matrices, Gram
matrices, explicit shell counts, explicit roots, and explicit transition maps
carry the construction from the code to E8.
...
\]
Here \(W_C\) is the homogeneous weight enumerator
\(\sum_{c\in C} x^{8-\mathrm{wt}(c)}y^{\mathrm{wt}(c)}\); the variables \(x\)
and \(y\) mark zero and one coordinates, respectively, and
\(\mathrm{wt}(c)\) is the Hamming weight of the codeword~\(c\).
...
The main code-theoretic Lean declarations are:
...
\end{lstlisting}
The first records the $[8,4,4]$ parameters, the second records the doubly-even
...
\]
Here \(A(C)\) is the Construction~A integer lattice attached to \(C\).
...
It then relates the short-vector shell to an independent doubled-coordinate
root-list model in the repository.  This model has the usual octonionic
reading: after dividing every coordinate by two and choosing the fixed
octonion basis, the two root families are
\[
  \pm e_i \pm e_j \quad (i<j)
  \qquad\text{and}\qquad
  \frac12(\pm e_0 \pm e_1 \pm \cdots \pm e_7),
\]
Here \(e_0,\ldots,e_7\) are the fixed octonion basis vectors, with \(e_0\) the
real unit; \(i\) and \(j\) range over basis labels; and the half-integral sign
vectors satisfy Coxeter's parity condition~\cite{Coxeter1946,Baez2002}.  Thus
the independent root-list comparison also identifies the code-built short shell
with the classical integral-octonion presentation of the E8 roots.  The
octavian multiplication layer uses the same coordinate convention to define the
Cayley--Dickson product and prove the corresponding order and unit-shell
closure statements.
...
\]
Here \(q\) is the formal theta-series variable, \(v\) ranges over E8 lattice
vectors, and \(\|v\|^2\) is the squared norm in the standard E8 normalization.
The exponent \(\|v\|^2/2\) makes roots of squared norm~\(2\) contribute to the
coefficient of~\(q\).
For the standard E8 lattice, the classical formula is~\cite{ConwaySloane1999}:
\[
  \Theta_{E_8}(q) = E_4(q) = 1 + 240\sum_{n>0}\sigma_3(n)\,q^n.
\]
Here \(E_4\) is the normalized weight-four Eisenstein series and
\(\sigma_3(n)=\sum_{d\mid n} d^3\).  Thus the coefficient of \(q^n\), for
\(n>0\), is \(240\) times the sum of cubes of the divisors of~\(n\).
...
doubly-even codes, the general Type~II scaled evenness and self-duality theorem,
...
reflection layers are part of the core package.  Direct Sphere-Packing-Lean
comparisons belong under the optional \code{CodeLatticeE8SPL} root.  The theorem
index is \code{CodeLatticeE8/Publication/TheoremIndex.lean}.

\subsection{Octavian Multiplication Layer}

The octavian layer fixes a Cayley--Dickson signed-XOR basis convention and
connects it to the Hamming Construction~A order:
\begin{lstlisting}[style=lean]
CodeLatticeE8/Octonion/CayleyDickson.lean
CodeLatticeE8/Octonion/Basic.lean
CodeLatticeE8/Octonion/Octavian.lean
\end{lstlisting}
The file \code{Octavian.lean} defines the normalized doubled-coordinate product
\lean{octavianUnitMul}.  If integer vectors \lean{x} and \lean{y} represent
octonions \lean{x / 2} and \lean{y / 2}, then
\lean{octavianUnitMul x y} represents their product in the same doubled
coordinates.  The closure proof reduces arbitrary products to bilinearity of
\lean{mulInt}, the explicit Construction~A basis, and an $8\times 8$
basis-product certificate.

\subsection{Theta-Series Layer}

The theta-series material is split into a core finite layer and an
analytic/Sphere-Packing-Lean layer.

The core development records the coefficient normalization and the finite
...
normalized $E_4$ coefficients through $q^6$.  The semantic shell-count theorem
and the analytic identity are recorded separately.

The core development also contains the all-shell Construction~A convolution
theorem:
\begin{lstlisting}[style=lean]
...
The bridge to Sphere-Packing-Lean is given by explicit matrices and linear
maps.  The local development reproduces the E8 basis matrix and compares Gram
forms.  The imported bridge then gives a coordinate map from the local
Construction~A model to
$\code{Submodule.E8}\;R$.
...
\end{lstlisting}
and the doubly-even theorem used directly by Construction~A is:
\begin{lstlisting}[style=lean]
...
\end{lstlisting}
This says that the chosen parity-check representation represents, up to
coordinate permutation, the unique binary linear code with parameters
$[8,4,4]$~\cite{MacSloane1977,ConwaySloane1999}.
...
\code{hammingConstructionBasis}.  The spanning theorem is:
\begin{lstlisting}[style=lean]
...
The determinant computation is formalized as:
\begin{lstlisting}[style=lean]
...
\end{lstlisting}
These are proved from the explicit basis matrix.  The basis matrix is block
upper triangular with a $4\times 4$ determinant~$-1$ block and a $2I_4$ block,
so its determinant is~$-16$; the raw Gram determinant is its square, and the
...
\end{lstlisting}
these results provide the integral basis, Gram, determinant, and evenness data.
...
theorem uses explicit parametrizations of the weight-zero and weight-four
Hamming classes.
...
This root-list model is also the octonionic model used in the paper.  A vector
\lean{v : Fin 8 -> Z} represents the octonion with coordinates
$v_i/2$ in the fixed basis.  The predicate \lean{IsE8Root} says exactly that
the doubled vector has squared norm~$8$, has uniform coordinate parity, and has
coordinate sum divisible by~$4$.  Equivalently, the represented octonion has
squared norm~$2$ and lies in the $240$-element Coxeter root shell.  In this
language, \lean{shortShell\_perm\_rootList} identifies three presentations of
the same finite object: Hamming Construction~A short vectors, the doubled
coordinate E8 root list, and the Coxeter integral-octonion roots.

The same coordinate convention supports the corresponding octavian
multiplication layer.  In the Hamming short-shell normalization, a vector
\lean{x : Fin 8 -> Z} represents the octonion \lean{x / 2}; vectors of
\lean{sqNorm x = 4} therefore represent norm-one octavian units.  The raw
Cayley--Dickson coordinate product is \lean{mulInt}; the normalized
doubled-coordinate product is \lean{octavianUnitMul x y = mulInt x y / 2}.
Exact divisibility by~$2$ is proved for all elements of the Construction~A
order:
\begin{lstlisting}[style=lean]
theorem two_dvd_mulInt_of_hammingConstructionA {x y : Coords Z}
    (hx : x in hammingConstructionA) (hy : y in hammingConstructionA)
    (k : Fin 8) :
    2 divides mulInt x y k
\end{lstlisting}
The order closure and $240$-unit closure theorems are:
\begin{lstlisting}[style=lean]
theorem octavianUnitMul_mem_hammingConstructionA {x y : Coords Z}
    (hx : x in hammingConstructionA) (hy : y in hammingConstructionA) :
    octavianUnitMul x y in hammingConstructionA

theorem octavianUnitMul_mem_shortShell {x y : Coords Z}
    (hx : x in hammingE8ShortShell) (hy : y in hammingE8ShortShell) :
    octavianUnitMul x y in hammingE8ShortShell
\end{lstlisting}
The proof expands arbitrary Construction~A vectors in the explicit eight-vector
basis, uses bilinearity of \lean{mulInt}, checks the $8\times 8$ basis products,
and then uses \lean{normSqInt\_mulInt} to recover the unit-shell norm.
...
root-system presentation.  The congruence itself is proved by kernel-checked
...
The formalization also records the first Weyl-theoretic closure property of
...
\]
Here \(r\) is the reflecting root, \(v\) is another doubled-coordinate root, and
\(\langle v,r\rangle\) is the integer dot product in doubled root-list
coordinates.  The factor \(1/4\) appears because doubled roots satisfy
\(\langle r,r\rangle=8\), so \(2/\langle r,r\rangle=1/4\) in the usual
reflection formula.
The theorem
\begin{lstlisting}[style=lean]
...
The core development organizes the theta comparison in two finite layers.  First,
it proves the semantic shell statements:
\begin{lstlisting}[style=lean]
...
Second, the core development records a finite Hamming weight-contribution table and
proves:
\begin{lstlisting}[style=lean]
...
\end{lstlisting}
This is a finite table theorem.  The all-shell Construction~A convolution
theorem \lean{CodeLatticeE8.E8.thetaShellCount\_eq\_convolution} gives the
semantic shell-count statement; the SPL-facing layer then identifies that
semantic coefficient function with $E_4$ for every~$n$.

The SPL-facing layer uses a semantic coefficient function
...
Beyond matching determinant and root-count data, the bridge supplies concrete
maps between coordinate models and proves the corresponding inner-product
formula.  Thus the Hamming Construction~A lattice is connected directly to the
E8 object used by the formalized analytic sphere-packing proof.
...
The formalization also includes the corresponding general Type~II
...
Thus the formal library records the standard abstraction that Type~II codes
give even self-dual, hence unimodular, lattices after the conventional
$1/\sqrt{2}$ scaling~\cite{MacSloane1977,ConwaySloane1999}.  The rank-eight
basis, determinant, and Cartan comparisons remain explicit because they provide
the coordinate bridges identifying the Hamming Construction~A model with the E8
lattice used elsewhere in the formalization.
...
\[
  16 + 14 \times 16 = 240.
\]
Here the first \(16\) counts the coordinate-spike vectors, \(14\) is the number
of weight-four codewords in the extended Hamming code, and the final factor
\(16=2^4\) counts sign choices on the four supported coordinates.
The Lean proof realizes this classification as an explicit finite list and
proves both soundness and completeness for the short-vector predicate.
...
The core development proves the semantic $q^0$ and $q^1$ coefficients, the finite
Hamming contribution table through $q^6$, and the all-shell
...
\]
Here \(\mathrm{sqNorm}_{\mathrm{CA}}\) is the unscaled integer Construction~A
squared norm, \(\phi\) is the bridge map into the standard E8/SPL coordinate
model, and \(n\) is the theta coefficient index.
...
The theorem $\Theta_{E_8} = E_4$ identifies the theta series abstractly, while
the sphere-packing endpoint is phrased for the E8 model used inside
Sphere-Packing-Lean.  The explicit matrix and lattice bridges connect the
code-built lattice and the SPL lattice by named Lean maps and inner-product
statements.
...
this paper are checked by building the relevant Lean modules under the pinned
toolchain.
...
\end{lstlisting}
The core root \code{CodeLatticeE8} is independent of Sphere-Packing-Lean.  The
standalone Lake wrapper checks this layer without resolving the optional SPL
dependency:
\begin{lstlisting}[style=shell]
...
verification checklist, including axiom reports for the theorem declarations
that form the main theorem spine.
...
Coxeter's integral Cayley numbers~\cite{Coxeter1946} and Baez's survey of the
octonions~\cite{Baez2002} provide the octonionic background for the same
$240$-element shell.  The present formalization uses that classical description
in two ways: first through the doubled-coordinate root predicate and root-list
comparison, and second through a Cayley--Dickson coordinate product proving
closure of the Hamming Construction~A octavian order and of its $240$ norm-one
units.
...
construction~\cite{Viazovska2017}.  The present work complements that analytic
formalization by supplying a formal combinatorial bridge to the E8 object used
there.
...
serves as an entry point to E8: from a parity-check matrix over $\F_2$, Lean
...
also provides reusable infrastructure for further work: Golay-to-Leech
...
\[
  \mathrm{sqNorm}(z) = 4n.
\]
Here \(z\) ranges over Construction~A integer vectors, \(\mathrm{sqNorm}\) is
the unscaled coordinate squared norm, and \(n\) is the theta-series coefficient
index.
...
\]
Here \(\phi\) is the explicit map from the Construction~A integer model to the
standard E8/Sphere-Packing-Lean model; the right-hand norm is the standard E8
squared norm.
...
E8/octonionic root-list characterization &
  \lean{...Roots.mem\_rootList\_iff\_isE8Root} &
  \lean{E8/Roots.lean} \\[2pt]
Construction~A to root-list bridge &
  \lean{...RootBridge.shortShell\_perm\_rootList} &
  \lean{E8/RootBridge.lean} \\[2pt]
Octavian order product has even raw coordinates &
  \lean{...Octonion.two\_dvd\_mulInt\_of\_hammingConstructionA} &
  \lean{Octonion/Octavian.lean} \\[2pt]
Octavian order closure &
  \lean{...Octonion.octavianUnitMul\_mem\_hammingConstructionA} &
  \lean{Octonion/Octavian.lean} \\[2pt]
Octavian $240$-unit closure &
  \lean{...Octonion.octavianUnitMul\_mem\_shortShell} &
  \lean{Octonion/Octavian.lean} \\[2pt]
Cartan bridge &
...
The main theorem spine is free of \lean{native\_decide} and
\lean{Lean.trustCompiler}.  The octavian closure theorems use kernel-checked
finite certificates for the displayed basis products.
...
\[
  W_C(x,y) = x^8 + 14x^4y^4 + y^8.
\]
Here \(W_C(x,y)=\sum_{c\in C}x^{8-\mathrm{wt}(c)}y^{\mathrm{wt}(c)}\), so the
coefficient \(14\) records the fourteen codewords of Hamming weight~\(4\).
...
\]
Here \(C_{\mathrm{Hamming}}\) is the fixed extended Hamming code and
\(z\bmod 2\) means coordinatewise reduction of \(z\in\Z^8\) to
\((\Z/2\Z)^8\).
The formalization gives an explicit basis \lean{hammingConstructionBasis}.
...
\]
Here \(G_{ij}\) is the \((i,j)\)-entry of the Gram matrix, \(b_i\) is the
\(i\)-th basis vector, and \(k\) ranges over the eight coordinate positions.

Because the E8 normalization divides the quadratic form by~$2$, the
formalization defines the rational scaled Gram matrix
\lean{hammingConstructionScaledGram} and proves $\tilde{G}_{ii} = 2$ for every
diagonal entry.

The determinant proof is packaged through a block-upper-triangular decomposition
of the basis matrix:
\[
  \det(B) = -16, \quad \det(G) = (-16)^2 = 256, \quad
  \det(\tilde{G}) = 2^{-8}\cdot 256 = 1.
\]
Here \(B\) is the displayed Construction~A basis matrix, \(G\) is the unscaled
Gram matrix, and \(\tilde{G}\) is the scaled Gram matrix obtained by dividing
the quadratic form by~\(2\) in rank~\(8\).
...
\]
Here \(P\) is the explicit change-of-basis matrix, \(G\) is the Construction~A
Gram matrix, and \(\mathrm{Cartan}_{E_8}\) is the standard E8 Cartan matrix in
the chosen Bourbaki ordering.
...
\]
Here \(A(C)\) is the Construction~A lattice for the extended Hamming code and
\(\mathrm{sqNorm}\) is the unscaled integer squared norm.  The value~\(4\)
corresponds to squared norm~\(2\) after the standard E8 scaling.
...
The Lean development proves this count, the completeness of the explicit
short-vector list, and the map to the repository's E8 root list.
...
\]
Here \(n\) is the theta-series index and the factor~\(4\) is the unscaled
Construction~A normalization corresponding to standard E8 squared norm~\(2n\).

The formalization has two finite theta layers.
...
\]
Here \(A_w\) denotes the number of codewords of Hamming weight~\(w\) in the
...
\]
Here \(\text{coeff}_n\) is the coefficient of \(q^n\) in the theta series and
...
audit.  The expected axiom profile for the main theorem declarations is the
standard Lean/Mathlib set
\[
  \{\texttt{propext},\; \texttt{Classical.choice},\; \texttt{Quot.sound}\}.
\]
Here \texttt{propext} is propositional extensionality, \texttt{Classical.choice}
is Lean's classical choice principle, and \texttt{Quot.sound} is the standard
quotient soundness axiom used throughout Mathlib.
...
\item Bourbaki~\cite{Bourbaki2002} is used for the E8 root-system and Cartan
      matrix conventions.
\item Coxeter~\cite{Coxeter1946} and Baez~\cite{Baez2002} are used for the
      integral-octonion interpretation of the doubled-coordinate E8 root list
      and for the octavian-order/unit-shell multiplication statement.
...
\bibitem{Coxeter1946}
H.~S.~M. Coxeter.
\newblock Integral {C}ayley numbers.
\newblock \textit{Duke Mathematical Journal}, 13:561--578, 1946.

\bibitem{Baez2002}
J.~C. Baez.
\newblock The octonions.
\newblock \textit{Bulletin of the American Mathematical Society},
  39(2):145--205, 2002.
