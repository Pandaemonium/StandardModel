...
Lorentz-invariance leg uses the canonical homogeneous Poisson sprinkling, whose law is
Poincare invariant and from which Bombelli--Henson--Sorkin prove that no preferred finite
direction set or finite-valency graph can be extracted equivariantly \Import; this gives the
standard \emph{kinematic sprinkling} count $\alpha=1$.  Their theorem does not classify all
Lorentz-invariant point-process laws and therefore does not, by itself, exclude a Lorentz-invariant
hyperuniform alternative.  The \emph{mathematical} dichotomy is now \Kernel{} (\lm{LambdaFermionicFork},
