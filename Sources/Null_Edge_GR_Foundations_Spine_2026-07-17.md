...
| F8: one gravity action | One graph action converges to Einstein-Hilbert plus boundary and controlled corrections | Exact affine-action no-go, coframe determinant variation, Palatini-to-Einstein composition, closed-carrier incidence cancellation, exact regional cut flux and a conditional fixed-boundary interior action-to-Einstein theorem, nonlinear and joint two-field chart actions, spinor-null coframe and directed Levi-Civita/Ricci reconstruction, aggregate-weight coframe coverage, an exact independent pointwise-connection variation, its local periodic Euler coefficient and torsion-free Levi-Civita no-go, a group-valued link-curvature substrate, scalar, Euclidean finite-fiber, and full Krein-paired link/face Euler chains, plus a spacetime-derived six-component Lorentz-bivector representation preserved by the concrete null-edge `SL(2,C)` action and exactly equivalent to the matrix Lorentz Lie algebra with normalized trace pairing, the exact right-trivialized nonlinear Lorentz-plaquette tangent with its four-corner adjoint formula and additive identity-link limit, a displayed scalar ordered holonomy action with matching product/inverse derivative along canonical exponential link curves, its exact four-family nonidentity local link Euler coefficients, ordinary coframe derivative, joint `6 + 16` stationarity, exact antisymmetric-curvature Palatini rewrite, arbitrary-coframe identity `PalatiniDensity(e,F) = -det(e) R(e^{-1},F)`, exact coframe-response identity and stationarity-to-mixed-Einstein equivalence, finite Levi-Civita uniqueness and arbitrary-invertible-coframe affine first-jet selection with an identity-background actual-action corollary, an exact flat joint-stationarity witness and a nonflat actual-action vacuum-Weyl jet with zero link-Euler first derivative and zero coframe-Euler quadratic coefficient, conditional passage of stationary refinements with jointly convergent varying coframes and curvature to a limiting mixed vacuum Einstein equation, exact equivalence between the finite pointwise equations and all weak test-tensor pairings, the corresponding volume-weighted equivalence for nonzero site weights, a conditional changing-carrier weak-limit endpoint, the static square and full fixed-null-wave-connection joint-stationarity no-go theorems, determinant-weighted nonlinear action, and the complementary coframe-derived curvature-face coefficient `(1/2) epsilon^(cdab) star(e_c wedge e_d)` with exact divergence, proper-Lorentz covariance, and concrete action gauge invariance | Derive the aggregate weights and synchronized frame from the operator sector, identify the Gram and operator metrics, derive a graph refinement, continuum test sampler, and metric dual-cell weights, prove weak convergence of its Einstein residual and the supplied coframe convergence hypotheses, construct a nonflat jointly stationary refinement with simultaneous link/coframe backreaction, extend first-jet Levi-Civita selection to exact nonidentity-link finite spacing, identify the action-visible target with common Riemann curvature, construct the geometric boundary counterterm and identify its continuum limit, derive `G_N`/`Lambda`, global descent, and global variation-limit interchange |
...
neighbors. Zero connection and coframe jets now give an exact flat jointly
stationary family, so the capstone antecedent is inhabited rather than merely
formal.

`NonlinearLorentzPalatiniConcreteVacuumWeylJet` supplies the curved control on
the same carrier. Actual affine-exponential links remain proper eta-Lorentz and
have the nonzero unit vacuum-Weyl tensor as their exact plaquette quadratic
coefficient. Every actual link Euler coefficient has derivative zero, every
coframe Euler coefficient has zero constant, linear, and quadratic terms, and
the induced connection is Christoffel with vanishing standard mixed coordinate
Einstein tensor. The guarded bundle
`fourTorusUnitVacuumWeyl_actualActionJet` records these statements together.
This is a finite second-jet consistency theorem, not an exact nonflat stationary
branch or a refinement limit.

The remaining curvature-identification debt is graph-analytic:
...
`FinitePalatiniBoundaryFlux` removes the closed-carrier restriction at the
finite variational level. For every chosen interior, the summed incidence
divergence is exactly the oriented flux through edges crossing its cut. The
target-only side of the same two-vertex witness has cut flux `+1`, so the
boundary response is genuinely nonzero. If a boundary counterterm response
cancels this cut flux in every symmetric metric direction supported in the
interior, the derivative of the same decomposed action is the
volume-weighted Einstein-matter pairing there. With nonzero interior volumes,
ordinary fixed-boundary action stationarity is therefore equivalent to the
finite Einstein equation at every interior site. This is an exact finite
boundary identity and a conditional action theorem, not a construction of the
counterterm or an identification with the Gibbons--Hawking--York term.
...
connection/coframe pair. Explicit plus-like and cross-like coupled
link/coframe perturbations are jointly stationary, and every real linear
combination remains jointly stationary. Their additive curls are two
independent nonzero vacuum-Riemann curvature directions. The same module proves
...
match the curvature routes, construct the boundary counterterm whose variation
cancels the exact cut flux, identify its continuum boundary functional, and
establish variation-limit interchange.
...
4. **Complete the nonlinear Palatini-to-Einstein bridge.** The additive
   ...
   The abstract boundary algebra is now exact: regional divergence equals cut
   flux, and a canceling boundary response yields the interior Einstein
   equation under fixed-boundary variations. Derive that response from the
   concrete nonlinear Lorentz-plaquette action and test whether it cancels the
   compact-carrier quadratic obstruction on an open carrier.
