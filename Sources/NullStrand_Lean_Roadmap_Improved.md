...
## Claude calibration after Furey-fiber synthesis - 2026-06-26

Provenance: Claude analysis supplied in attachment
`dd8b5295-8531-402c-b340-c20a8701799b/pasted-text.txt`. Full integration is in
`Sources/Null_Edge_Unified_Mass_Model_Working_Plan.md` Section 37.

### Implementation correction

Several Furey bridge targets should now be treated as already substantially
formalized, not as speculative future work:

```text
Furey internal spectrum / charge table;
anomaly inheritance wrappers;
lawful Phi_H interface;
occupation-parity chi_E;
almost-commutative product square;
module/endormorphism formulation avoiding raw octonion ideal language.
```

Future Furey work should therefore focus on:

```text
1. J* / right-handed-sector completion;
2. the FUR-H10-style Yukawa/intertwiner moduli audit;
3. promotion/unification of scattered bridge theorems where useful;
4. explicit convention-distinctness witnesses.
```

Do not resubmit broad "construct the Furey product" jobs unless the job is
specifically about integration quality, assumptions, naming, or theorem
unification.

### Gate C is the binding blocker

C64 moves the no-doubling gate from "likely Route B" to a sharper blocker:

```text
there are off-branch determinant zeros, including a cube-root/cyclotomic
q_star, and the natural g5 split T_lin does not gap them.
```

Roadmap priority:

```text
1. full determinant-zero / nodal-locus classification;
2. richer split term only after the full locus is understood;
3. projected-sector release only if the release certificate is explicitly
   rewritten around projected zeros, not the bare determinant-zero locus.
```

The preferred next module is:
...
```text
mink(pCov(phaseU q)) = 0 iff q belongs to an explicit cyclotomic/nodal family.
```

### Convention-distinctness module

Add a small, high-value defensive module:

```text
PhysicsSM/Draft/NullEdgeInvolutionDistinctness.lean
```

It should consolidate theorem-level non-identification witnesses among:
...
This is not a new physics gate, but it protects every later super-Dirac,
Furey, and Gate C statement from accidental two-valued-structure collapse.
## Furey/Baez source-ingest refinement - 2026-06-27

The Furey/Baez angle is now best treated as a focused internal-sector program,
not a replacement for Gate C or Gate D.

Useful sharpenings:

```text
1. Promote the Furey/Baez prediction test to an explicit
   IntertwinerCodimensionAudit:
      dim Hom_G(L tensor H, R)
      versus generic Standard Model Yukawa freedom.

2. Add an electroweak/internal stabilizer comparison:
      Furey/Baez internal u(1)_EM
      equals
      ker(X -> X H_0) from the finite electroweak orbit-stiffness theorem.

3. Keep the left-action convention explicit:
      Furey modules are modules for the associative algebra generated by
      complex-octonion left multiplication operators, not raw octonion ideals.

4. Prioritize right-handed-sector completion through J* or conjugate-sector
   formalization before claiming the Furey internal fiber is fully docked.
```

New source anchors ingested into Zotero/Neo4j:

```text
2606.15235  Baez-Schwahn, Standard Model gauge group from exceptional Jordan algebra.
2209.13016  Furey-Hughes, one Weyl generation in R tensor C tensor H tensor O.
1806.00612  Furey, SM gauge symmetry from division-algebraic ladder operators.
0904.1556   Baez-Huerta, Algebra of Grand Unified Theories.
math/0105155 Baez, The Octonions.
```

Operational consequence:

```text
Furey/Baez support Gate H and possibly Gate F.
They do not release Gate C. Continue to treat full nodal control, projected
physical sector, Krein positivity, and ghost safety as independent external
kinetic obligations.
```
## Gate C v3 roadmap: null-Wilson/projected release target - 2026-06-27

Adopt the release-target change:

```text
Bare D_+ is the primitive null-edge kinetic seed, not the final released
physical operator.
```

Gate C should now aim to release a specified regulated/projected operator:

```text
D_phys = projected / overlap / domain-wall / point-split operator built from
D_+ plus a null-Wilson branch regulator.
```
...
```text
W(q) = 0 iff q = 0 mod 2*pi on the real Brillouin torus.
Every non-origin real determinant zero gets regulator cost.
Near the origin, R_W(hk) = O(h |k|^2), so the leading Dirac symbol survives.
```

Roadmap warning:

```text
This is a Wilson-like regulator/counterterm, not a prediction.
The coefficient r and operator placement must enter the moduli ledger unless
later forced.
```
...
```text
C70 NullEdgeNullWilsonRegulator:
  prove W zero/positivity facts and known-zero lifting.

C71 NullWilson operator-placement audit:
  decide whether Gamma_s R_W, scalar Wilson kernel, overlap kernel, or
  projected/domain-wall construction is the right physical target.

C72 Projected Gate C Wilson release API:
  state the v3 release theorem from regulated nodal control plus residue safety.

C73 Gauge-covariant null-Wilson link dressing:
  construct or audit R_W^A using T_a and T_a^sharp with gauge parallel transport.

C74 Post-gauge residue/ghost-safety plan:
  turn the Golterman-Shamir warning into a finite residue theorem target.
```
