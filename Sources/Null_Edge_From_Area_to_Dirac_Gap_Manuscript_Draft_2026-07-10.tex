...
constant-coefficient $1+1$ walk.  A finite four-mode quartic extension has
forward two-particle amplitude $z$ and conjugate reverse amplitude, so it
retains orientation information beyond the norm-only free gap; its associated
normalized phase kick is exactly reversible and Fock-norm preserving.  The
quartic operator is Hermitian, and the kick equals it on the distinguished pair
sector while acting identically off that sector.  A strict
alias-free discrete-time successor, changing-lattice PDE convergence, and a
local second-quantized continuum theory remain open; an absolute mass requires
an additional scale-bearing input.
\end{abstract}
...
Six contributions organize the paper.
\begin{enumerate}[leftmargin=2em]
  ...
  nonzero $|z|$.
  \item We give the finite fermionic many-body seed explicitly: a generic
  occupation-basis construction satisfies all canonical anticommutation
  relations, and a four-mode quartic pair transfer has oriented two-particle
  amplitudes $z$ and $\bar z$.  At the primitive-spinor fixture $z=3+4\ii$ the
  two channels differ exactly, and the associated normalized pair kick is
  an explicit complex-linear automorphism, involutive and Fock-inner-product
  preserving.  This is a
  finite unitary two-particle operation; the quartic operator is Hermitian and
  agrees with the kick on the
  pair sector.  The kick fixes every one-particle occupation state but is
  nontrivial on the displayed two-particle state.  This is not yet a completed
  second quantization of the spatial walk: only the determinant-minor vacuum,
  one-particle, number, and parity theorems are landed.  Creation covariance,
  functoriality, inherited locality, an operator-exponential theorem, and a
  continuum vertex remain open.
\end{enumerate}
...
Finite two-particle probe
 ...
 & The exhibited quartic transfer reuses the same wedge as amplitudes $z$ and $\bar z$\\
Absolute scale
 & Not predicted
...
converges uniformly on bounded $1+1$ momentum boxes; a finite local $3+1$
 ...
 defect index and localized mode, creation covariance/functoriality of the
 determinant-minor lift, inherited many-body locality, and a continuum
 interaction theory.\\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}[leftmargin=2em]
  ...
  monotone band exhaustion are now closed for arbitrary $L^2$ data; what
  remains is the concrete scaled sampling/interpolation isometry and its
  composition with the walk symbol.
  ...
  phase has zero winding, while an explicit periodic link field with winding
  one is proved not to admit such a lift.  The next theorem must derive that
  branch/transition data, or a sign/domain wall through the collinear zero set,
  from the local Pluecker field, reduce the resulting operator to a finite
  index, and exhibit a localized protected mode.  The existing finite winding
  operator is not yet derived from this local field.
  ...
  the one-particle matrix, and preserves number and parity.  Complete its
  creation covariance, functoriality, unitarity, and inherited causal locality,
  then derive an operator-exponential law and the quartic dynamics from one
