...
\begin{abstract}
The rest gap is normally an independent input to a Dirac quantum walk.  We
derive its operator from supplied null-spinor geometry and prove, with a
machine-checked finite development, exactly what that derivation adds.  For
$P=\sum_i\psi_i\psi_i^\dagger$ built from massless rank-one momenta,
\[
  \det P=\sum_{i<j}|\psi_i\wedge\psi_j|^2,
\]
and the invariant vanishes exactly when the occupied spinors span at most one
complex direction.  For two spinors, the complex area
$z=\psi_1\wedge\psi_2$ uniquely determines the odd Hermitian rest operator
\[
  B_z=\begin{pmatrix}0&z\\ \overline z&0\end{pmatrix},\qquad
  B_z^2=\det(P)\id,\qquad
  (k\sigma_z+B_z)^2=(k^2+\det P)\id.
\]
Thus the scalar mass input is replaced by null-spinor data, not eliminated:
the construction derives the form and geometric zero locus of the gap but does
not predict an absolute mass scale.
...
on compact momentum sets at rate $O(1/n)$, and a position-dependent field
$z(x)$ yields an exactly reversible finite-torus update with a strict causal
cone and explicit endpoint links under local chiral rotation.  Most
importantly, the oriented datum is already visible in the \emph{free}
one-particle spectrum.  For equal-modulus site fields coupled by an edge phase
$w$, the exact two-site Hamiltonian satisfies
\[
 (H^2-(|z_L|^2+t^2)\id)^2
 =t^2|z_R-(\overline w)^2z_L|^2\id.
\]
Hence transported relative phase, unlike a common phase, cannot be reproduced
by assigning only the scalar $|z|$.  The same chiral covariance uniquely
selects a phase-reading quartic pair coupling at block level; its local Fock
placement, exact pulse law, and finite causal support are also verified.

The formal audit exposes a sharp regulator boundary rather than hiding it.  In
the ordered $3+1$ successor, all zero- and $\pi$-quasienergy crossings are
classified and their charges cancel at each gap, while exact no-go theorems
show that the live minimal architecture retains high-frequency aliases and
cannot open a global Floquet gap with a momentum-independent onsite coin.
Accordingly, the result is a derived finite rest operator with a
phase-sensitive free observable and a verified dynamical chain, not a completed
continuum field theory.  Position-space PDE convergence, an alias-free local
$3+1$ successor, a fully interleaved interacting Fock dynamics, and absolute
mass selection remain open.
\end{abstract}
...
the rest operator and its finite dynamical chain.  Second, transported relative
phase is a free-spectrum observable rather than merely a supplied interaction
phase.  Third, the minimal ordered $3+1$ architecture has a completely audited
high-frequency boundary, turning the search for a clean strict regulator into
a precise algebra problem.  Seven technical contributions support those
headlines.
\begin{enumerate}[leftmargin=2em]
  ...
  are separated at the displayed fixtures $z=3+4\ii$ and $z=5$, inside the
  supplied pair-kick sector, by a double-kick
...
\begin{itemize}[leftmargin=2em]
  ...
  onsite coin, the complex Pluecker mass coin included, removes any of
...
$B_w$ to $K(z)$.  The pre-registered selection question --- which
...
\eqref{eq:l2wavepacket} for
  ...
  The concrete cell-sampling dense core is now closed as well.  Let $f$ be a
...
The first exact primitive to survive that obstruction uses a genuinely
...
additional generic roots in the external oracle; any successful embedding now
...
path, and the endpoint formulas are composed in one theorem.  This condition
was exposed by hostile audit and is now NewResult{} Kernel{} as a reusable
reality gate, not as a root-existence theorem.
...
construction is the next honest test.
...
The first of those gaps is now closed constructively.  Three explicit rational
...
The first global test is already negative.  At the distinct torus phase
...
kernel-proved nonconstant.  An exact-coefficient oracle consistently finds two
additional off-corner identity roots.  The first is now an exact kernel
theorem:
\[
 (z_x,z_y,z_z)=
...
off-axis crossing is now exact as well.  In tangent coordinates its third
...
\begin{enumerate}[leftmargin=2em]
  ...
  composition is now closed: a coefficient-local unitary one-particle update,
  ...
  generator-level gate is now closed: covariance of
...
checkerboard corner ratio $-\ii\tan(a\mu)$.  The
   ...
   spinors.  A common phase is invisible to the one-particle rest spectrum, but
   transported relative phase changes the exact two-site free spectrum through
   $\Delta=z_R-(\overline w)^2z_L$; the supplied pair-kick sector provides a
   second, two-particle interference readout.  These statements form
   one finite
