...
not one determinant; that dimensional argument, not the kernel check, is what makes
the ceiling *universal*). The *failure of the naive rank-3 extension* is kernel-checked
at explicit points: at rank 2 the
determinant *is* `mass²` (`RankCeiling.rank2_ok`), but at rank 3 the naive `det P₃`
is a *different function* from the physical pairwise mass — they disagree on explicit
witnesses and `det P₃` even vanishes on a nonzero-mass configuration
(`RankCeiling.rank3_det_ne_pairwise`, **M**). The table is a rank-≤2 statement; higher
spin is out of scope, not claimed. (3) **Phase-blindness.** `det P = |m|²` keeps the modulus and discards
...
spinor space* — massless is rank-collapse, massive is spread. And the determinant
is not an arbitrary choice of "area": among all quadratic forms on the (symmetric
`2×2`) momentum matrix, it is the **unique** one, up to scale, that vanishes on
every null edge — so rank-collapse *forces* exactly this invariant
(`DetPUniqueness.detP_unique`, **M**; remarkably, null-vanishing alone suffices, and
the classical `SL(2)` congruence-invariance `det` also enjoys is not even needed).
So:
...
physical pion or rho. The bulk of a nucleon's mass is *dynamical* QCD: the trace
anomaly / gluon field energy, Wilczek's "mass without mass" `[import]`, generated by
confinement and dimensional transmutation. That mechanism is a different object from
`det P`, and our finite constructions do **not** derive it; the det-`P` and
binding-defect statements are the kinematic/structural bookkeeping of disagreement
and coherence, not the dynamical origin of the ~99 % of hadron mass that confinement
supplies. Where the headline reads across composites, it re-labels that mass in
null-edge coordinates — it does not explain it.
...
drift) is the pre-registered falsifier — now itself kernel-checked: a sum-of-squares
...
spectral action whose order-0 term is `Λ` (`SpectralActionAvatar`, §10a). That
single action is now varied *both* ways in one kernel-checked statement: the
...
perturbations), `even_can_gap`, **M**). So the honest picture is: *balanced* grading has
no protection (the `EvenMassGaps` toy, delivering only the "every potential"
counterexample); *imbalanced* grading has genuine index-protection robust to all odd
perturbations (the correct demonstration). The index — the imbalance — is the whole
content.
...
3. **Index-protected (chiral).** A chiral surplus (imbalanced grading) *forces* a zero
   ...
   this section). Its robustness is now kernel-checked in the imbalanced case: for
   `Γ=diag(1,1,−1)`, *every* odd perturbation preserves a zero mode
   (`ImbalancedChiralProtection.odd_has_zero_mode`, `∀`-quantified, **M**), by the
   `1×2`-kernel rank argument — the genuine index / SSH–BdG protection; an *even* mass
   term can still gap a given mode (above). No tuning — the imbalance is the whole content.
...
novelty; `α < 1` gains novelty but kills the number — not both. The *arithmetic*
underlying this dichotomy is kernel-checked (`LambdaExponentFork.fork_iff`,
`hyperuniform_faster`, **M**: `lamExp(α)=α/2−1`, and `lamExp(α)=−½ ⟺ α=1`) — but,
honestly, that theorem certifies only the linear solve; the *physics* (that `lamExp`
is the `Λ`-exponent, and `−½` the everpresent threshold) lives in the model definitions
above, argued in prose, not derived by the arithmetic. The kernel confirms the fork is
*decidable on `α`*; it does not by itself justify the exponent form.
...
| 3 | `det_vanishes_on_edges`, `detP_unique`, `acCoeff_not_null`, `uniqueness_verdict` | `NullEdge/DetPUniqueness.lean` | M, self-guarded (in-file pin) | **det P is canonical, not chosen**: among all quadratic forms on symmetric `2×2` matrices, the determinant is (up to scale) the *unique* one that vanishes on every null edge — `null-vanishing alone` forces `Q = k·det` (SL(2)-congruence invariance, which `det` also has, is not even needed in the proof). An explicit control (`Q'(P)=ac`) vanishes on the coordinate edges but not the diagonal one, so null-vanishing is a real constraint. Answers "why the determinant?" — it is forced |
| 3 | `det_P_eq_massSq`, `null_iff_massless`, `P_selfAdjoint`, `pauli_momentum_verdict` | `NullEdge/PauliMomentumPhysLean.lean` (port) | M, self-guarded (in-file pin) | **`det P = m²` grounded in PhysLean's Pauli convention (port)**: the σ-map `P(p)=p_μσ^μ` (PhysLean `PauliMatrix`) is the Hermitian little-group matrix with `det P = p₀²−p₁²−p₂²−p₃² = m²` and `det P = 0 ⟺` null — massive/null/spacelike witnesses (`det = 1/0/−1`). This is the **correct `P`** the audit flagged (little-group spinor, not the 4-vector Gram); grounds §3's central identity in PhysLean's machine-verified convention |
| 3 | `boost_on_shell`, `mass_boost_invariant`, `frame_dependence_control`, `boost_invariance_verdict` | `NullEdge/RationalBoostInvariance.lean` | M, self-guarded (in-file pin) | **mass is boost-invariant (why it must be the `det`)**: an explicit *rational* Lorentz boost `L=[[5/4,3/4],[3/4,5/4]]` (`β=3/5`) preserves `η` (`LᵀηL=η`, `det L=1`) and leaves `mass²=det P=E²−k²` **invariant**, while individual momentum components change (`(1,0)↦(5/4,3/4)`). So `mass=det P` is frame-independent *because* it is the determinant — the little-group invariant, not an arbitrary component/minor (closes the audit's frame-dependence point) |
| 3 | `rank2_ok`, `rank3_det_ne_pairwise`, `rank_ceiling_verdict` | `NullEdge/RankCeiling.lean` | M, self-guarded (in-file pin) | **the rank-2 ceiling (honest boundary)**: at rank 2 the determinant *is* `mass²` (`det!![p,x;x,q]=pq−x²`); at rank 3 the naive `det P₃` is a *different function* from the physical pairwise mass `Σ2p_i·p_j` — they disagree on explicit witnesses (`6≠2`), and `det P₃` even vanishes on a nonzero-mass config (`4` vs `0`). Kernel-refutes `det P₃ = massPair` *as an identity* at explicit rank-3 points (the "intrinsically two-edge" universal rests on the dimensional argument, not these witnesses); higher spin is *not* claimed (formalizes the audit kill-test as a boundary) |
...
| 3 | `P_eq_null_edge_sum`, `det_is_disagreement`, `det_zero_iff_massless`, `det_pos_iff_massive`, `sigmamap_null_edge_verdict` | `NullEdge/SigmaMapNullEdges.lean` (port) | M, self-guarded (in-file pin) | **the σ-map IS a sum of null edges (closes "which `P`")**: the PhysLean Pauli σ-map `P(p)=p_μσ^μ` — the *same* little-group `P` the audit flagged (row `PauliMomentumPhysLean`) — is *exactly* a nonneg sum of two rank-1 null-edge dyads, `P=(E+kz)·edge e₀+(E−kz)·edge e₁`; its determinant is the null-edge *disagreement* `(E+kz)(E−kz)=E²−kz²=m²` (product of edge weights), collapsing to *one* edge (`det=0`) at masslessness and *two* when massive. So the PhysLean-grounded `P` and the manuscript's null-edge Gram `M Mᴴ` are the **same** `P`, not two constructions — the bridge from `PauliMomentumPhysLean` (the correct `P`) to `MasslessEdgeCount` (edge count = rank). Honest scope: real `(t,z)` rational avatar; complex case identical with Hermitian dyads |
| 3 | `dirac_det`, `det_zero_iff_massshell`, `det_pos_iff_off_shell`, `massless_det`, `dirac_massshell_det_verdict` | `NullEdge/DiracOperatorMassShellDet.lean` (port) | M, self-guarded (in-file pin) | **the 4-spinor Dirac-operator determinant face of mass**: the full `4×4` Dirac operator `D=pslash−m·1` has `det D = (m²−E²+kz²)² = (m²−p²)²`, which vanishes **exactly** on the mass shell (`det D=0 ⟺ E²−kz²=m²`, `D` singular) and is a *positive square* off shell (`D` invertible). Because `pslash²=(E²−kz²)·1`, the characteristic polynomial is a perfect square so the root cancels — purely rational, no `√`. This is the 4-spinor companion to the `2×2` little-group `det P = m²` (`PauliMomentumPhysLean`), and it realizes the program's determinant-level mass-shell test `det D(q)=0`. At `m=0`, `det D=(E²−kz²)²=(det P)²` (`massless_det`, stated as interpretation — the little-group `P` is a *separate* object). `[import]` Dirac dispersion, real `(t,z)` avatar |
...
| 4 | `witness_match`, `family_fails`, `opposite_sign`, `signmismatch_verdict` | `NullEdge/BudgetSignMismatch.lean` | M, self-guarded (in-file pin) | **the sign-mismatch kill-test, kernel-checked**: a sum-of-squares `budget = a²+b²+2x²` matches `c·detP` (`detP = ab−x²`) at one witness (`c=10/3` at `(2,2,1)`) but the *same* `c` fails at `(2,2,0)`; `budget` rises and `detP` falls with `|x|` (opposite sign). Confirms `totalBudget = c·det P` is witness-fitted, not a family identity — the §4 boundary as a theorem (the audit→theorem arc, cf. `RankCeiling` for §3) |
| 4 | `groundmass_det_family_law`, `det_eq_eigenvalue_product`, `defect_exact`, `spectral_answers_to_invariants`, `ground_mass_det_verdict` | `NullEdge/GroundMassDetFamilyLaw.lean` | M, self-guarded (in-file pin) | **the correct DETERMINANT-side family law (positive completion of the kill-test arc)**: where the row above *killed* the trace-side `budget = c·det P`, this is the exact law it pointed to. On the aperture-closure block `P=!![λ,κ;κ,λ]` the **ground mass** `μ₋=λ−κ` = free mass `λ` + defect (`−κ` **exactly**, `defect_exact`, not a fitted `c·det P`); `det P = μ₋·μ₊ = μ₋(2λ−μ₋)` (`groundmass_det_family_law`), so the ground mass **answers to `det` given `tr`**, exactly over the whole family — the spectrum is an exact function of `(tr,det)` (`spectral_answers_to_invariants`), whereas the trace-side budget is *not* a function of `det` alone (`BudgetSignMismatch`). Converts the central §3↔§4 tie from a witness-level coincidence into a **law**. (Spectrum/`det` of this block are also in `MassPhaseDiagram`; the new content is the family-law synthesis + exact defect + spectral-vs-invariants contrast.) |
...
| 10a | `everpresent_value`, `hyperuniform_faster`, `fork_iff`, `exponent_fork_verdict` | `NullEdge/LambdaExponentFork.lean` | M, self-guarded (in-file pin) | **the sharp decidable fork**: the `Λ`-exponent `lamExp(α)=α/2−1` (from `Λ_rms=N^{α/2}/N`) is strictly increasing in the count-variance exponent `α`, with `lamExp(α)=−½ ⟺ α=1`; `α<1` (hyperuniform) gives `lamExp<−½` (faster decay, the `10⁻¹²²` number fails), `α>1` slower. The pre-registered kill as a theorem: everpresent survives iff `α=1` — keep the number *or* gain hyperuniform novelty, not both |
...
| 2a | `eta_eq_indefiniteDiagonal`, `null_iff`, `convention_note` | `NullEdge/MinkowskiConvention.lean` | M, self-guarded (in-file pin) | **Minkowski convention grounded (port)**: our `η=diag(1,−1,−1,−1)` = the reindexed Mathlib `LieAlgebra.Orthogonal.indefiniteDiagonal` = PhysLean's `minkowskiMatrix` convention (`(+,−,−,−)`); `mink u u=0 ⇔` null cone. Clean-room port, provenance recorded |
...
| 8 | `index_is_one`, `odd_has_zero_mode`, `even_can_gap`, `chiral_protection_verdict` | `NullEdge/ImbalancedChiralProtection.lean` | M, self-guarded (in-file pin) | **the correct chiral protection** (the honest replacement): for the *imbalanced* grading `Γ=diag(1,1,−1)` (`tr Γ=1`, index 1), **every** odd perturbation preserves a zero mode in `H₊` — `odd_has_zero_mode` is `∀`-quantified over the odd operators, via the `1×2`-kernel rank argument, so it is *not* cherry-picked — while an even (grading-breaking) term can gap it. This is the genuine index / SSH–BdG protection the balanced `EvenMassGaps` toy could not exhibit; the imbalance is the whole content |
