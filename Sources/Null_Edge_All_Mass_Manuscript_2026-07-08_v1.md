...
| 4 | `HAC_eq_clifford`, `Jmet_eq_clifford` | `Carrier/CliffordAssembly.lean` | M, guard-pinned (`CarrierAxiomGuard`) | **T2 carrier provenance, kernel-certified**: the hand-typed Krein form/metric IS the Cl(4) Kronecker assembly `J(Q_A+Q_C)` / `Js⊗I3` (verbatim, no convention change) — closes the flagship audit's docstring-provenance gap |
