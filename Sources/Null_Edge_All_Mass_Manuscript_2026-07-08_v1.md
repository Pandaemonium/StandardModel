...
not one determinant). This ceiling is itself now kernel-checked: at rank 2 the
...
novelty; `α < 1` gains novelty but kills the number — not both. This exponent
dichotomy is now kernel-checked (`LambdaExponentFork.fork_iff`,
`hyperuniform_faster`, **M**: `lamExp(α)=α/2−1`, and `lamExp(α)=−½ ⟺ α=1`).
