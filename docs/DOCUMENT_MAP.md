...
- `PhysicsSM/Draft/NullEdge/PeriodicVacuumWeylLinearizedBackreaction.lean` and `Scripts/oracle/null_wave_linearized_backreaction.py` - exact identity-background Hessian successor to the fixed-link no-go. Lean proves all 48 link and 32 coframe equations for an explicit curved plus-like coupled perturbation, identifies its curl with twice the guarded vacuum-Weyl wave, proves a flat injective 12-parameter local Lorentz family lies in the joint kernel, and proves the curved class is not gauge. The independent exact-rational oracle reports an 80-variable Hessian of rank 52 and nullity 28 with a two-dimensional curvature image on the vacuum-Riemann kernel; only one curved generator is currently landed in Lean. Nonlinear continuation, gauge-reduced propagation, Levi-Civita selection, and refinement convergence remain open. [DRAFT-LEAN/ORACLE]
