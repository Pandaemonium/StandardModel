...
- `PhysicsSM/Draft/NullEdge/PeriodicVacuumWeylCurvatureRankTwo.lean`, `PeriodicVacuumWeylSecondOrderObstruction.lean`, `SecondOrderCurveExpansion.lean`, and `PeriodicVacuumWeylAnalyticNoBranch.lean` - the complete stationary additive-curvature image of the concrete two-site joint linearization is exactly an injective two-real-dimensional plus/cross range. Its integrated time-time second-order coframe Euler charge is exactly `-8(p^2+c^2)` and is independent of arbitrary second-order link corrections. Explicit `o(t^2)` expansion witnesses propagate this obstruction through the noncommutative plaquette and actual nonlinear coframe Euler curves, proving that every Taylor-admissible identity-based branch that is coframe-stationary near the background has `p=c=0`, independently also of the second-order coframe correction. Gauge completion, generic `C^2` curve packaging, propagation, refinement, and continuum Taub identification remain open. [DRAFT-LEAN]
...
- `PhysicsSM/Draft/NullEdge/NonlinearLorentzPalatiniAffineConnectionTorsionSelection.lean` - first connection-dependent P8(a) successor. It derives the Lorentz four-vector and Palatini bivector tangents, their Hodge commutation and Krein-skew sign, and identifies the arbitrary-invertible-tetrad linear connection equation with `de + omega wedge e = 0` under the predecessor convention. The finite affine residual is exactly `h L + h^2 Q - h^3 C`; a nonzero shrinking-spacing family forces zero covariant torsion, and a connection-generated coframe jet witnesses nonvacuity. Nonidentity exact exponential links, arbitrary-background agreement with the full nonlinear link Euler equation, finite-spacing local uniqueness, varying jets, and graph refinement remain open. [DRAFT-LEAN]
- `PhysicsSM/Draft/NullEdge/CausalLeviCivita.lean` and `NonlinearLorentzPalatiniCoordinateLeviCivita.lean` - guarded finite first-jet fundamental theorem and Palatini selection bridge. A supplied inverse metric and symmetric first jet admit a unique torsion-free, metric-compatible coordinate connection. For any invertible coframe, vanishing affine connection residuals make `eInv (hat(omega_a)e - V_a)` equal that Christoffel connection; at the identity coframe, vanishing ordinary derivatives of the actual nonlinear link Euler coefficients gives the same endpoint for site-uniform link variations. Exact nonidentity-link finite-spacing selection and graph-derived refinement remain open. [DRAFT-LEAN]
- `PhysicsSM/Draft/NullEdge/LorentzCoordinateCurvatureBridge.lean` - exact finite second-jet tetrad curvature dictionary. With symmetric mixed predecessor coframe second jets, the curvature of `Gamma_a = eInv (hat(omega_a)e - V_a)` equals `eInv F_ab e`, including both connection commutators. The full differential curvature is proved to remain in the matrix Lorentz Lie algebra and is reconstructed exactly from the same six ordered coordinates used by the action. A guarded capstone bundles this with arbitrary-coframe affine Palatini-to-Christoffel selection. Graph-derived second jets and finite-plaquette-to-differential-curvature convergence remain open. [DRAFT-LEAN]
- `PhysicsSM/Draft/NullEdge/LorentzCoordinateEinsteinContraction.lean` - exact coordinate contraction successor. It identifies twice the ordinary mixed Einstein tensor of the induced coordinate curvature with the action's convention-normalized coframe Euler combination, proves local Palatini coframe stationarity equivalent to `G^d_c = 0`, and gives a guarded supplied-jet capstone where affine connection plus coframe stationarity imply Christoffel selection and coordinate vacuum Einstein. Graph-derived jets, finite-plaquette identification, and refinement convergence remain open. [DRAFT-LEAN]
- `PhysicsSM/Draft/NullEdge/NonlinearLorentzPalatiniSampledCoordinateEinsteinLimit.lean` - guarded changing-carrier coordinate endpoint for derivatives of the actual nonlinear Palatini action. On a supplied common affine stencil, joint sampled Euler-map derivative stationarity gives first-jet Christoffel selection and the standard coordinate mixed Einstein equation for the curvature reconstructed from `dA`. A compatible second tetrad jet, the nonlinear `A wedge A` term, graph-derived sampling, and unconditional refinement remain open. [DRAFT-LEAN]
- `PhysicsSM/Draft/NullEdge/PeriodicVacuumWeylLinearizedBackreaction.lean`, `PeriodicVacuumWeylCurvatureCompletenessData.lean`, `PeriodicVacuumWeylCurvatureCompleteness.lean`, `PeriodicVacuumWeylCurvatureRankTwo.lean`, `PeriodicVacuumWeylSecondOrderObstruction.lean`, `Scripts/oracle/null_wave_linearized_backreaction.py`, `Scripts/oracle/emit_null_wave_curvature_completeness_lean.py`, and `Scripts/oracle/null_wave_nonlinear_continuation.py` - exact identity-background Hessian and second-order successors to the fixed-link no-go. Lean proves all 48 link and 32 coframe equations for explicit curved plus-like and cross-like coupled perturbations and their full real span, proves that their curls are independent vacuum-Riemann directions, proves a flat injective 12-parameter local Lorentz family lies in the joint kernel, and proves that no nonzero plus/cross combination is gauge. The generated-data successor checks all 80 sparse Hessian rows against the concrete-action derivatives and checks exact row-space certificates for every curvature residual. Lean consequently proves that every stationary curvature has a unique plus/cross pair and that the complete stationary curvature image is exactly the range of an injective two-real-parameter map, excluding a third curvature direction anywhere in the full kernel. The formal second-order exponential-jet successor proves that the integrated time-time coframe Euler charge annihilates every second-order link correction but equals `-8(p^2+c^2)` on the curved sector, so no nonzero plus/cross mode passes the same-carrier continuation condition. This triggers the two-site nonlinear-continuation kill condition and redirects the positive route to a larger/open carrier, boundary flux, changing background, matter, or modified weights. The independent exact-rational oracle still supplies the unformalized Hessian rank 52 and nullity 28. Gauge-quotient completeness, propagation, an analytic arbitrary-curve no-branch bridge, Levi-Civita selection, and refinement convergence remain open. [DRAFT-LEAN/ORACLE]
