...
- `PhysicsSM/Draft/NullEdge/PeriodicVacuumWeylLinearizedBackreaction.lean`, `PeriodicVacuumWeylCurvatureCompletenessData.lean`, `PeriodicVacuumWeylCurvatureCompleteness.lean`, `PeriodicVacuumWeylCurvatureRankTwo.lean`, `Scripts/oracle/null_wave_linearized_backreaction.py`, and `Scripts/oracle/emit_null_wave_curvature_completeness_lean.py` - exact identity-background Hessian successor to the fixed-link no-go. Lean proves all 48 link and 32 coframe equations for explicit curved plus-like and cross-like coupled perturbations and their full real span, proves that their curls are independent vacuum-Riemann directions, proves a flat injective 12-parameter local Lorentz family lies in the joint kernel, and proves that no nonzero plus/cross combination is gauge. The generated-data successor then checks all 80 sparse Hessian rows against the actual concrete-action derivatives and checks exact row-space certificates for every curvature residual. Lean consequently proves that every stationary curvature has a unique plus/cross pair and that the complete stationary curvature image is exactly the range of an injective two-real-parameter map, excluding a third curvature direction anywhere in the full kernel. The independent exact-rational oracle still supplies the unformalized Hessian rank 52 and nullity 28. This is not yet a physical two-polarization theorem: gauge-quotient completeness, propagation, nonlinear continuation, Levi-Civita selection, and refinement convergence remain open. [DRAFT-LEAN/ORACLE]
