...
| 3 | I1.8 | Normalized dictionary (det rho = m^2/4E^2, concurrence = m/E, entropy-velocity) | Draft determinant/purity/faithfulness dictionary ported in GateI1 core. EIGENVALUE-ORDERING + VON NEUMANN (BINARY) ENTROPY LANDED (2026-07-05, `GateI1/MassEntropyDictionary.lean`, kernel-checked, axioms propext/Classical.choice/Quot.sound): normalized-block eigenvalues `(1 +/- |v|)/2` with ordering/sum/product, `vonNeumannEntropy`, nonnegativity, `negMulLog_eq_zero_iff`, and the headline `vonNeumannEntropy = 0 <-> minkowskiSq = 0` ("null edges do not age": null <-> pure <-> zero visible entropy) + `vonNeumannEntropy_pos_of_timelike`. OBSERVER-CONDITIONED (normalized rho, frame-dependent E); invariant statement stays det P = m^2. Remaining optional extension: full binary-entropy monotonicity/`= log 2`-at-rest quantitative package | night |
