...
| 8 | C1 | Tetrahedral operator gap -> self-adjointness -> sign/GW release | FREE-OPERATOR HALF DONE (draft-trust, 2026-07-03): coercive gap `tetraFreeOperator_gap_equalN`, self-adjointness `Hfree_selfAdjoint`, symbol Hermiticity `H_symbol_hermitian`, no-zero-modes `Hfree_ker_trivial` all kernel-checked (axioms propext/Classical.choice/Quot.sound). NEXT RUNG = sign(Hfree)/GW release; setup + representation-bridge plan in `AgentTasks/nerd-gate-c1-gw-release-setup-2026-07-03.md`. Then Gate C2 (gauge). `PhysicsSM/Draft/NullEdge/GateC1/` | day |
